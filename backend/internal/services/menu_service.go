...
type IMenuService interface {
	GetPaginatedMenus(page, limit int, search string) (*utils.SPaginatedResponse, error)
...
func (s *menuService) GetPaginatedMenus(page, limit int, search string) (*utils.SPaginatedResponse, error) {
	menus, total, err := s.repo.FindPaginated(page, limit, search)
	if err != nil {
		return nil, err
	}
	return utils.NewPaginatedResponse(menus, total, page, limit), nil
}
