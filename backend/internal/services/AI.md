...
This file described the **old layered architecture** (`internal/services/` etc.). DAPEN has migrated to **Domain-Based + DDD-Lite + Layered** architecture (see `~/.gemini/ARCHITECTURE.md` and `~/.claude/projects/.../memory/dapen-architecture.md`).
...
- `internal/identity/user/service.go` — User business logic
- `internal/identity/permission/service.go` — Permission business logic (per-tab reads, transactional updates, matrix generation)
- `internal/identity/auth/service.go` — Auth business logic
- `internal/menu/service.go` — Master menu business logic
...
The `internal/services/` global folder will be DELETED once all domains have been migrated. Track progress in `tasks/` and the plan file.
