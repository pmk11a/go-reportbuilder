...
type SKasBankHeader struct {
	...
	// Note is the free-text header note (max 500 chars) — displays as "Kode Project" in the UI.
	Note string `json:"note"`
	// TglJurnal is the due date / "Batas Waktu" (from DBTRANS.TglJurnal).
	TglJurnal *time.Time `json:"tgljurnal"`
	// NoJurnal is the order number / "No. Order" (from DBTRANS.NoJurnal).
