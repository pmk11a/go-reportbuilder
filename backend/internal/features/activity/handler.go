...
func (h *SActivityLogHandler) GetConfigs(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to retrieve configurations")
		return
	}
	response.Success(c, "Configurations retrieved successfully", configs)
}
...
func (h *SActivityLogHandler) GetConfigByTableName(c *gin.Context) {
	...
	if err != nil {
		// Could be not found
		response.Success(c, "Configuration not found", nil)
		return
	}
	response.Success(c, "Configuration retrieved successfully", config)
}
...
// @Accept json
// @Produce json
// @Param request body SActivityLogConfigReq true "Configuration Data"
// @Success 200 {object} map[string]interface{}
...
func (h *SActivityLogHandler) GetTables(c *gin.Context) {
	...
	response.Success(c, "Tables retrieved successfully", tables)
}
