...
This file described the **old layered architecture** (`internal/legacy/handlers/` etc.). DAPEN has migrated to **Domain-Based + DDD-Lite + Layered** architecture (see `~/.gemini/ARCHITECTURE.md` and `~/.claude/projects/.../memory/dapen-architecture.md`).
...
- `internal/features/identity/user/handler.go` — User CRUD HTTP layer
- `internal/features/identity/permission/handler.go` — Permission HTTP layer (with per-tab + matrix + export)
- `internal/features/identity/auth/handler.go` — Auth (login, refresh, change-password) HTTP layer
...
- Use `internal/infrastructure/response/` helpers (e.g., `response.Success(c, "msg", data)`) for all responses.
...
The `internal/legacy/handlers/` global folder will be DELETED once all domains have been migrated. Track progress in `tasks/` and the plan file.
