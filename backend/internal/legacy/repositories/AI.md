...
This file described the **old layered architecture** (`internal/legacy/repositories/` etc.). DAPEN has migrated to **Domain-Based + DDD-Lite + Layered** architecture (see `~/.gemini/ARCHITECTURE.md` and `~/.claude/projects/.../memory/dapen-architecture.md`).
...
- `internal/features/identity/user/repository.go` — User CRUD persistence
- `internal/features/identity/permission/repository.go` — Per-user menu/report/COA permission persistence (dbflmenu, DBFLMENUREPORT, DBAKSESPERKIRAAN)
- `internal/features/identity/auth/repository.go` — Auth persistence
- `internal/features/menu/repository.go` — Master menu catalogue persistence
...
The `internal/legacy/repositories/` global folder will be DELETED once all domains have been migrated. Track progress in `tasks/` and the plan file.
