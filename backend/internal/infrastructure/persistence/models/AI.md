...
This file described the **old layered architecture** (`internal/infrastructure/persistence/models/` etc.). DAPEN has migrated to **Domain-Based + DDD-Lite + Layered** architecture (see `~/.gemini/ARCHITECTURE.md` and `~/.claude/projects/.../memory/dapen-architecture.md`).
...
- `internal/features/identity/user/entity.go` — `SUser`, `SDBFLPASS`
- `internal/features/identity/permission/entity.go` — `SUserPermission`, `SUserCoaAccess`, `SPermissionReportRow`
- `internal/features/identity/auth/entity.go` — Auth entities
- `internal/features/menu/entity.go` — `SDbMenu`, `SDBMENUREPORT`
...
The `internal/infrastructure/persistence/models/` global folder will be DELETED once all domains have been migrated. Track progress in `tasks/` and the plan file.
