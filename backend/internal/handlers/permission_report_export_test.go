...
func buildSampleReportRows(n int) []dto.SPermissionReportRowDTO {
	rows := make([]dto.SPermissionReportRowDTO, n)
	for i := 0; i < n; i++ {
		rows[i] = dto.SPermissionReportRowDTO{
...
func TestFormatPermissionReportXLSX_MinimalRows(t *testing.T) {
	...
	filters := dto.SReportFilters{MenuType: "menu"}
...
func TestFormatPermissionReportXLSX_EmptyRows(t *testing.T) {
	var buf bytes.Buffer
	err := formatPermissionReportXLSX(nil, dto.SReportFilters{}, &buf)
	assert.NoError(t, err)
...
func TestFormatPermissionReportXLSX_WithFilters(t *testing.T) {
	...
	filters := dto.SReportFilters{UserID: &uid, MenuID: &mid, MenuType: "report"}
...
func TestFormatPermissionReportXLSX_NilWriterReturnsError(t *testing.T) {
	err := formatPermissionReportXLSX(nil, dto.SReportFilters{}, nil)
	assert.Error(t, err)
}
...
func TestFormatPermissionReportPDF_EmptyRows(t *testing.T) {
	var buf bytes.Buffer
	err := formatPermissionReportPDF(nil, dto.SReportFilters{}, &buf)
	assert.NoError(t, err)
...
func TestFormatPermissionReportPDF_PaginatesLargeReport(t *testing.T) {
	...
	var buf bytes.Buffer
	err := formatPermissionReportPDF(rows, dto.SReportFilters{MenuType: "report"}, &buf)
	assert.NoError(t, err)
	assert.NotEmpty(t, buf.Bytes())
}
...
func TestFormatPermissionReportPDF_NilWriterReturnsError(t *testing.T) {
	err := formatPermissionReportPDF(nil, dto.SReportFilters{}, nil)
	assert.Error(t, err)
}
...
func TestBuildFilterSummary(t *testing.T) {
	t.Run("Empty filters defaults menuType to menu", func(t *testing.T) {
		s := buildFilterSummary(dto.SReportFilters{})
	...
	t.Run("With userId and menuId", func(t *testing.T) {
		uid := "ADMIN"
		mid := "M01"
		s := buildFilterSummary(dto.SReportFilters{UserID: &uid, MenuID: &mid, MenuType: "menu"})
	...
	t.Run("Report type passes through", func(t *testing.T) {
		s := buildFilterSummary(dto.SReportFilters{MenuType: "report"})
...
func TestFormatPermissionReportRowExcel(t *testing.T) {
	row := dto.SPermissionReportRowDTO{
		UserID:     "ADMIN",
		FullName:   "Admin",
		Role:       "admin",
		MenuCode:   "M01",
