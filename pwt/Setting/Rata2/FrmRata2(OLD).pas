    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (xTipe='AWL') or (xTipe='PBL') or (xTipe='ADI') or (xTipe='RPJ') or (xTipe='TRI') or (xTipe='HP')
          ...
          begin
            ...
            begin
              ...
              xRefNoBukti:=DM.QuKOde.FieldByName('NoBukti').AsString;
              xRefUrut:=DM.QuKOde.FieldByName('Urut').AsInteger;
              if DataBersyarat('select isnull(B.HPP,0) HPP from DBRPenyerahanBhndet A '+
          ...
          if (xTipe='RPB') or (xTipe='PNJ') or (xTipe='TRO') or (xTipe='UKO') or (xTipe='ADO') or (xTipe='PMK')
            or (xTipe='PBO') then
            InHPPtoTRS(DM.QuKOde.Fieldbyname('Nobukti').AsString, DM.QuKOde.Fieldbyname('Urut').AsString, xTipe, xHrgRata);
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                SQL.Add('values ('+QuotedStr(IDUser)+','+IntToStr(iUrutStockMinus)+','+QuotedStr(xJenisBahan)+','+QuotedStr(xKodeGdg)+','+QuotedStr(xKodeBrg)+','''','''','+QuotedStr('')+')');
                ExecSQL;
              end;
              QuBeli.Close;
              QuBeli.Open;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SQL.Add('  QNTADI, Qnt2ADI, HRGADI, QNTADO, Qnt2ADO, HRGADO, QNTUKI, Qnt2UKI, HRGUKI, QNTUKO, Qnt2UKO, HRGUKO, ');
        ...
        SQL.Add('  a.QNTADI, a.Qnt2ADI, a.HRGADI, a.QNTADO, a.Qnt2ADO, a.HRGADO, a.QNTUKI, a.Qnt2UKI, a.HRGUKI, a.QNTUKO, a.Qnt2UKO, a.HRGUKO, ');
...

{
  ...
  begin
    ...
    SQL.Add('       Case when a.Tipe in (''BP'', ''RBP'',''ADI'',''PRD'') then Sum(a.HrgSaldo) else ');
    sql.Add('       Case when Sum(a.QntSaldo)<0 then -SUM(a.QntSaldo)');
    sql.Add('            else SUM(a.QntSaldo) ');
    sql.Add('       end*Max(C.HRGRATA) end DPP, Max(C.HRGRATA) HPP, a.XCode');
    sql.Add('from vwStock a');
    sql.add('     left outer join dbbarang b on b.kodebrg=a.kodebrg ');
    SQl.Add('     left outer join (Select Kodebrg, Bulan, Tahun, kodegdg, HrgRata From DBSTOCKBRG ');
    SQl.Add('                      ) C on C.KODEBRG=a.Kodebrg and C.BULAN=@bulan and C.TAHUN=@Tahun and c.KODEGDG=A.Kodegdg');
    sql.Add('where a.Bulan=@bulan and a.Tahun=@tahun and a.Tipe not in (''AWL'') ');
    if ComboBox1.ItemIndex=1 then
    begin
      Sql.add('      and (a.Kodebrg between @kodebrg1 and @kodebrg2) ');
    end;
    sql.Add('group by a.Kodebrg,B.Namabrg,A.Tipe, A.kodegdg, a.XCode');
    sql.Add('Order by a.Kodebrg');
    Prepared;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if DM.QuCari.FieldByName('Tipe').AsString='BP' then
        begin
        ...
        begin
          sql.Add('update DBstockbrg set HRGPB=@DPP ');
          Sql.add('where kodebrg=@kodebrg and Bulan=@bulan and Tahun=@tahun and Kodegdg=@kodegdg');
          sql.Add('update DBPenyerahanBHNdet set Hpp=@harga');
          sql.add('from DBPenyerahanBHN a ');
          Sql.add('where a.nobukti=DBPenyerahanBHNdet.nobukti and DBPenyerahanBHNdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
        end
        else if DM.QuCari.FieldByName('Tipe').AsString='RBPB' then
        begin
          sql.Add('update DBstockbrg set HRGRBP=@DPP ');
          Sql.add('where kodebrg=@kodebrg and Bulan=@bulan and Tahun=@tahun and Kodegdg=@kodegdg');
          sql.Add('update DBRPenyerahanBHNdet set Hpp=@harga');
          sql.add('from DBRPenyerahanBHN a ');
          Sql.add('where a.nobukti=DBRPenyerahanBHNdet.nobukti and DBRPenyerahanBHNdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
        end
        ...
        begin
          ...
          if (DM.QuCari.FieldByName('Tipe').AsString='ADO')  then
          begin
        ...
        else if (DM.QuCari.FieldByName('Tipe').AsString='PRD')  then
        begin
          sql.Add('update DBstockbrg set HRGHPrd=@DPP ');
          Sql.add('where kodebrg=@kodebrg and Bulan=@bulan and Tahun=@tahun and Kodegdg=@kodegdg');
          sql.Add('update DBHasilPRDdet set Hpp=@harga');
          sql.add('from DBHasilPRD a ');
          Sql.add('where a.nobukti=DBHasilPRDdet.nobukti and DBHasilPRDdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
        end
        else if (DM.QuCari.FieldByName('Tipe').AsString='INVC')  then
        begin
          sql.Add('update DBInvoicePLdet set Hpp=@harga');
          sql.add('from DBInvoicePL a ');
          Sql.add('where a.nobukti=DBInvoicePLdet.nobukti and DBInvoicePLdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
        end
        else if (DM.QuCari.FieldByName('Tipe').AsString='RINVC')  then
        begin
          sql.Add('update DBInvoiceRPJdet set Hpp=@harga');
          sql.add('from DBInvoiceRPJ a ');
          Sql.add('where a.nobukti=DBInvoiceRPJdet.nobukti and DBInvoiceRPJdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
        end
        else if (DM.QuCari.FieldByName('Tipe').AsString='SPB') then
        begin
          sql.Add('update DBstockbrg set HRGSPB=@DPP ');
          Sql.add('where kodebrg=@kodebrg and Bulan=@bulan and Tahun=@tahun and Kodegdg=@kodegdg');
          sql.Add('update DBSPBdet set Hpp=@harga');
          sql.add('from DBSPB a ');
          Sql.add('where a.nobukti=DBSPBdet.nobukti and DBSPBdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
        end
        else if (DM.QuCari.FieldByName('Tipe').AsString='RSPB')   then
        begin
          sql.Add('update DBstockbrg set HRGRSPB=@DPP ');
          Sql.add('where kodebrg=@kodebrg and Bulan=@bulan and Tahun=@tahun and Kodegdg=@kodegdg');
          sql.Add('update DBRSPBdet set Hpp=@harga');
          sql.add('from DBRSPB a ');
          Sql.add('where a.nobukti=DBRSPBdet.nobukti and DBRSPBdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
          sql.Add('update DBSPBRJUALdet set Hpp=@harga');
          sql.add('from DBSPBRJUAL a ');
          Sql.add('where a.nobukti=DBSPBRJUALdet.nobukti and DBSPBRJUALdet.kodebrg=@kodebrg and month(a.tanggal)=@bulan and year(a.tanggal)=@tahun');
        end
        else if (DM.QuCari.FieldByName('Tipe').AsString='TRI') or (DM.QuCari.FieldByName('Tipe').AsString='TRO')  then
        begin
          if (DM.QuCari.FieldByName('Tipe').AsString='TRI')  then
          begin
            sql.Add('update DBstockbrg set HRGTRI=@DPP ');
            Sql.add('where kodebrg=@kodebrg and Bulan=@bulan and Tahun=@tahun and Kodegdg=@kodegdg');
            sql.Add('update DBTransferdet set Hpp=@harga');
            sql.add('from DBTransfer a ');
            Sql.add('where a.nobukti=DBTransferdet.nobukti and DBTransferdet.kodebrg=@kodebrg and ');
            sql.add('month(a.tanggal)=@bulan and year(a.tanggal)=@tahun and DBTransferdet.GdgTujuan=@Kodegdg');
          end
          else if (DM.QuCari.FieldByName('Tipe').AsString='TRO')  then
          begin
            sql.Add('update DBstockbrg set HRGTRO=@DPP ');
            Sql.add('where kodebrg=@kodebrg and Bulan=@bulan and Tahun=@tahun and Kodegdg=@kodegdg');
            sql.Add('update DBTransferdet set Hpp=@harga');
            sql.add('from DBTransfer a ');
            Sql.add('where a.nobukti=DBTransferdet.nobukti and DBTransferdet.kodebrg=@kodebrg and ');
            sql.add('month(a.tanggal)=@bulan and year(a.tanggal)=@tahun and DBTransferdet.GdgAsal=@Kodegdg');
          end;
        end;
        Prepared;
...
begin
  ...
  begin
    ...
    begin
      Close;
      sql.Clear;
      sql.Add('Update dbStockBrg set HRGPBL=0,HRGRPB=0,HRGPNJ=0,HRGRPJ=0,HRGADI=0,HRGADO=0,');
