...
type
  TFrSubArusKas_ = class(TForm)
    ...
    procedure KodeSubGrpExit(Sender: TObject);
  private
...
procedure TFrSubArusKas_.TampilData;
begin
  QuBeli.Close;
  QuBeli.SQL.Strings[4]:='where KodeAK='+QuotedStr(SubGroup_KodeGrp);
  QuBeli.Open;
end;

procedure TFrSubArusKas_.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrSubArusKas_.ClearPanelDetail;
begin
  KodeSubGrp.Text:='';
  Nama.Text:='';
end;

procedure TFrSubArusKas_.UpdateDataBeli(pChoice: Char);
begin
   ...
   if pChoice='I' then
   begin
     DM.QuCari.SQL.Add('insert into dbArusKasDet (KodeAK, KodeSubAK, NamaSubAK)');
     DM.QuCari.SQL.Add('values('+QuotedStr(SubGroup_KodeGrp)+','+QuotedStr(KodeSubGrp.Text)+','+QuotedStr(Nama.Text)+')');
   end else
   if pChoice='U' then
   begin
     DM.QuCari.SQL.Add('update dbArusKasDet set NamaSubAK='+QuotedStr(Nama.Text));
     DM.QuCari.SQL.Add('where KodeAK='+QuotedStr(SubGroup_KodeGrp)+' and KodeSubAK='+QuotedStr(QuBeliKodeSubGrp.AsString));
   end else
   if pChoice='D' then
   begin
     DM.QuCari.SQL.Add('delete dbArusKasDet ');
     DM.QuCari.SQL.Add('where KodeAK='+QuotedStr(SubGroup_KodeGrp)+' and KodeSubAK='+QuotedStr(QuBeliKodeSubGrp.AsString));
   end;
   ...
   try
     ...
     begin
       TampilData;
       QuBeli.Locate('KodeSubAK', KodeSubGrp.Text, [LOP,LOC]);
     end
...
function TFrSubArusKas_.CekDataSudahAda: Boolean;
begin
  with DM.QuCari do
  begin
    Close;
...
procedure TFrSubArusKas_.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=13 then
...
procedure TFrSubArusKas_.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=cafree;
end;

procedure TFrSubArusKas_.BitBtn2Click(Sender: TObject);
begin
  Close;
end;

procedure TFrSubArusKas_.FormShow(Sender: TObject);
begin
  TampilData;
end;

procedure TFrSubArusKas_.TambahBtnClick(Sender: TObject);
begin
  Model:='write';
...
procedure TFrSubArusKas_.KodeSubGrpKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrSubArusKas_.KoreksiBtnClick(Sender: TObject);
begin
   mValid:=False;
   Model:='koreksi';
...
procedure TFrSubArusKas_.HapusBtnClick(Sender: TObject);
begin
   ...
   begin
      ...
      [QuBeli.FieldByName('KodeSubAK').AsString, QuBeli.FieldByname('NamaSubAK').AsString]));
...
procedure TFrSubArusKas_.BitBtn1Click(Sender: TObject);
...
procedure TFrSubArusKas_.SpeedButton1Click(Sender: TObject);
begin
  mValid:=False;
...
procedure TFrSubArusKas_.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrSubArusKas_.PerkPKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrSubArusKas_.PerkHKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=vk_Return then
  begin
...
procedure TFrSubArusKas_.PerkPPNKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key=vk_Return then
  begin
...
procedure TFrSubArusKas_.JnsTambahBtnClick(Sender: TObject);
begin
...
procedure TFrSubArusKas_.PerkBiayaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key=vk_Return then
  begin
...
procedure TFrSubArusKas_.KodeSubGrpEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrSubArusKas_.KodeSubGrpExit(Sender: TObject);
begin
    if mValid then
    begin
      if length(Kodesubgrp.Text)<>0 then
      begin
        if MyFindField('dbArusKasdet','KodeSubAK',KodesubGrp.Text)=true then
        begin
          ShowMessage('Data Sudah Ada !');
          ActiveControl:=KodesubGrp;
        end;
