...
procedure TFrCustSales.ClearKomponentarget;
var i:integer;
begin
   for i:=FrCustSales.Panel6.ControlCount-1 downto 0 do
   begin
      if (FrCustSales.Panel6.Controls[i] is TPBNumEdit) then
         (FrCustSales.Panel6.Controls[i] as TPBNumEdit).Value:=0;
   end;
end;

procedure TFrCustSales.FormClose(Sender: TObject;
...
procedure TFrCustSales.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=13 then
...
procedure TFrCustSales.BitBtn1Click(Sender: TObject);
begin
...
procedure TFrCustSales.BitBtn2Click(Sender: TObject);
begin
  mValid:=False;
  PanelShow1.Enabled:=True;
  Panel2.Enabled:=True;
...
procedure TFrCustSales.KodeSalesEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrCustSales.FormShow(Sender: TObject);
begin
    //if IsSimpan= then
   tampildet;
...
procedure TFrCustSales.TambahBtnClick(Sender: TObject);
begin
  panel1.Visible:=true;
  ActiveControl:=KodeCustSupp;
  xsimpan:='I';
...
procedure TFrCustSales.SimpanData(Mode:Char);
begin
   if Mode='I' then
      begin
        with Dm.Qucari2 do
...
tampildet;
 ...
 procedure TFrCustSales.SimpanDataTaRget(Mode:Char);
begin
   if Mode='I' then
      begin
        with Dm.Qucari2 do
     ...
     Begin
      with Dm.Qucari2 do
        begin
          ...
          Parameters[12].Value:=FrCustSales.Quview2KeyNik.AsInteger;
          Parameters[13].Value:=FrCustSales.Quview2Tahun.AsInteger;
          ExecSQL;
        end;
     End
   else if Mode='D' then
     begin
      with Dm.Qucari2 do
        begin
          ...
          Parameters[0].Value:=FrCustSales.QuViewKeynik.AsInteger;
          Parameters[1].Value:=FrCustSales.Quview2Tahun.AsString;
          ExecSQL;
        end;
     end;
tampildet;
...
procedure TFrCustSales.KoreksiBtnClick(Sender: TObject);
begin
    xsimpan:='U';
    Panel1.Visible:=True;
...
procedure TFrCustSales.KodeCustSuppExit(Sender: TObject);
begin
  if mValid=True then
  begin
...
procedure TFrCustSales.TampilIsiCustSupp;
begin
  KodeBrows:=1154;
  ...
  begin
    ...
    begin
      KodeCustSupp.Text:=fieldbyname('KodeCust').AsString;
      lnamaCustSupp.Caption:='[ '+fieldbyname('NamaCust').AsString+' ]';
...
procedure TFrCustSales.KodeCustSuppEnter(Sender: TObject);
begin
mValid:=True;
end;
procedure TfrCustSales.tampildet;
begin
   QuView.Close;
   Quview.SQL.Strings[2]:='select @Keynik=:0';
...
procedure TFrCustSales.HapusBtnClick(Sender: TObject);
begin
...
procedure TFrCustSales.SpeedButton4Click(Sender: TObject);
begin
  panel6.Visible:=true;
  tahun.Enabled:=True;
  ActiveControl:=tahun;
...
procedure TFrCustSales.SpeedButton3Click(Sender: TObject);
begin
    XSimpantarget:='U';
    Panel6.Visible:=True;
...
procedure TFrCustSales.SpeedButton2Click(Sender: TObject);
begin
 if QuView.IsEmpty then
      ShowMessage('data kosong')
   else
...
procedure TFrCustSales.BitBtn4Click(Sender: TObject);
begin
...
procedure TFrCustSales.BitBtn3Click(Sender: TObject);
begin
  mValid:=False;
  Panel4.Enabled:=True;
  Panel5.Enabled:=True;
...
procedure TFrCustSales.TahunKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrCustSales.KodeCustSuppKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=VK_Escape then
   begin
