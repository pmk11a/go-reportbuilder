...
implementation
uses myModul,Myprocedure,MyGlobal, FrmSubPemakai,FrmMenu,FrmSubKota,
  FrmSetForm ;
{$R *.dfm}

procedure TFrKota.AmbilData;
begin
   with FrSubKota do
   begin
...
procedure TFrKota.SimpanData(Mode:Char);
begin
  With Sp_Merk do
  begin
    ...
    begin
      Parameters[2].Value := FrSubKota.KodeKota.Text;
      Parameters[3].Value := FrSubKota.NamaKota.Text;
      Parameters[4].Value := FrSubKota.KodeArea.Text;
    end;
    try
      execproc;
      if Mode='I' then
      begin
        QuView.Requery;
        QuView.Locate('KodeKota',FrSubKota.KodeKota.Text,[LOP,LOC]);
        LoggingData(IDUser,Mode,'KOT','','Kode Kota = '+FrSubKota.KodeKota.Text+
                    ' , Nama Kota = '+FrSubKota.NamaKota.Text);
...
procedure TFrKota.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrKota.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrKota.FormDestroy(Sender: TObject);
begin
  FrKota:=nil;
end;

procedure TFrKota.FormShow(Sender: TObject);
begin
   with QuView do
   begin
      close;
...
procedure TFrKota.ToolButton10Click(Sender: TObject);
begin
   with QuView do
   begin
      close;
...
procedure TFrKota.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrKota.ExportExcel1Click(Sender: TObject);
begin
 if (IsExcel) then
  begin
...
procedure TFrKota.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrKota.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrKota.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrKota.Show1Click(Sender: TObject);
begin
...
procedure TFrKota.ToolButton1Click(Sender: TObject);
begin
  if IsTambah then
  begin
    Application.CreateForm(TFrSubKota, FrSubKota);
    FrSubKota.IsSimpan:=True;
    FrSubKota.ShowModal;
  end else
  begin
...
procedure TFrKota.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrKota.ToolButton2Click(Sender: TObject);
begin
  if IsKoreksi then
  begin
    Application.CreateForm(TFrSubKota, FrSubKota);
    FrSubKota.IsSimpan:=false;
    AmbilData;
    FrSubKota.ShowModal;
    if FrSubKota.ModalResult=mrOK then
    begin
    end;
...
procedure TFrKota.ToolButton3Click(Sender: TObject);
begin
  if IsHapus then
  begin
...
procedure TFrKota.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrKota.ToolButton4Click(Sender: TObject);
begin
  if (IsCetak) then
  begin
...
procedure TFrKota.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrKota.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrKota.dxDBGrid1CustomDrawFooter(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
procedure TFrKota.FormCreate(Sender: TObject);
begin
  //Panel1.Color:=FrSetForm.PanelMain.Color;
  //ToolBar1.Color:=FrSetForm.ToolBarMain.Color;
end;

procedure TFrKota.Edit1Change(Sender: TObject);
begin
 with QuView do
   begin
      close;
