...
type
  ...
  TFrPPn = class(TForm)
    ...
    QuViewPersen: TFloatField;
    QuViewPilihan: TBooleanField;
    dxDBGrid1Urut: TdxDBGridMaskColumn;
    dxDBGrid1Persen: TdxDBGridMaskColumn;
    dxDBGrid1Pilihan: TdxDBGridCheckColumn;
...
implementation
uses myModul,Myprocedure,MyGlobal,  FrmSubPPn;
{$R *.dfm}

Procedure TFrPPn.AmbilDataFor;
begin
end;

Procedure TFrPPn.Tampildata;
begin
  with QuView do
  begin
    close;
...
procedure TFrPPn.AmbilData;
begin
   with FrSubPPn do
   begin
     Bulan.Value := QuViewUrut.AsInteger;
     PPn.Value := QuViewPersen.AsInteger;
     Pilihan.Checked   := QuViewPilihan.AsBoolean;
     //Service.Value := QuViewService.AsInteger;
   end;
end;

procedure TFrPPn.SimpanData(Mode:Char);
begin
   With Sp_Barang, FrSubPPn do
   begin
...
procedure TFrPPn.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrPPn.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrPPn.FormDestroy(Sender: TObject);
begin
 FrPPn:=nil;
end;

procedure TFrPPn.FormShow(Sender: TObject);
begin
  Tampildata;
  activeControl:=dxDBGrid1;
end;

procedure TFrPPn.ToolButton10Click(Sender: TObject);
begin
  Tampildata;
end;

procedure TFrPPn.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrPPn.ExportExcel1Click(Sender: TObject);
begin
  if (IsExcel) then
  begin
      Save('xls', 'Microsoft Excel (*.xls)|*.xls', FrPpn.Caption, dxDBGrid1.SaveToXLS);
...
procedure TFrPPn.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrPPn.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrPPn.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrPPn.Show1Click(Sender: TObject);
begin
...
procedure TFrPPn.ToolButton1Click(Sender: TObject);
begin
  if IsTambah then
  begin
    Application.CreateForm(TFrSubPPn, FrSubPPn);
    FrSubPPn.IsSimpan:=True;
    FrSubPPn.ShowModal;
    end
  else
  begin
...
procedure TFrPPn.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPPn.ToolButton2Click(Sender: TObject);
begin
  if IsKoreksi then
  begin
    Application.CreateForm(TFrSubPPn,FrSubPPn);
    AmbilData;
    frSubPPn.Bulan.Enabled := False;
    //frSubPPn.Tahun.Enabled := False;
    FrSubPPn.ShowModal;
  end
  else
  begin
...
procedure TFrPPn.ToolButton3Click(Sender: TObject);
begin
  if IsHapus then
  begin
...
procedure TFrPPn.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrPPn.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrPPn.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrPPn.dxDBGrid1CustomDrawFooter(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
