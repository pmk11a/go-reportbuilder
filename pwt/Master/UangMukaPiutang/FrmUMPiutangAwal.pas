...
type
  TFrUMPiutangAwal = class(TForm)
    ...
    procedure JumlahDChange(Sender: TObject);
...
var
  FrUMPiutangAwal: TFrUMPiutangAwal;
...
implementation
uses MyProcedure,MyGlobal,MyModul, FrmUMPiutang;
{$R *.DFM}

procedure TFrUMPiutangAwal.RefreshAll;
var i:integer;
begin
...
Function TFrUMPiutangAwal.IsNoFakturSudahAda(nobukti:string):boolean;
begin
   with DM.QuCari do
   begin
       Close;
       SQL.Clear;
       SQL.Add('Select * from dbhutpiut where nofaktur=:0 and Perkiraan=:1 and Kodecustsupp=:2 and Tipe=''UPT'' ');
       Prepared;
       Parameters[0].Value:=nobukti;
       Parameters[1].Value:=FrUMPiutang.QuView.FieldByname('Perkiraan').AsString;
       Parameters[2].Value:=FrUMPiutang.QuView.FieldByname('kodecustSupp').AsString;
       Open;
   end;
...
procedure TFrUMPiutangAwal.ClearDetailPanel;
var i:integer;
begin
   for i:= 0 to Panel3.ControlCount-1 do
   begin
...
procedure TFrUMPiutangAwal.FormShow(Sender: TObject);
begin
...
procedure TFrUMPiutangAwal.QuTransaksiCalcFields(DataSet: TDataSet);
begin
...
procedure TFrUMPiutangAwal.BitBtn1Click(Sender: TObject);
begin
  mNoBukti:=NoBukti.Text;
  if model='write' then
  begin
     frUMPiutang.SimpanData('I');
     ClearDetailPanel;
     ActiveControl:=NoBukti;
  end
  else if model='koreksi' then
  begin
     frUMPiutang.SimpanData('U');
     SpeedButton1.Click;
  end;

end;

procedure TFrUMPiutangAwal.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrUMPiutangAwal.NoBuktiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrUMPiutangAwal.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
Action:=cafree;
end;

procedure TFrUMPiutangAwal.NoBuktiEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrUMPiutangAwal.NoBuktiExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrUMPiutangAwal.FormDestroy(Sender: TObject);
begin
 FrUMPiutangAwal:=nil;
end;

procedure TFrUMPiutangAwal.ValasEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrUMPiutangAwal.ValasExit(Sender: TObject);
var i : Integer;
begin
  if mValid Then
  begin
    ...
    begin
      ...
      TampilIsiData(FrUMPiutangAwal,Valas,Label1,1006,'kodevls','Namavls');
    for I := LOW(xValue) to High(xValue) do
    begin
...
procedure TFrUMPiutangAwal.JumlahDChange(Sender: TObject);
begin
  if mValid then
  begin
    JumlahRp.Value:=JumlahD.Value*Kurs.Value;
  end;
end;

procedure TFrUMPiutangAwal.SpeedButton1Click(Sender: TObject);
begin
  ClearDetailPanel;
  Close;
end;

procedure TFrUMPiutangAwal.KreditEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrUMPiutangAwal.KreditChange(Sender: TObject);
begin
  if mValid then
  begin
