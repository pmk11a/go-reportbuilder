...
type
  TFrSetSaldoAktivaFk = class(TForm)
...
var
  FrSetSaldoAktivaFk: TFrSetSaldoAktivaFk;
implementation
Uses MyProcedure,MyGlobal,MyModul, FrmAktivaFK;
...
var i:integer;
begin
   for i:= 0 to FrSetSaldoAktivaFk.ControlCount-1 do
   begin
      if (FrSetSaldoAktivaFk.Controls[i] is TEdit) then
         (FrSetSaldoAktivaFk.Controls[i] as TEdit).Clear;
   end;
end;

procedure TFrSetSaldoAktivaFk.AmbilData;
begin
...
procedure TFrSetSaldoAktivaFk.View(Perkiraan:String);
begin
   with QuSetStock do
   begin
      Close;
      ...
      SQL.Add('from dbaktivafk b');
      SQL.Add('     left outer join dbAktivadetfk a on b.perkiraan=a.perkiraan');
      ...
      Parameters[3].Value:=FrAktivaFk.QuView.FieldByname('Devisi').AsString;
      Open;
      if not IsEmpty then
      begin
...
procedure TFrSetSaldoAktivaFk.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TFrSetSaldoAktivaFk.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=13 then
...
procedure TFrSetSaldoAktivaFk.wwDBGrid1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=13 then SpeedButton1.Click;
end;

procedure TFrSetSaldoAktivaFk.FormShow(Sender: TObject);
begin
   View(Perkiraan);
end;

procedure TFrSetSaldoAktivaFk.FormCreate(Sender: TObject);
begin
   Perkiraan:=frAktivaFk.Quview.fieldbyname('kodeaktiva').AsString;
end;

procedure TFrSetSaldoAktivaFk.ValasKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var i : Integer;
begin
  ...
  begin
    ...
    begin
      ...
      TampilIsiData(FrSetSaldoaktivaFk,Valas,Label4,1006,'kodevls','Namavls');
    for I := LOW(xValue) to High(xValue) do
    begin
...
procedure TFrSetSaldoAktivaFk.BitBtn1Click(Sender: TObject);
begin
    Sp_SetQnt.Close;
    Sp_SetQnt.ProcedureName :='SP_AktivaDetFK;1';
    with Sp_SetQnt do
    begin
      close;
      Prepared;
      ...
      Parameters[2].Value := FrAktivaFk.QuView.FieldByname('Devisi').AsString;
...
procedure TFrSetSaldoAktivaFk.SpeedButton1Click(Sender: TObject);
begin
  Close;
end;
