...
type
  ...
  TFrHPPProduksi = class(TForm)
    ...
    AmbilBulanLalu1: TMenuItem;
    AmbilDataDariFile1: TMenuItem;
    ...
    procedure AmbilDataDariFile1Click(Sender: TObject);
    procedure AmbilBulanLalu1Click(Sender: TObject);
...
procedure TFrHPPProduksi.AmbilDataDariFile1Click(Sender: TObject);
begin
...
procedure TFrHPPProduksi.AmbilBulanLalu1Click(Sender: TObject);
var xTahunLalu, xBulanLalu, xTahunIni, xBulanIni: Integer;
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    xTahunIni:=StrToInt(PeriodThn);
    xBulanIni:=StrToInt(PeriodBln);
    if xBulanIni=1 then
    begin
      xBulanLalu:=12;
      xTahunLalu:=xTahunIni-1;
    end else
    begin
      xBulanLalu:=xBulanIni-1;
      xTahunLalu:=xTahunIni;
    end;
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('update dbHPPProduksi set HPPBrg=isnull(B.HPPBrg,0)');
      SQL.Add('from dbHPPProduksi A');
      SQL.Add('left outer join ');
      SQL.Add('  (select KodeBrg, HPPBrg from dbHPPProduksi where Tahun='+IntToStr(xTahunLalu)+' and Bulan='+IntToStr(xBulanLalu));
      SQL.Add('  ) B on B.KodeBrg=A.KodeBrg ');
      SQL.Add('where A.Tahun='+IntToStr(xTahunIni)+' and Bulan='+IntToStr(xBulanIni));
      SQL.Add('and B.KodeBrg is not null ');
      ExecSQL;
    end;
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('insert into dbHPPProduksi (Tahun, Bulan, KodeBrg, HPPBrg)');
      SQL.Add('select '+IntToStr(xTahunIni)+', '+IntToStr(xBulanIni)+', A.KodeBrg, isnull(A.HPPBrg,0) ');
      SQL.Add('from dbHPPProduksi A');
      SQL.Add('left outer join dbHPPProduksi B on B.KodeBrg=A.KodeBrg and B.Tahun='+IntToStr(xTahunIni)+' and B.Bulan='+IntToStr(xBulanIni));
      SQL.Add('where A.Tahun='+IntToStr(xTahunLalu)+' and A.Bulan='+IntToStr(xBulanLalu));
      SQL.Add('and B.KodeBrg is null ');
      ExecSQL;
    end;
