...
implementation
uses MyProcedure,MyGlobal,MyModul, FrmPiutang;
{$R *.DFM}

procedure TFrPiutangAwal.RefreshAll;
var i:integer;
begin
...
Function TFrPiutangAwal.IsNoFakturSudahAda(nobukti:string):boolean;
begin
   with DM.QuCari do
   begin
       Close;
       SQL.Clear;
       SQL.Add('Select * from dbhutpiut where nofaktur=:0 and Perkiraan=:1 and Kodecustsupp=:2 and Tipe=''PT'' ');
       Prepared;
       Parameters[0].Value:=nobukti;
       Parameters[1].Value:=FrPiutang.QuView.FieldByname('Perkiraan').AsString;
       Parameters[2].Value:=FrPiutang.QuView.FieldByname('kodecustSupp').AsString;
       Open;
   end;
...
procedure TFrPiutangAwal.ClearDetailPanel;
var i:integer;
begin
   for i:= 0 to Panel3.ControlCount-1 do
   begin
...
procedure TFrPiutangAwal.FormShow(Sender: TObject);
begin
//  TglBukti.Date:=Now;
...
procedure TFrPiutangAwal.QuTransaksiCalcFields(DataSet: TDataSet);
begin
...
procedure TFrPiutangAwal.BitBtn1Click(Sender: TObject);
begin
  mNoBukti:=NoBukti.Text;
  if model='write' then
  begin
     frPiutang.SimpanData('I');
     ClearDetailPanel;
     ActiveControl:=NoBukti;
  end
  else if model='koreksi' then
  begin
     frPiutang.SimpanData('U');
     SpeedButton1.Click;
  end;

end;

procedure TFrPiutangAwal.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPiutangAwal.NoBuktiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPiutangAwal.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
Action:=cafree;
end;

procedure TFrPiutangAwal.NoBuktiEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrPiutangAwal.NoBuktiExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrPiutangAwal.FormDestroy(Sender: TObject);
begin
 FrPiutangAwal:=nil;
end;

procedure TFrPiutangAwal.ValasEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrPiutangAwal.ValasExit(Sender: TObject);
var i : Integer;
begin
  if mValid Then
  begin
    ...
    begin
      ...
      TampilIsiData(FrPiutangAwal,Valas,Label1,1006,'kodevls','Namavls');
    for I := LOW(xValue) to High(xValue) do
    begin
   ...
   VlsIDR(Valas,Kurs);
  if Kurs.Enabled=false then ActiveControl:=jumlahD else ActiveControl:=Kurs;
end;

procedure TFrPiutangAwal.JumlahDChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrPiutangAwal.SpeedButton1Click(Sender: TObject);
begin
  ClearDetailPanel;
  Close;
end;

procedure TFrPiutangAwal.KreditEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrPiutangAwal.KreditChange(Sender: TObject);
begin
  if mValid then
  begin
