...
var
  FrUMHutangAwal: TFrUMHutangAwal;
...
implementation
uses MyProcedure,MyGlobal,MyModul, FrmUMHutang;
{$R *.DFM}

procedure TFrUMHutangAwal.RefreshAll;
var i:integer;
begin
...
Function TFrUMHutangAwal.IsNoFakturSudahAda(nobukti:string):boolean;
begin
   with DM.QuCari do
   begin
       Close;
       SQL.Clear;
       SQL.Add('Select * from dbhutpiut where nofaktur=:0 and Perkiraan=:1 and kodecustsupp=:2 and Tipe=''UHT'' ');
       Prepared;
       Parameters[0].Value:=nobukti;
       Parameters[1].Value:=FrUMHutang.QuView.FieldByname('Perkiraan').AsString;
       Parameters[2].Value:=FrUMHutang.QuView.FieldByname('kodecustSupp').AsString;
       Open;
   end;
...
procedure TFrUMHutangAwal.ClearDetailPanel;
var i:integer;
begin
   for i:= 0 to Panel3.ControlCount-1 do
   begin
...
procedure TFrUMHutangAwal.FormShow(Sender: TObject);
begin
...
procedure TFrUMHutangAwal.QuTransaksiCalcFields(DataSet: TDataSet);
begin
...
procedure TFrUMHutangAwal.BitBtn1Click(Sender: TObject);
begin
  mNoBukti:=NoBukti.Text;
  if model='write' then
  begin
     frUMHutang.SimpanData('I');
     ClearDetailPanel;
     ActiveControl:=NoBukti;
  end
  else if model='koreksi' then
  begin
     frUMHutang.SimpanData('U');
     SpeedButton1.Click;
  end;

end;

procedure TFrUMHutangAwal.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrUMHutangAwal.NoBuktiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrUMHutangAwal.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
Action:=cafree;
end;

procedure TFrUMHutangAwal.NoBuktiEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrUMHutangAwal.NoBuktiExit(Sender: TObject);
begin
if mValid then
begin
...
procedure TFrUMHutangAwal.FormDestroy(Sender: TObject);
begin
 FrUMHutangAwal:=nil;
end;

procedure TFrUMHutangAwal.ValasEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrUMHutangAwal.ValasExit(Sender: TObject);
var i : Integer;
begin
  if mValid Then
  begin
    ...
    begin
      ...
      TampilIsiData(FrUMHutangAwal,Valas,Label1,1006,'kodevls','Namavls');
    for I := LOW(xValue) to High(xValue) do
    begin
...
procedure TFrUMHutangAwal.DebetChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrUMHutangAwal.SpeedButton1Click(Sender: TObject);
begin
  ClearDetailPanel;
  Close;
end;

procedure TFrUMHutangAwal.KreditChange(Sender: TObject);
begin
   if mValid then
  begin
...
procedure TFrUMHutangAwal.KreditEnter(Sender: TObject);
begin
  mValid := True;
end;
