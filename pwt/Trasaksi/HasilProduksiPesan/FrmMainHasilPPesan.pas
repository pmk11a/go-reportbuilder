...
type
  ...
  TFrMainHasilPPesan = class(TForm)
    ToolBar1: TToolBar;
  ...
  private
    ...
    procedure UpdatePesanan(NoBukti : string; IsSiap : Boolean);
  public
    { Public declarations }
...
var
  FrMainHasilPPesan: TFrMainHasilPPesan;
...
procedure TFrMainHasilPPesan.GetData(bulan,tahun:integer);
begin
  QuNavigator.Close;
...
procedure TFrMainHasilPPesan.UpdatePesanan(NoBukti : string; IsSiap : Boolean);
begin
  if IsSiap then
    begin
      with DM.QuCari do
        begin
          Close;
          SQL.Clear;
          SQL.Add('update DBPesanTunaiDet set IsSelesai = 1 where NoSPK in ');
          SQL.Add('(select NoSPK from DBHASILPRDDET where NOBUKTI = :0) ');
          Parameters[0].Value := NoBukti;
          try
            ExecSQL;
          except
            ShowMessage('Proses Update Pesanan Gagal !');
          end;
        end;
    end
  else
    begin
      with DM.QuCari do
        begin
          Close;
          SQL.Clear;
          SQL.Add('update DBPesanTunaiDet set IsSelesai = 0 where NoSPK in ');
          SQL.Add('(select NoSPK from DBHASILPRDDET where NOBUKTI = :0) ');
          Parameters[0].Value := NoBukti;
          try
            ExecSQL;
          except
            ShowMessage('Proses Batal Pesanan Gagal !');
          end;
        end;
    end;
end;

procedure TFrMainHasilPPesan.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainHasilPPesan.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrHasilPPesan')<>nil then
    begin
       MessageDlg('Modul '+FrHasilPPesan.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainHasilPPesan.FormDestroy(Sender: TObject);
begin
  FrMainHasilPPesan:=nil;
end;

procedure TFrMainHasilPPesan.FormShow(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainHasilPPesan.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainHasilPPesan.ExportExcel1Click(Sender: TObject);
begin
  with QuView do
  begin
    Close;
...
procedure TFrMainHasilPPesan.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.Show1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrHasilPPesan) then FrHasilPPesan.Show else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrHasilPPesan,FrHasilPPesan);
        CekOtoritasMenu(IDUser,tag,FrHasilPPesan.Istambah,FrHasilPPesan.Ishapus,
          FrHasilPPesan.Iskoreksi, FrHasilPPesan.isCetak, FrHasilPPesan.isExcel);
...
procedure TFrMainHasilPPesan.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainHasilPPesan.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
    begin
      ...
      if Not QuMasterNeedOtorisasi.Value then
       ...
       Begin
        if Assigned(FrHasilPPesan) then
          FrHasilPPesan.Show
        else
        begin
          ModalKoreksi:=True;
...
procedure TFrMainHasilPPesan.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainHasilPPesan.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainHasilPPesan.FormCreate(Sender: TObject);
begin
  Panel1.Color:=FrSetForm.PanelMain.Color;
  ToolBar1.Color:=FrSetForm.ToolBarMain.Color;
end;

procedure TFrMainHasilPPesan.tvMasterFocusedRecordChanged(
...
procedure TFrMainHasilPPesan.ToolButton4Click(Sender: TObject);
begin
if Iscetak then
...
procedure TFrMainHasilPPesan.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DBHasilPrd',QuMasterNoBukti.AsString);
end;

procedure TFrMainHasilPPesan.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainHasilPPesan.ToolButton12Click(Sender: TObject);
begin
 {if QuMasterIsOtorisasi1.Value then
  ...
  begin
    ...
    begin
      ...
      UpdatePesanan(QuMasterNOBUKTI.Value,True);
      Begin
        ToolButton10.Click;
...
procedure TFrMainHasilPPesan.ToolButton13Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  ...
  begin
    ...
    UpdatePesanan(QuMasterNOBUKTI.Value,False);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
