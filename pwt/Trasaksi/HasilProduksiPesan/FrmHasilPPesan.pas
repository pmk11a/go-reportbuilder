...
implementation
uses FrmBrows,MyProcedure, MyModul, FrmMainBeli, FrmMainHasilPPesan;
{$R *.DFM}

// PROCEDURE DAN FUNCTION PRIBADI

procedure TFrHasilPPesan.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrHasilPPesan.TampilIsiSupplier;
begin

end;

procedure TFrHasilPPesan.TampilIsiGudang;
begin
  KodeBrows:=11002;
...
procedure TFrHasilPPesan.IsiSatuan;
begin
  LNamaSat.Caption:=IsiSatuanBrg(KodeBrg.Text, mNamaSatuan, mIsiSatuan);
end;

procedure TFrHasilPPesan.TampilIsiBarang;
begin
//  KodeBrows:=23101;
  KodeBrows:=24201;
  Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrHasilPPesan.TampilIsiBarangPO;
begin

end;

procedure TFrHasilPPesan.TampilIsiEkspedisi;
begin

end;

procedure TFrHasilPPesan.IsiNoBuktiBaru;
begin
...
procedure TFrHasilPPesan.TampilIsiNoSPK;
begin
  KodeBrows:=101411;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.IsiData:='1';
...
procedure TFrHasilPPesan.TampilData(No_Bukti:String);
begin
   QuBeli.Close;
...
Function TFrHasilPPesan.CheckFormMaster:boolean;
begin
   CheckFormMaster:=True;
end;

procedure TFrHasilPPesan.UpdatedbBeli;
begin
...
procedure TFrHasilPPesan.ClearPanelDetail;
begin
   KodeBrg.Text:='';
   NoSPK.Text:='';
   NoSat.Value:=1;
...
procedure TFrHasilPPesan.RefreshAll;
var i:integer;
begin
   mValid:=False;
   mUbahHd:=False; mUbahHdDet:=False;
...
procedure TFrHasilPPesan.IsiHargaBrg;
begin
end;

procedure TFrHasilPPesan.TampilanQnt1Qnt2;
begin

end;

procedure TFrHasilPPesan.UpdateDataBeli(Choice:Char;Kodet:String);
begin
...
Procedure TFrHasilPPesan.AmBilDataBeli;
begin
...
procedure TFrHasilPPesan.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if mExit=true then
  begin
...
procedure TFrHasilPPesan.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=13 then SendMessage(handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrHasilPPesan.FormShow(Sender: TObject);
begin
  mValid:=False;
  xModalKoreksi:=FrMainHasilPPesan.ModalKoreksi;
  IsTampil:=False;
  ...
  Begin
    NOBUKTI.Text:=FrMainHasilPPesan.QuMasterNoBukti.AsString;
...
procedure TFrHasilPPesan.FormCreate(Sender: TObject);
begin
  mValid:=False;
  TipeTrans:='HPD';
...
procedure TFrHasilPPesan.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrHasilPPesan.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrHasilPPesan.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrHasilPPesan.KoreksiBtnClick(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    mValid:=False;
...
procedure TFrHasilPPesan.wwDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
  ...
  begin
   if FrMainHasilPPesan.ModalKoreksi Then
    KoreksiBtn.Click
   else
    TambahBtn.Click;
  end
...
procedure TFrHasilPPesan.HapusBtnClick(Sender: TObject);
begin
...
procedure TFrHasilPPesan.wwDBGrid1Enter(Sender: TObject);
begin
  FrHasilPPesan.KeyPreview:=false;
  CheckFormMaster;
end;

procedure TFrHasilPPesan.wwDBGrid1Exit(Sender: TObject);
begin
  FrHasilPPesan.KeyPreview:=true;
end;

procedure TFrHasilPPesan.wwDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrHasilPPesan.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrHasilPPesan.TanggalEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrHasilPPesan.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrHasilPPesan.KodeSuppEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrHasilPPesan.KodeSuppExit(Sender: TObject);
begin
  //
end;

procedure TFrHasilPPesan.CetakClick(Sender: TObject);
var N1, N2, N3 : string;
    T1, T2 : Real;
    Tot : byte;
    H1  : real;
...
procedure TFrHasilPPesan.BitBtn1Click(Sender: TObject);
begin
  mValid:=False;
  //if CekPeriode(IDUser, Tanggal.Date)=True then
  //begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ActiveControl:=NoSPK;
          // SpeedButton1.Click;
          FrMainHasilPPesan.ToolButton10.Click;
        end
        else
        begin
...
procedure TFrHasilPPesan.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
  ClearPanelDetail;
...
procedure TFrHasilPPesan.FormDestroy(Sender: TObject);
begin
  FrHasilPPesan:=nil;
end;

procedure TFrHasilPPesan.KodeGdgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrHasilPPesan.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrHasilPPesan.DsgCetakBtnClick(Sender: TObject);
begin
{  With QuUpdatedbbeli do
  begin
    close;
...
procedure TFrHasilPPesan.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrHasilPPesan.NoUrutChange(Sender: TObject);
begin
  if mValid then
  begin
    if Length(NoUrut.Text)=4 then
...
procedure TFrHasilPPesan.KodeBrgExit(Sender: TObject);
begin
  if mValid then
  begin
    TampilIsiBarangPO;
...
procedure TFrHasilPPesan.KodeBrgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrHasilPPesan.NoPOEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrHasilPPesan.NoPOExit(Sender: TObject);
begin
  {if mValid Then
  begin
    if NoPO.Text<>'-' then
...
procedure TFrHasilPPesan.TanggalChange(Sender: TObject);
begin
  mUbahHd:=True;
end;

procedure TFrHasilPPesan.NoSatChange(Sender: TObject);
begin
 { if mValid then
  begin
...
procedure TFrHasilPPesan.NoSPKExit(Sender: TObject);
begin
 if mValid Then
  begin
    if NoSPK.Text<>'-' then
    begin
      ...
      begin
        if DataBersyarat('Select A.NoBukti,A.KodeBrg BrgJ,A.Qnt QntJ,A.Nosat NosatJ,A.Isi IsiJ,A.Satuan SatJ,E.NamaBrg NamaBrgJ '+
                         'From dbSPK A '+
                         'Left Outer join dbBarang E on E.KodeBrg=A.Kodebrg '+
                         'where NoBukti Not In(Select NoSPK from dbHASILPRDDet) and NoBukti=:0 and A.IsPesan = 1',[NoSPK.text],Dm.Qucari)=true then
        begin
           with DM.QuCari do
            Begin
             NoSPK.Text:=fieldbyname('NoBukti').AsString;
...
procedure TFrHasilPPesan.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DBHasilPrd',Nobukti.Text);
end;

procedure TFrHasilPPesan.NoUrutExit(Sender: TObject);
