...
type
  ...
  TFrMainRBeliGdg = class(TForm)
    ...
    QuMasterKETERANGAN: TWideStringField;
    QuMasterFAKTURSUPP: TStringField;
    QuMasterKODEVLS: TStringField;
    QuMasterKURS: TBCDField;
    QuMasterPPN: TWordField;
    QuMasterTIPEBAYAR: TWordField;
    QuMasterHARI: TIntegerField;
    QuMasterTipeDisc: TWordField;
    QuMasterDISC: TFloatField;
    QuMasterDISCRP: TBCDField;
    QuMasterNILAIPOT: TBCDField;
    QuMasterNILAIDPP: TBCDField;
    QuMasterNILAIPPN: TBCDField;
    QuMasterNILAINET: TBCDField;
...
var
  FrMainRBeliGdg: TFrMainRBeliGdg;
  S: array[0..255] of char;
implementation
uses myModul, Myprocedure, MyGlobal, FrmBrows, FrmRBeliGdg, FrmSetForm;
{$R *.dfm}

procedure TFrMainRBeliGdg.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainRBeliGdg.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainRBeliGdg.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrRBeliGdg')<>nil then
    begin
       MessageDlg('Modul '+FrRBeliGdg.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainRBeliGdg.FormDestroy(Sender: TObject);
begin
  FrMainRBeliGdg:=nil;
end;

procedure TFrMainRBeliGdg.FormShow(Sender: TObject);
begin
  xStatus := 'S';
...
procedure TFrMainRBeliGdg.ToolButton10Click(Sender: TObject);
begin
   GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainRBeliGdg.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainRBeliGdg.ExportExcel1Click(Sender: TObject);
begin
{  with QuView do
  begin
    Close;
...
procedure TFrMainRBeliGdg.ToolButton1Click(Sender: TObject);
begin
   ...
   begin
     ...
     begin
      if IsTambah then
      begin
         if Assigned(FrRBeliGdg) then
            FrRBeliGdg.Show
         else                              
         begin
           ModalKoreksi:=False;
           xStatus := 'H';
           Application.CreateForm(TFrRBeliGdg,FrRBeliGdg);
           CekOtoritasMenu(IDUser,tag,FrRBeliGdg.Istambah,FrRBeliGdg.Ishapus,
                       FrRBeliGdg.Iskoreksi, FrRBeliGdg.isCetak, FrRBeliGdg.isExcel);
           FrRBeliGdg.Show;
         end;
      end
      else
...
procedure TFrMainRBeliGdg.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainRBeliGdg.ToolButton2Click(Sender: TObject);
begin
...
procedure TFrMainRBeliGdg.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainRBeliGdg.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainRBeliGdg.FormCreate(Sender: TObject);
begin
  Panel1.Color:=FrSetForm.PanelMain.Color;
  ToolBar1.Color:=FrSetForm.ToolBarMain.Color;
end;

procedure TFrMainRBeliGdg.tvMasterFocusedRecordChanged(
...
procedure TFrMainRBeliGdg.ToolButton12Click(Sender: TObject);
begin
...
procedure TFrMainRBeliGdg.ToolButton4Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
...
procedure TFrMainRBeliGdg.ToolButton13Click(Sender: TObject);
begin
IF DataBersyarat('select Nobukti from VwmasterRbeli where NeedOtorisasi=1 '+
                 ' and Nobukti=:0',[QuMasterNoBukti.ASstring],DM.Qucari)=true then
begin
    BM := QuMaster.GetBookmark;
    begin
      pembatalan('DBRBeli','IsBatal',QuMasterNoBukti.AsString,IDUser,MenuRBeli);
      Begin
        ToolButton10.Click;
        if QuMaster.BookmarkValid(BM) then
        begin
 ...
 ShowMessage('Nomor Retur pembelian Gudang'+QuMasterNoBukti.AsString+' Sudah Terotorisasi invoice retur pembelian') ;
...
procedure TFrMainRBeliGdg.tvMasterCellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
   if QuMasterIsBatal.AsBoolean=true then
...
procedure TFrMainRBeliGdg.tvMasterCustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
begin
if AViewInfo.GridRecord.DisplayTexts[23]='True' then
