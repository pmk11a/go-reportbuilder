...
var
  FrMainPBLain: TFrMainPBLain;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmPBLain;
{$R *.dfm}

procedure TFrMainPBLain.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainPBLain.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainPBLain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrPBLain')<>nil then
    begin
       MessageDlg('Modul '+FrPBLain.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainPBLain.FormDestroy(Sender: TObject);
begin
  FrMainPBLain:=nil;
end;

procedure TFrMainPBLain.FormShow(Sender: TObject);
begin
...
procedure TFrMainPBLain.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainPBLain.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainPBLain.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainPBLain.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainPBLain.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainPBLain.Show1Click(Sender: TObject);
begin
...
procedure TFrMainPBLain.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrPBLain) then FrPBLain.Show else
      begin
        ModalKoreksi:=False;
        PGIndex:=dxPageControl1.ActivePageIndex;
        Application.CreateForm(TFrPBLain,FrPBLain);
        CekOtoritasMenu(IDUser,tag,FrPBLain.Istambah,FrPBLain.Ishapus,
          FrPBLain.Iskoreksi, FrPBLain.isCetak, FrPBLain.isExcel);
        FrPBLain.Show;
      end;
    end
    else
...
procedure TFrMainPBLain.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainPBLain.ToolButton2Click(Sender: TObject);
begin
...
procedure TFrMainPBLain.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainPBLain.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainPBLain.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainPBLain.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainPBLain.tvMasterFocusedRecordChanged(
...
procedure TFrMainPBLain.dxPageControl1Change(Sender: TObject);
begin
{ToolButton1.Enabled:=(dxPageControl1.ActivePageIndex=0);// and (QuBPPBStok.AsFloat>=QuBPPBSisa.AsFloat);
 ToolButton2.Enabled:=dxPageControl1.ActivePageIndex=1;}
end;

procedure TFrMainPBLain.tvPOCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrMainPBLain.tvPOKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainPBLain.tvPOMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
 //ToolButton1.Enabled:= (QuBPPBStok.AsFloat>=QuBPPBSisa.AsFloat);
end;

procedure TFrMainPBLain.ToolButton4Click(Sender: TObject);
var i : integer;
...
procedure TFrMainPBLain.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainPBLain.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbpenyerahanBhn',QuMasterNoBukti.AsString);
end;

procedure TFrMainPBLain.ToolButton13Click(Sender: TObject);
var i:Integer;
begin
...
procedure TFrMainPBLain.ToolButton12Click(Sender: TObject);
var i:Integer;
begin
...
procedure TFrMainPBLain.CetakPO1Click(Sender: TObject);
var i : integer;
    DaftarNoBukti : String;
begin
  frxDBDataset1.DataSet := DM.QuCari;
...
procedure TFrMainPBLain.CetakPOTanpaHarga1Click(Sender: TObject);
var i : integer;
    DaftarNoBukti : String;
begin
  frxDBDataset1.DataSet := DM.QuCari;
...
procedure TFrMainPBLain.Prepress1Click(Sender: TObject);
var i : integer;
    DaftarNoBukti : String;
begin
  frxDBDataset1.DataSet := DM.QuCari;
