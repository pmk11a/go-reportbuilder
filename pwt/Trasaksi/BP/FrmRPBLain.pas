...
implementation
uses FrmBrows, MyProcedure, MyModul, FrmMainRPBLain;
{$R *.DFM}

// PROCEDURE DAN FUNCTION PRIBADI

procedure TFrRPBLain.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrRPBLain.TampilIsiDept;
begin

end;

procedure TFrRPBLain.TampilIsiBarang;
begin
  KodeBrows:=242017;
...
procedure TFrRPBLain.IsiNoBuktiBaru;
begin
  ...
  //Check_Nomor(StrToInt(PeriodBln),StrToInt(PeriodThn),'RBP',Nomor,NomorBukti,Tanggal.Date);
    Check_NomorRPB(StrToInt(PeriodBln),StrToInt(PeriodThn),TipeTrans,Nomor,NomorBukti,Tanggal.Date,kodegdg.text,1);
  NoUrut.Text := NomorBukti;
  NOBUKTI.Text := Nomor;
end;

procedure TFrRPBLain.TampilData(No_Bukti,ID:String);
begin
...
else if FrMainRPB.ModalKoreksi=False Then
 Begin
   QuBeli.Close;
   QuBeli.SQL.Clear;
...
Function TFrRPBLain.CheckFormMaster:boolean;
begin
   CheckFormMaster:=true;
end;

procedure TFrRPBLain.UpdatedbBeli;
begin
  if (not QuBeli.IsEmpty)and(FrMainRPBLain.ModalKoreksi) then
  begin
    ...
    begin
       ...
       SQL.Add('set TANGGAL=:0,Kodegdg=:1,NoPenyerahanBhn=:2');
...
procedure TFrRPBLain.ClearPanelDetail;
begin
   mValid:=False;
   KodeBrg.Text:='';
   LNamaBrg.Caption:='[ . . . ]';
...
procedure TFrRPBLain.RefreshAll;
begin
  mValid:=False;
  KodeGdg.Text:='';
  NoBppB.Text:='';
...
procedure TFrRPBLain.UpdateDataBeli(Choice:Char;Kodet:String);
begin
   if Choice='I' then
   begin
   ...
   FrMainRPBLain.mMainRPB_NoBukti:=NoBukti.Text;
   With Sp_Beli do
   begin
       Parameters.Refresh;
       ...
       Parameters[15].Value:=1;//flagtipe jenis lain
       Parameters[16].Value:=Ket.text;
       try
          ExecProc;
          if Choice='I' then
          begin
...
Procedure TFrRPBLain.AmBilDataBeli;
begin
...
procedure TFrRPBLain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (KodeGdg.Text <> '-') or (QuBeli.IsEmpty) then
    begin
      ...
      begin
         ...
         FrMainRPBLain.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
         Action:=cafree;
      end
      else
      begin
...
procedure TFrRPBLain.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=13 then SendMessage(handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrRPBLain.FormShow(Sender: TObject);
begin
  mValid:=False;
  xModalKoreksi:=FrMainRPBLain.ModalKoreksi;
  IsTampil:=False;
  IsTampil1:=False;
  If FrMainRPBLain.ModalKoreksi = False then
  begin
    RefreshAll;
    IsiNoBuktiBaru;
  ...
  Begin
    NOBUKTI.Text := FrMainRPBLain.QuMaster.FieldByname('Nobukti').AsString;
...
procedure TFrRPBLain.FormCreate(Sender: TObject);
begin
  TipeTrans :='RPBL';
  Hapus_Daftar_Nomor_User(TipeTrans ,IDUser);
end;

procedure TFrRPBLain.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrRPBLain.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrRPBLain.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrRPBLain.KoreksiBtnClick(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    mValid:=False;
    ...
    begin
      ...
      Ket.text :=qubeli.fieldbyname('keterangan').AsString;
...
procedure TFrRPBLain.wwDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
  ...
  begin
  if FrMainRPBLain.ModalKoreksi Then
    KoreksiBtn.Click
  else
    TambahBtn.Click;
  end
...
procedure TFrRPBLain.HapusBtnClick(Sender: TObject);
begin
      if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
      begin
...
procedure TFrRPBLain.wwDBGrid1Enter(Sender: TObject);
begin
  FrRPBLain.KeyPreview:=false;
...
procedure TFrRPBLain.wwDBGrid1Exit(Sender: TObject);
begin
  FrRPBLain.KeyPreview:=true;
end;

procedure TFrRPBLain.wwDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrRPBLain.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=Vk_Return then
  begin
...
procedure TFrRPBLain.TanggalEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrRPBLain.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrRPBLain.CetakClick(Sender: TObject);
var N1, N2, N3 : string;
    T1, T2 : Real;
    Tot : byte;
    H1  : real;
...
procedure TFrRPBLain.BitBtn1Click(Sender: TObject);
begin
  if Model='write' then
  begin
...
procedure TFrRPBLain.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
...
procedure TFrRPBLain.FormDestroy(Sender: TObject);
begin
  FrRPBLain:=nil;
end;

procedure TFrRPBLain.KodeGdgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPBLain.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrRPBLain.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrRPBLain.SpeedButton8Click(Sender: TObject);
begin
  frxReport1.LoadFromFile(currDir+'Nota\NotareturPemakaianBahan.fr3');
  frxReport1.DesignReport;
end;

procedure TFrRPBLain.TanggalChange(Sender: TObject);
begin
  mUbahHd:=True;
end;

procedure TFrRPBLain.NoUrutEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPBLain.IsiSatuan;
begin
...
procedure TFrRPBLain.KodeBrgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPBLain.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
     if length(KodeBrg.Text)=0 then TampilIsiBarang else
      begin
        if DataBersyarat(
                   'Select A.Nobukti, A.urut, A.kodebrg, d.NAMABRG, A.Qnt,A.Qnt2, A.Isi, A.NoSat, (A.Qnt-isnull(B.Qnt,0)) QntSisa,(A.Qnt2-isnull(B.Qnt2,0)) Qnt2Sisa,'+#13+
                   '       d.NFix'+#13+
                   ...
                   '       and A.nobukti=:0 and A.kodebrg=:1 and A.IsSampel=0'+#13+
                   'Order by A.kodebrg',[NoBPPB.Text, kodebrg.text],Dm.Qucari)=true then
        begin
          with DM.QuCari do
          begin
            KodeBrg.Text:=fieldbyname('KodeBrg').AsString;
...
procedure TFrRPBLain.TampilIsiGudang;
begin
  KodeBrows:=11009;
  Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrRPBLain.KodeGdgExit(Sender: TObject);
begin
 if mValid Then
  begin
...
procedure TFrRPBLain.QntChange(Sender: TObject);
begin
  if mValid then
  begin
    if NoSat.AsInteger=1 then
...
procedure TFrRPBLain.TampilIsiBeli;
begin
  KodeBrows:=242020;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.GrpTipe:=0;
...
procedure TFrRPBLain.TampilanQnt1Qnt2;
begin
  Qnt1.Enabled:=False;
  Qnt2.Enabled:=False;
  if mNFixBarang then
...
procedure TFrRPBLain.NoBppBExit(Sender: TObject);
begin
  if mValid Then
  begin
    if NoBppB.Text<>'-' then
    begin
      ...
      begin
        if DataBersyarat('Select A.Nobukti, C.Tanggal'+#13+
                     ...
                     '                  else 1'+#13+
                     '             end As Bit)=0 and ((A.Qnt-isnull(B.Qnt,0)>0) or (A.Qnt2-isnull(B.Qnt2,0)>0))'+#13+
                     '      and A.nobukti=:0 and c.isSampel=0'+#13+
...
procedure TFrRPBLain.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrRPBLain.NoSatChange(Sender: TObject);
begin
  if mValid then
  begin
    TampilanQnt1Qnt2;
...
procedure TFrRPBLain.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbRpenyerahan',NoBukti.Text);
end;

procedure TFrRPBLain.NoUrutExit(Sender: TObject);
