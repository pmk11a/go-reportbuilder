...
implementation
uses FrmBrows, MyProcedure, MyModul, FrmMainRPB;
{$R *.DFM}

// PROCEDURE DAN FUNCTION PRIBADI

procedure TFrRPB.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrRPB.TampilIsiDept;
begin

end;

procedure TFrRPB.TampilIsiBarang;
begin
  KodeBrows:=242017;
...
procedure TFrRPB.IsiNoBuktiBaru;
begin
  ...
  //Check_Nomor(StrToInt(PeriodBln),StrToInt(PeriodThn),'RBP',Nomor,NomorBukti,Tanggal.Date);
    Check_NomorRPB(StrToInt(PeriodBln),StrToInt(PeriodThn),TipeTrans,Nomor,NomorBukti,Tanggal.Date,kodegdg.text,0);
  NoUrut.Text := NomorBukti;
  NOBUKTI.Text := Nomor;
end;

procedure TFrRPB.TampilData(No_Bukti,ID:String);
begin
...
Function TFrRPB.CheckFormMaster:boolean;
begin
   CheckFormMaster:=true;
end;

procedure TFrRPB.UpdatedbBeli;
begin
  if (not QuBeli.IsEmpty)and(FrMainRPB.ModalKoreksi) then
  begin
...
procedure TFrRPB.ClearPanelDetail;
begin
   mValid:=False;
   KodeBrg.Text:='';
   LNamaBrg.Caption:='[ . . . ]';
...
procedure TFrRPB.UpdateDataBeli(Choice:Char;Kodet:String);
begin
   if Choice='I' then
   begin
   ...
   FrMainRPB.mMainRPB_NoBukti:=NoBukti.Text;
   With Sp_Beli do
   begin
       Parameters.Refresh;
       ...
       Parameters[15].Value:=0;//flagtipe
...
Procedure TFrRPB.AmBilDataBeli;
begin
...
procedure TFrRPB.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (KodeGdg.Text <> '-') or (QuBeli.IsEmpty) then
    begin
      ...
      begin
         ...
         FrMainRPB.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
         Action:=cafree;
      end
      else
      begin
...
procedure TFrRPB.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=13 then SendMessage(handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrRPB.FormShow(Sender: TObject);
begin
  mValid:=False;
  xModalKoreksi:=FrMainRPB.ModalKoreksi;
  IsTampil:=False;
  IsTampil1:=False;
  If FrMainRPB.ModalKoreksi = False then
  begin
    RefreshAll;
    IsiNoBuktiBaru;
  ...
  Begin
    NOBUKTI.Text := FrMainRPB.QuMaster.FieldByname('Nobukti').AsString;
...
procedure TFrRPB.FormCreate(Sender: TObject);
begin
  TipeTrans :='RBP';
  Hapus_Daftar_Nomor_User(TipeTrans ,IDUser);
end;

procedure TFrRPB.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrRPB.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrRPB.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrRPB.KoreksiBtnClick(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    mValid:=False;
...
procedure TFrRPB.wwDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrRPB.HapusBtnClick(Sender: TObject);
begin
      if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
      begin
...
procedure TFrRPB.wwDBGrid1Enter(Sender: TObject);
begin
  FrRPB.KeyPreview:=false;
...
procedure TFrRPB.wwDBGrid1Exit(Sender: TObject);
begin
  FrRPB.KeyPreview:=true;
end;

procedure TFrRPB.wwDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrRPB.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=Vk_Return then
  begin
...
procedure TFrRPB.TanggalEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrRPB.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrRPB.CetakClick(Sender: TObject);
var N1, N2, N3 : string;
    T1, T2 : Real;
    Tot : byte;
    H1  : real;
...
procedure TFrRPB.BitBtn1Click(Sender: TObject);
begin
  if Model='write' then
  begin
...
procedure TFrRPB.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
...
procedure TFrRPB.FormDestroy(Sender: TObject);
begin
  FrRPB:=nil;
end;

procedure TFrRPB.KodeGdgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPB.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrRPB.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrRPB.SpeedButton8Click(Sender: TObject);
begin
  frxReport1.LoadFromFile(currDir+'Nota\NotareturPemakaianBahan.fr3');
  frxReport1.DesignReport;
end;

procedure TFrRPB.TanggalChange(Sender: TObject);
begin
  mUbahHd:=True;
end;

procedure TFrRPB.NoUrutEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPB.IsiSatuan;
begin
...
procedure TFrRPB.KodeBrgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPB.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrRPB.TampilIsiGudang;
begin
  KodeBrows:=11009;
  Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrRPB.KodeGdgExit(Sender: TObject);
begin
 if mValid Then
  begin
...
procedure TFrRPB.QntChange(Sender: TObject);
begin
  if mValid then
  begin
    if NoSat.AsInteger=1 then
...
procedure TFrRPB.TampilIsiBeli;
begin
  KodeBrows:=242016;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.GrpTipe:=0;
...
procedure TFrRPB.TampilanQnt1Qnt2;
begin
  Qnt1.Enabled:=False;
  Qnt2.Enabled:=False;
  if mNFixBarang then
...
procedure TFrRPB.NoBppBExit(Sender: TObject);
begin
  if mValid Then
  begin
    if NoBppB.Text<>'-' then
...
procedure TFrRPB.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrRPB.NoSatChange(Sender: TObject);
begin
  if mValid then
  begin
    TampilanQnt1Qnt2;
...
procedure TFrRPB.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbRpenyerahan',NoBukti.Text);
end;

procedure TFrRPB.NoUrutExit(Sender: TObject);
