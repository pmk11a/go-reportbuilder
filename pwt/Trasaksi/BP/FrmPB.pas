...
implementation
uses FrmBrows, MyProcedure, MyModul, FrmMainPB;
{$R *.DFM}

// PROCEDURE DAN FUNCTION PRIBADI

procedure TFrPB.IsiSPK;
begin
  with DM.QuCari do
    begin
      Close;
...
procedure TFrPB.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrPB.TampilIsiDept;
begin

end;

procedure TFrPB.TampilIsiBarang;
begin
  //KodeBrows:=242013;
  //KodeBrows:=9149 ;
...
procedure TFrPB.TampilIsiBarangNonBJ;
begin
  //KodeBrows:=2420112;
  kodebrows:=242013;
  Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrPB.IsiNoBuktiBaru;
begin
  ...
  Check_NomorPB(StrToInt(PeriodBln),StrToInt(PeriodThn),TipeTrans,Nomor,NomorBukti,Tanggal.Date,kodegdg.text,0);

  NoUrut.Text := NomorBukti;
  NOBUKTI.Text := Nomor;
end;

procedure TFrPB.TampilData(No_Bukti,ID:String);
begin
   QuBeli.Close;
...
Function TFrPB.CheckFormMaster:boolean;
begin
   CheckFormMaster:=true;
end;

procedure TFrPB.UpdatedbBeli;
begin
  if (not QuBeli.IsEmpty)and(FrMainPB.ModalKoreksi) then
  begin
...
procedure TFrPB.ClearPanelDetail;
begin
   mValid:=False;
   KodeBrg.Text:='';
   LNamaBrg.Caption:='[ . . . ]';
...
procedure TFrPB.UpdateDataBeli(Choice:Char;Kodet:String);
var iProses: Integer;
...
begin
   ...
   FrMainPB.mMainPB_NoBukti:=NoBukti.Text;
   With Sp_Beli do
   begin
       Parameters.Refresh;
...
procedure TFrPB.TampilanQnt1Qnt2;
begin
  Qnt1.Enabled:=False;
  Qnt2.Enabled:=False;
  if mNFixBarang then
...
procedure TFrPB.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (KodeGdg.Text <> '-') or (QuBeli.IsEmpty) then
    begin
      ...
      begin
         ...
         FrMainPB.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
         Action:=cafree;
      end
      else
      begin
...
procedure TFrPB.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=13 then SendMessage(handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrPB.FormShow(Sender: TObject);
begin
  mValid:=False;
  xModalKoreksi:=FrMainPB.ModalKoreksi;
  IsTampil:=False;
  IsTampil1:=False;
  ...
  begin
    ...
    Begin
     ...
     NoBppB.Text:=FrMainPB.QuBPPBNOBUKTI.AsString;
     mvalid:=true;
     {NoBppBExit(NoBppB);
     TampilData(NoBukti.Text,'0');}
    //NoUrut.Enabled:=True;
  ...
  Begin
    NOBUKTI.Text := FrMainPB.QuMaster.FieldByname('Nobukti').AsString;
...
procedure TFrPB.FormCreate(Sender: TObject);
begin
  TipeTrans := 'BP';
  Hapus_Daftar_Nomor_User(TipeTrans,IDUser);
end;

procedure TFrPB.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrPB.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPB.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPB.KoreksiBtnClick(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    mValid:=False;
...
procedure TFrPB.wwDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrPB.HapusBtnClick(Sender: TObject);
begin
      if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
      begin
...
procedure TFrPB.wwDBGrid1Enter(Sender: TObject);
begin
  FrPB.KeyPreview:=false;
...
procedure TFrPB.wwDBGrid1Exit(Sender: TObject);
begin
  FrPB.KeyPreview:=true;
end;

procedure TFrPB.wwDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrPB.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {if Key=Vk_Return then
  begin
...
procedure TFrPB.TanggalEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrPB.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrPB.CetakClick(Sender: TObject);
var N1, N2, N3 : string;
    T1, T2 : Real;
    Tot : byte;
    H1  : real;
...
procedure TFrPB.BitBtn1Click(Sender: TObject);
begin
  if Model='write' then
  begin
...
procedure TFrPB.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
...
procedure TFrPB.FormDestroy(Sender: TObject);
begin
  FrPB:=nil;
end;

procedure TFrPB.KodeGdgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrPB.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrPB.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrPB.SpeedButton8Click(Sender: TObject);
begin
  if ComboBox1.ItemIndex=0 then
  frxReport1.LoadFromFile(currDir+'Nota\NotaPemakaianBahan.fr3')
  else
...
procedure TFrPB.TanggalChange(Sender: TObject);
begin
  mUbahHd:=True;
end;

procedure TFrPB.NoUrutEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrPB.IsiSatuan;
begin
...
procedure TFrPB.KodeBrgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrPB.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrPB.TampilIsiGudang;
begin
  KodeBrows:=11009;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.IsiData:=KodeGdg.Text;
...
procedure TFrPB.KodeGdgExit(Sender: TObject);
begin
 if mValid Then
  begin
...
procedure TFrPB.QntChange(Sender: TObject);
begin
  if mValid then
  begin
    if NoSat.AsInteger=1 then
...
procedure TFrPB.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrPB.NoSatChange(Sender: TObject);
begin
  if mValid then
  begin
    TampilanQnt1Qnt2;
...
procedure TFrPB.NoBppBExit(Sender: TObject);
begin
...
procedure TFrPB.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DbpenyerahanBhn',NoBukti.Text);
end;

procedure TFrPB.NoUrutExit(Sender: TObject);
...
procedure TFrPB.KodeGdgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=Vk_Return then
  begin
...
procedure TFrPB.PerkiraanExit(Sender: TObject);
begin
  if mValid=true then
  begin
    if Perkiraan.Text<>'-' then
...
procedure TFrPB.QntExit(Sender: TObject);
begin
