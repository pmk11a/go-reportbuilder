...
var
  FrMainRPBLain: TFrMainRPBLain;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmRPBLain;
{$R *.dfm}

procedure TFrMainRPBLain.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainRPBLain.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainRPBLain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrRPBLain')<>nil then
    begin
       MessageDlg('Modul '+FrRPBLain.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainRPBLain.FormDestroy(Sender: TObject);
begin
  FrMainRPBLain:=nil;
end;

procedure TFrMainRPBLain.FormShow(Sender: TObject);
begin
...
procedure TFrMainRPBLain.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainRPBLain.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainRPBLain.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainRPBLain.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainRPBLain.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainRPBLain.Show1Click(Sender: TObject);
begin
...
procedure TFrMainRPBLain.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrRPBLain) then FrRPBLain.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrRPBLain,FrRPBLain);
        CekOtoritasMenu(IDUser,tag,FrRPBLain.Istambah,FrRPBLain.Ishapus,
          FrRPBLain.Iskoreksi, FrRPBLain.isCetak, FrRPBLain.isExcel);
        FrRPBLain.Show;
      end;
    end
    else
...
procedure TFrMainRPBLain.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainRPBLain.ToolButton2Click(Sender: TObject);
begin
...
procedure TFrMainRPBLain.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainRPBLain.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainRPBLain.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainRPBLain.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainRPBLain.tvMasterFocusedRecordChanged(
...
procedure TFrMainRPBLain.tvPOCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrMainRPBLain.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainRPBLain.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbRpenyerahan',QuMasterNoBukti.AsString);
end;

procedure TFrMainRPBLain.ToolButton4Click(Sender: TObject);
begin
if Iscetak then
...
procedure TFrMainRPBLain.ToolButton13Click(Sender: TObject);
begin
...
procedure TFrMainRPBLain.ToolButton12Click(Sender: TObject);
begin
