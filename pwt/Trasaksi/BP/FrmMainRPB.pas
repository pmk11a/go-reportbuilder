...
var
  FrMainRPB: TFrMainRPB;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmRPB;
{$R *.dfm}

procedure TFrMainRPB.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainRPB.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainRPB.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrRPB')<>nil then
    begin
       MessageDlg('Modul '+FrRPB.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainRPB.FormDestroy(Sender: TObject);
begin
  FrMainRPB:=nil;
end;

procedure TFrMainRPB.FormShow(Sender: TObject);
begin
...
procedure TFrMainRPB.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainRPB.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainRPB.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainRPB.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainRPB.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainRPB.Show1Click(Sender: TObject);
begin
...
procedure TFrMainRPB.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrRPB) then FrRPB.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrRPB,FrRPB);
        CekOtoritasMenu(IDUser,tag,FrRPB.Istambah,FrRPB.Ishapus,
          FrRPB.Iskoreksi, FrRPB.isCetak, FrRPB.isExcel);
        FrRPB.Show;
      end;
    end
    else
...
procedure TFrMainRPB.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainRPB.ToolButton2Click(Sender: TObject);
begin
...
procedure TFrMainRPB.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainRPB.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainRPB.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainRPB.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainRPB.tvMasterFocusedRecordChanged(
...
procedure TFrMainRPB.tvPOCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrMainRPB.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainRPB.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbRpenyerahan',QuMasterNoBukti.AsString);
end;

procedure TFrMainRPB.ToolButton4Click(Sender: TObject);
begin
if Iscetak then
...
procedure TFrMainRPB.ToolButton13Click(Sender: TObject);
begin
...
procedure TFrMainRPB.ToolButton12Click(Sender: TObject);
begin
