...
var
  FrMainPPL: TFrMainPPL;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmPPL;
{$R *.dfm}

procedure TFrMainPPL.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainPPL.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainPPL.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrPPL')<>nil then
    begin
       MessageDlg('Modul '+FrPPL.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainPPL.FormDestroy(Sender: TObject);
begin
  FrMainPPL:=nil;
end;

procedure TFrMainPPL.FormShow(Sender: TObject);
begin
...
procedure TFrMainPPL.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
   ViewOtorisasiNonBandedTableView(TvMaster,Qumaster,PPLOL);
end;

procedure TFrMainPPL.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainPPL.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainPPL.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainPPL.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainPPL.Show1Click(Sender: TObject);
begin
...
procedure TFrMainPPL.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrPPL) then FrPPL.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrPPL,FrPPL);
        CekOtoritasMenu(IDUser,tag,FrPPL.Istambah,FrPPL.Ishapus,
          FrPPL.Iskoreksi, FrPPL.isCetak, FrPPL.isExcel);
        FrPPL.Show;
      end;
    end
    else
...
procedure TFrMainPPL.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainPPL.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainPPL.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainPPL.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainPPL.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainPPL.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainPPL.tvMasterFocusedRecordChanged(
...
procedure TFrMainPPL.dxPageControl1Change(Sender: TObject);
begin
 //ToolButton1.Enabled:=dxPageControl1.ActivePageIndex=0;
 //ToolButton2.Enabled:=dxPageControl1.ActivePageIndex=1;
end;

procedure TFrMainPPL.ToolButton4Click(Sender: TObject);
begin
    if IsCetak then
      begin
...
procedure TFrMainPPL.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainPPL.frxReport1AfterPrintReport(Sender: TObject);
var cetakke:Integer;
begin
 DataBersyarat('Select Isnull(Cetakke,0)+1 cetakke from dbPPL where nobukti=:0',[QuMasterNoBukti.AsString], Dm.Qucari);
 cetakke:=Dm.Qucari.Fieldbyname('Cetakke').AsInteger;
 with dm.qucari do
  begin
    close;
    sql.Clear;
    Sql.add('Update DBPPL set cetakke=:0 where Nobukti='+Quotedstr(QuMasterNoBukti.asstring));
...
procedure TFrMainPPL.ToolButton13Click(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      Otorisasi('DBPPL',MenuPPL,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainPPL.ToolButton12Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DBPPL',MenuPPL,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
...
procedure TFrMainPPL.tvMasterCellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TFrMainPPL.ToolButton14Click(Sender: TObject);
begin
IF DataBersyarat(' select A.Nobukti,B.NOPPL from DBPPL A   '+
...
procedure TFrMainPPL.tvMasterCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
