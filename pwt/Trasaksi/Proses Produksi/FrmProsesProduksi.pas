...
type
  ...
  TFrProsesProduksi = class(TForm)
    ...
    dxDBGrid1TglSelesai: TdxDBGridDateColumn;
...
begin
   ...
   begin
      //Nospk.Text :=QuViewNOBUKTI.AsString;
      tanggal.Date:=QuViewTANGGAL.AsDateTime;
      NamaCustomer.Caption:=QuViewNAMACUSTSUPP.AsString;
      Qnt.Caption:=QuViewQnt.AsString;
      JenisPesanan.Caption:=QuViewKODEBRG.AsString;
      Nospk.Text:=norpp;

      Kodearea.Text:=QuViewKODEmesin.AsString;
      if QuViewKODEmesin.AsString='' then
        mValid:=false
      else
        mValid:=true;
      KodeAreaExit(kodearea);
...
begin
  ...
  begin
    Close;
    sql.Clear;
    sql.Add('select * from dbprosesproduksi where nobukti =:0');
    Parameters[0].Value := QuViewNOBUKTI.AsString;
    Open;
  ...
  begin
    ...
    begin
      Parameters[3].Value := FrSubprosesproduksi.Proses.ItemIndex;
      Parameters[4].Value := FrSubprosesproduksi.KodeArea.Text;
      Parameters[2].Value := FrSubprosesproduksi.Nospk.Text;
    end;
    try
      execproc;
      if Mode='I' then
...
begin
   ...
   begin
      close;
      SQL.Strings[2]:='select @tahun='+PeriodThn;
