...
begin
  ...
  begin
    ...
    begin
      ...
      qnt.MaxValue:=  FieldByName('sisaspk').AsFloat;
    end;
  end
   else
    ActiveControl:=NoSPK;
...
function  TFrHasilp.CekAdaSuratJln(pNOSO: String): Boolean;
...
{  xNoSuratJln:='';
  ...
  begin
    ...
    SQL.Add('where  Nospp='+QuotedStr(QuBeliNoBukti.AsString));
    Open;
    xStrMsgDet:='';
    if not IsEmpty then
    begin
...
begin
 if mValid Then
  ...
  begin
        Close;
        sql.Clear;
        sql.Add('select dbo.CekHpdOutSourceBhn ('+quotedstr(nospk.text)+' ) Sisabahan');
        Open;
        if FieldByName('sisabahan').AsBoolean  then

           MessageDlg('Masih Ada Bahan Bahan Belum Masuk',mtInformation,[mbOK],0);
  end;
end;
