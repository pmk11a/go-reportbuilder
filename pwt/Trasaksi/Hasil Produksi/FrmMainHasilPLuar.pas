...
var
  FrMainHasilPLuar: TFrMainHasilPLuar;
...
procedure TFrMainHasilPLuar.GetData(bulan,tahun:integer);
begin
  QuNavigator.Close;
...
procedure TFrMainHasilPLuar.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainHasilPLuar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrHasilPLuar')<>nil then
    begin
       MessageDlg('Modul '+FrHasilPLuar.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainHasilPLuar.FormDestroy(Sender: TObject);
begin
  FrMainHasilPLuar:=nil;
end;

procedure TFrMainHasilPLuar.FormShow(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainHasilPLuar.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainHasilPLuar.ExportExcel1Click(Sender: TObject);
begin
  with QuView do
  begin
    Close;
...
procedure TFrMainHasilPLuar.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.Show1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrHasilPLuar) then FrHasilPLuar.Show else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrHasilPLuar,FrHasilPLuar);
        CekOtoritasMenu(IDUser,tag,FrHasilPLuar.Istambah,FrHasilPLuar.Ishapus,
          FrHasilPLuar.Iskoreksi, FrHasilPLuar.isCetak, FrHasilPLuar.isExcel);
...
procedure TFrMainHasilPLuar.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainHasilPLuar.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainHasilPLuar.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainHasilPLuar.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainHasilPLuar.FormCreate(Sender: TObject);
begin
  Panel1.Color:=FrSetForm.PanelMain.Color;
  ToolBar1.Color:=FrSetForm.ToolBarMain.Color;
end;

procedure TFrMainHasilPLuar.tvMasterFocusedRecordChanged(
...
procedure TFrMainHasilPLuar.ToolButton4Click(Sender: TObject);
begin
if Iscetak then
...
procedure TFrMainHasilPLuar.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DBHasilPrd',QuMasterNoBukti.AsString);
end;

procedure TFrMainHasilPLuar.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainHasilPLuar.ToolButton12Click(Sender: TObject);
begin
 {if QuMasterIsOtorisasi1.Value then
  ...
  begin
    ...
    begin
      Otorisasi('DBHasilPrdLuar',MenuHasilP,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainHasilPLuar.ToolButton13Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DBHasilPrdLuar',MenuHasilP,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
