...
procedure TFrKoreksiDK.IsiHargaBrg;
begin
  with DM.QuCari do
  begin
    Close;
...
procedure TFrKoreksiDK.IsiNoBuktiBaru;
begin
...
procedure TFrKoreksiDK.TampilIsiBarang;
begin
  //KodeBrows:=11051;
  KodeBrows:=242013;
  ...
  begin
    ...
    Qnt2.Enabled:=FrBrows.QuBrows.FieldByName('NFix').AsBoolean  ;
    Qnt2Cr.Enabled:=FrBrows.QuBrows.FieldByName('NFix').AsBoolean ;
    IsiHargaBrg;
  end
  else
...
procedure TFrKoreksiDK.TampilIsiGudang;
begin
  KodeBrows:=916;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.IsiData:=KodeGdg.Text;
...
procedure TFrKoreksiDK.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrKoreksiDK.UpdatedbBeli;
begin
...
Function TFrKoreksiDK.CheckFormMaster:boolean;
begin
...
procedure TFrKoreksiDK.ClearPanelDetail;
begin
   KodeBrg.Text:='';
   Qnt.Value:=0;
   Qnt2.Value:=0;
...
procedure TFrKoreksiDK.RefreshAll;
var i:integer;
begin
...
procedure TFrKoreksiDK.UpdateDataBeli(Choice:Char;Kodet:String);
begin
...
Procedure TFrKoreksiDK.AmBilDataBeli;
begin
...
procedure TFrKoreksiDK.TampilData(No_Bukti:String);
begin
  QuBeli.close;
...
procedure TFrKoreksiDK.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if mExit=true then
  begin
     FrMainKoreksiDK.xStatus := 'M';
...
procedure TFrKoreksiDK.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrKoreksiDK.NoUrutEnter(Sender: TObject);
begin
...
procedure TFrKoreksiDK.FormCreate(Sender: TObject);
begin
  TipeTrans :='KRS';
  Hapus_Daftar_Nomor_User(TipeTrans ,IDUser);
end;

procedure TFrKoreksiDK.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrKoreksiDK.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrKoreksiDK.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=VK_Escape then
   begin
...
procedure TFrKoreksiDK.TANGGALKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  ...
  begin
    ...
    begin
       if  (FrMainKoreksiDK.ModalKoreksi ) then
       UpdatedbBeli;
    end
    else
    begin
   ...
   if (not FrMainKoreksiDK.ModalKoreksi) then
     begin
      with QuUpdatedbbeli do
       begin
        Close;
  ...
  begin
    If FrMainKoreksiDK.ModalKoreksi = False then
    begin
      Hapus_Daftar_Nomor_User(TipeTrans ,IDUser);
      IsiNoBuktiBaru;
      Keterangan.Text :='';
...
procedure TFrKoreksiDK.KodeBrgEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrKoreksiDK.KoreksiBtnClick(Sender: TObject);
begin
  if CheckFormMaster=true then
  begin
     mValid:=false;
     ...
     begin
        ...
        Qnt2.Enabled:=QuBeliNfix.AsBoolean;
        Qnt2Cr.Enabled:=QuBeliNfix.AsBoolean;
        //IsiHargaBrg;
        {if Qnt.Value = 0 then
           Harga.Value :=0;}
        if LevelUserAccess=0 then
...
procedure TFrKoreksiDK.HapusBtnClick(Sender: TObject);
begin
  if CheckFormMaster=true then
  begin
...
procedure TFrKoreksiDK.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {if Key=Vk_Return then
  begin
...
procedure TFrKoreksiDK.TANGGALEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrKoreksiDK.TANGGALExit(Sender: TObject);
begin
  if mValid then
  begin
     if CekPeriode(IdUser,TANGGAL.Date)=false then
...
procedure TFrKoreksiDK.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
     ...
     begin
        ...
        begin
           ...
           Qnt2.Enabled:=DM.QuCari.FieldByName('NFix').AsBoolean;
           Qnt2Cr.Enabled:=DM.QuCari.FieldByName('NFix').AsBoolean;
           IsiHargaBrg;
        end
...
procedure TFrKoreksiDK.BitBtn1Click(Sender: TObject);
begin
  if (not FrMainKoreksiDK.ModalKoreksi) then
     begin
      with QuUpdatedbbeli do
   ...
   begin
     ...
     ShowMessage('Tanggal Periode Tidak Sesuai !');
   FrMainKoreksiDK.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrKoreksiDK.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
  TampilTombolDetail(False);
  ActiveControl:=dxDBGrid1;
...
procedure TFrKoreksiDK.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
   QuBeli.Sort:=AFieldName;
end;

procedure TFrKoreksiDK.KodegdgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrKoreksiDK.KodegdgEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrKoreksiDK.QntChange(Sender: TObject);
begin
...
procedure TFrKoreksiDK.FormShow(Sender: TObject);
begin
 If FrMainKoreksiDk.ModalKoreksi = False then
  begin
    FTambah:=1;
  ...
  Begin
    NOBUKTI.Text := FrMainKoreksiDK.QuMaster.FieldByname('Nobukti').AsString;
    TampilData(NOBUKTI.Text);
    AmBilDataBeli;
    NoUrut.Text := leftstr(FrMainKoreksiDK.QuMaster.FieldByname('nobukti').AsString,5);
  end;
end;

procedure TFrKoreksiDK.FormDestroy(Sender: TObject);
begin
  FrKoreksiDK:=nil;
end;

procedure TFrKoreksiDK.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrKoreksiDK.dxDBGrid1Enter(Sender: TObject);
begin
  FrKoreksiDK.KeyPreview:=false;
...
procedure TFrKoreksiDK.dxDBGrid1Exit(Sender: TObject);
begin
  FrKoreksiDK.KeyPreview:=true;
end;

procedure TFrKoreksiDK.dxDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrKoreksiDK.BPrevClick(Sender: TObject);
begin
   Hapus_Daftar_Nomor(TipeTrans,mBukti,iduser);
   RefreshAll;
   ActiveControl:=TANGGAL;
   ...
   FrMainKoreksiDK.QuNavigator.Locate('nobukti',FrMainKoreksiDK.QuMaster.fieldbyname('nobukti').AsString,[LOC,LOP]);
   FrMainKoreksiDK.QuNavigator.Prior;
   FrMainKoreksiDK.QuMaster.Locate('nobukti',FrMainKoreksiDK.QuNavigator.fieldbyname('nobukti').AsString,[LOC,LOP]);

   FormShow(self);
end;

procedure TFrKoreksiDK.BNextClick(Sender: TObject);
begin
   Hapus_Daftar_Nomor(TipeTrans,mBukti,iduser);
   RefreshAll;
   ActiveControl:=TANGGAL;
   ...
   FrMainKoreksiDK.QuNavigator.Locate('nobukti',FrMainKoreksiDK.QuMaster.fieldbyname('nobukti').AsString,[LOC,LOP]);
   FrMainKoreksiDK.QuNavigator.Next;
   FrMainKoreksiDK.QuMaster.Locate('nobukti',FrMainKoreksiDK.QuNavigator.fieldbyname('nobukti').AsString,[LOC,LOP]);

   FormShow(self);
end;

procedure TFrKoreksiDK.HargaExit(Sender: TObject);
begin
  //if Qnt.Value = 0 then
  //   Harga.Value :=0;
end;

procedure TFrKoreksiDK.NoUrutChange(Sender: TObject);
begin
  if Length(NoUrut.Text)=4 then
  begin
...
procedure TFrKoreksiDK.QntOpnameChange(Sender: TObject);
begin
...
procedure TFrKoreksiDK.QntOpnameKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
   begin
...
procedure TFrKoreksiDK.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrKoreksiDK.CetakClick(Sender: TObject);
begin
  With QuUpdatedbbeli do
  begin
    close;
...
procedure TFrKoreksiDK.SpeedButton8Click(Sender: TObject);
begin
  frxReport1.LoadFromFile(currDir+'Nota\NotaKoreksi.fr3');
  frxReport1.DesignReport;
end;

procedure TFrKoreksiDK.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrKoreksiDK.Qnt2OpnameChange(Sender: TObject);
begin
 Selisih2.AsFloat := Qnt2Opname.AsFloat-Stock2Comp.AsFloat;
  if Selisih2.AsFloat >= 0 then
  begin
...
procedure TFrKoreksiDK.Qnt2OpnameKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key=VK_Escape then
   begin
...
procedure TFrKoreksiDK.frxReport1AfterPrintReport(Sender: TObject);
begin
 NilaiCetak('NilaiCetak','DBKoreksi',NoBukti.Text);
end;

procedure TFrKoreksiDK.NoUrutExit(Sender: TObject);
var myTempNoUrut: String;
begin
 if FrKoreksiDK.IsKoreksi= false then
 begin
    if mValid then
    begin
...
procedure TFrKoreksiDK.qntcrChange(Sender: TObject);
begin
if XFix=false then
   begin
    if mvalid=true then
     Qnt2Cr.Value:=QntCR.Value/Xisi;
   End;
end;

procedure TFrKoreksiDK.QntEnter(Sender: TObject);
begin
mValid:=True;
end;
