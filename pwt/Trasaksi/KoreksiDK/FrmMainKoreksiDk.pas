...
type
  ...
  public
    { Public declarations }
    mPosisi,xStatus,menuKoreksi : string;
...
var
  FrMainKoreksiDK: TFrMainKoreksiDK;
  S: array[0..255] of char;
implementation
uses myModul,Myprocedure,MyGlobal, 
  FrmBrows, FrmKoreksiDK ;
{$R *.dfm}

procedure TFrMainKoreksiDK.GetData(bulan,tahun:integer);
begin
        with QuMaster do
        begin
                Close;
                SQL.Clear;
                SQL.Add('Select * from vwMasterKoreksi where month(tanggal)=:0 and year(tanggal)=:1 and nobukti like ''%KRS%'' Order by nobukti');
                Prepared;
...
procedure TFrMainKoreksiDK.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainKoreksiDK.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrMainKoreksiDK.FormDestroy(Sender: TObject);
begin
  FrMainKoreksiDK:=nil;
end;

procedure TFrMainKoreksiDK.FormShow(Sender: TObject);
begin
  xStatus := 'S';
  ToolButton10.Click;
...
procedure TFrMainKoreksiDK.ToolButton10Click(Sender: TObject);
begin
   GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainKoreksiDK.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainKoreksiDK.ExportExcel1Click(Sender: TObject);
begin
  with QuView do
  begin
    Close;
  ...
  if (IsExcel) then
  begin
      Save('xls', 'Microsoft Excel (*.xls)|*.xls', FrMainKoreksiDK.Caption, dxDBGrid1.SaveToXLS);
...
procedure TFrMainKoreksiDK.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiDK.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiDK.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiDK.Show1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiDK.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrkoreksiDK) then
         FrKoreksiDK.Show
      else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrKoreksiDK,FrkoreksiDK);
        CekOtoritasMenu(IDUser,Tag,FrkoreksiDK.Istambah,FrkoreksiDK.Ishapus,
                        FrkoreksiDK.Iskoreksi, FrkoreksiDK.isCetak, FrkoreksiDK.isExcel);
        FrkoreksiDK.PanelShow2.Visible:=false;
        FrkoreksiDK.Show;
        if  ((FrkoreksiDK.IsKoreksi=False) or (FrkoreksiDK.IsHapus=False)) then
          FrkoreksiDK.NoUrut.Enabled:=False
        else  FrkoreksiDK.NoUrut.Enabled:=True;
      end;
    end
    else
    begin
...
procedure TFrMainKoreksiDK.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainKoreksiDK.ToolButton2Click(Sender: TObject);
begin
                  ...
                  begin
                   ...
                   begin
                     ...
                     begin
                       ...
                       CekOtoritasMenu(IDUser,Tag,FrkoreksiDK.Istambah,FrkoreksiDK.Ishapus,
                                    FrkoreksiDK.Iskoreksi, FrkoreksiDK.isCetak, FrkoreksiDK.isExcel);
                       FrkoreksiDK.PanelShow2.Visible:=true;
...
procedure TFrMainKoreksiDK.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiDK.ppReport1PreviewFormCreate(Sender: TObject);
begin
//  TppReport(Sender).PreviewForm.WindowState:=wsMaximized;
//  TppViewer(TppReport(Sender).PreviewForm.Viewer).ZoomSetting:=zs100Percent;
end;

procedure TFrMainKoreksiDK.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainKoreksiDK.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrMainKoreksiDK.dxDBGrid1CustomDrawFooter(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
procedure TFrMainKoreksiDK.tvMasterFocusedRecordChanged(
...
procedure TFrMainKoreksiDK.frxReport1AfterPrintReport(Sender: TObject);
begin
 NilaiCetak('NilaiCetak','DBKoreksi',QuMasterNoBukti.AsString);
end;

procedure TFrMainKoreksiDK.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainKoreksiDK.ToolButton3Click(Sender: TObject);
begin
 { if (IsCetak) then
  begin
  ...
  if Iscetak then
    MainCetak(frxDBDataset1,frxDataPerusahaan,DM.QuCari,Dm.QuCari2,'CetakOpnamebahan','Vwperusahaan',QuMasterNoBukti.AsString,'Nota\NotaKoreksi.fr3',frxReport1)
else
   MsgTidakBerhakCetakData
end;

procedure TFrMainKoreksiDK.ToolButton4Click(Sender: TObject);
begin
 {if QuMasterIsOtorisasi1.Value then
  ...
  begin
    ...
    begin
      Otorisasi('DBKoreksi',MenuKoreksi,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainKoreksiDK.ToolButton11Click(Sender: TObject);
begin
CekNota('NIlaiCetak','DbKOreksi',QuMasterNobukti.AsString);
    if SCetak then MSGStatusCetak else
    begin
    BM := QuMaster.GetBookmark;
      {frLogin := TFrLogin.Create(Self);
      frLogin.Label2.Caption :='Otorisasi';
      if frLogin.ShowModal=mrok then  }
      begin
        BatalOtorisasi('DbKoreksi',MenuKoreksi,IDUser,'',QuMasterNoBukti.Value);
        Begin
          ToolButton10.Click;
          if QuMaster.BookmarkValid(BM) then
          begin
