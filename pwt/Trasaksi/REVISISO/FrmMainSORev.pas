...
uses
  ...
  cxGridCustomView, cxGrid, dxPageControl, frxClass, frxDBSet;

type
  TSaveMethod = procedure (const FileName: String; ASaveAll: Boolean) of object;

  TFrMainSORev = class(TForm)
    ToolBar1: TToolBar;
    ...
    dsQuOSDetail: TDataSource;
    QuOSDetailTanggal: TDateTimeField;
    cxGridDBTableView1NoBukti: TcxGridDBColumn;
    cxGridDBTableView1Tanggal: TcxGridDBColumn;
    cxGridDBTableView1KodeSupp: TcxGridDBColumn;
    cxGridDBTableView1NamaCustSupp: TcxGridDBColumn;
    cxGridDBTableView2KodeBrg: TcxGridDBColumn;
    cxGridDBTableView2NamaBrg: TcxGridDBColumn;
    cxGridDBTableView2Qnt: TcxGridDBColumn;
    cxGridDBTableView2NoSat: TcxGridDBColumn;
    cxGridDBTableView2Isi: TcxGridDBColumn;
    cxGridDBTableView2Satuan: TcxGridDBColumn;
    cxGridDBTableView2Qnt2: TcxGridDBColumn;
    cxGridDBTableView2SatuanRoll: TcxGridDBColumn;
    cxGridDBTableView2Harga: TcxGridDBColumn;
    cxGridDBTableView2DiscP1: TcxGridDBColumn;
    cxGridDBTableView2DiscRp1: TcxGridDBColumn;
    cxGridDBTableView2DiscTot: TcxGridDBColumn;
    cxGridDBTableView2TotalUSD: TcxGridDBColumn;
    cxGridDBTableView2TotalIDR: TcxGridDBColumn;
    cxGridDBTableView2NDPP: TcxGridDBColumn;
    cxGridDBTableView2NPPN: TcxGridDBColumn;
    cxGridDBTableView2Beban: TcxGridDBColumn;
    ...
    QuOSNoBukti: TStringField;
    QuOSTanggal: TDateTimeField;
    QuOSKodeCust: TStringField;
    QuOSNamaCustSupp: TStringField;
...
var
  FrMainSORev: TFrMainSORev;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmSetForm, FrmRevSO;
{$R *.dfm}

procedure TFrMainSORev.GetData(bulan,tahun:integer);
begin
  QuNavigator.Close;
  QuNavigator.SQL.Clear;
  QuNavigator.SQL.Add('select NoBukti from dbSO '+
...
procedure TFrMainSORev.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainSORev.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrRevSO')<>nil then
    begin
       MessageDlg('Modul '+FrRevSO.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainSORev.FormDestroy(Sender: TObject);
begin
  FrMainSORev:=nil;
end;

procedure TFrMainSORev.FormShow(Sender: TObject);
begin
...
procedure TFrMainSORev.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainSORev.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainSORev.ExportExcel1Click(Sender: TObject);
begin
{  with QuView do
  begin
    Close;
...
procedure TFrMainSORev.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainSORev.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainSORev.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainSORev.Show1Click(Sender: TObject);
begin
...
procedure TFrMainSORev.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrRevSO) then FrRevSO.Show else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        QuNavigator.Close;
        QuNavigator.SQL.Clear;
        QuNavigator.sql.Add('exec [sp_SORev] '+QuotedStr(QuOSNoBukti.AsString)+',0,0,'+QuotedStr('Revisi')+',0' );
        QuNavigator.ExecSQL;
        Application.CreateForm(TFrRevSO,FrRevSO);
        CekOtoritasMenu(IDUser,tag,FrRevSO.Istambah,FrRevSO.Ishapus,
          FrRevSO.Iskoreksi, FrRevSO.isCetak, FrRevSO.isExcel);
...
procedure TFrMainSORev.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainSORev.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
    begin
      ...
      begin
        ...
        begin
          ...
          //FrRevSO.PanelShow2.Visible:=true;
          //FrRevSO.ActiveControl := FrRevSO.Tanggal;
          FrRevSO.Show;
        end;
      end else
      begin
...
procedure TFrMainSORev.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainSORev.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainSORev.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainSORev.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainSORev.FormCreate(Sender: TObject);
begin
...
procedure TFrMainSORev.tvMasterFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TFrMainSORev.ToolButton4Click(Sender: TObject);
begin
 if QuMasterIsOtorisasi1.Value then
  begin
...
procedure TFrMainSORev.ToolButton12Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
...
procedure TFrMainSORev.cxGridDBTableView1FocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TFrMainSORev.ToolButton13Click(Sender: TObject);
begin

  Qudetail.Close;
  Qudetail.SQL.Clear;
  Qudetail.SQL.Add('exec sp_CetakSO1Revisi '+QuotedStr(QuMasterNoBukti.AsString));
  Qudetail.Open;
  frxDBCetak.Close;
  frxDBCetak.Open;
