...
var
  FrMainClose: TFrMainClose;
  S: array[0..255] of char;
implementation
uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmSO, FrmSetForm, frmLogin;
{$R *.dfm}

procedure TFrMainClose.GetData(bulan,tahun:integer);
...
procedure TFrMainClose.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainClose.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrMainClose.FormDestroy(Sender: TObject);
begin
  FrMainSO:=nil;
end;

procedure TFrMainClose.FormShow(Sender: TObject);
begin
  dxPageControl1.ActivePageIndex:=0;
  ToolButton10.Click;
end;

procedure TFrMainClose.ToolButton10Click(Sender: TObject);
begin
   GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainClose.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainClose.ToolButton1Click(Sender: TObject);
begin
...
procedure TFrMainClose.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainClose.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainClose.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainClose.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainClose.FormCreate(Sender: TObject);
begin
...
procedure TFrMainClose.ExportExcel1Click(Sender: TObject);
begin
...
procedure TFrMainClose.tvKotaSOFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TFrMainClose.tvMasterSOFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TFrMainClose.tvMasterFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TFrMainClose.ToolButton4Click(Sender: TObject);
begin
  //if Not QuMasterNeedOtorisasi.Value then
...
procedure TFrMainClose.ToolButton12Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
...
procedure TFrMainClose.ToolButton13Click(Sender: TObject);
begin
if Iscetak then
begin
...
procedure TFrMainClose.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DBSO',QuMasterNoBukti.AsString);
end;

procedure TFrMainClose.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainClose.ToolButton14Click(Sender: TObject);
begin
...
procedure TFrMainClose.tvMasterCellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
 if QuMasterIsBatal.AsBoolean=true then
...
procedure TFrMainClose.tvDetailCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrMainClose.tvMasterCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
