...
procedure TFrMainPO.frxReport1AfterPrintReport(Sender: TObject);
var CetakKe:Integer;
begin
...
procedure TFrMainPO.ToolButton12Click(Sender: TObject);
begin
{if IsCetak then
      begin
...
procedure TFrMainPO.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainPO.ToolButton13Click(Sender: TObject);
begin
...
procedure TFrMainPO.CetakPO1Click(Sender: TObject);
begin
  if IsCetak then
  begin
      ...
      frxReport1.ShowReport;
        ...
        if QuMasterKodevls.AsString <>'IDR' then

    begin
      frxDBDataset2.DataSet.Close;
      frxDBDataset2.DataSet := Dm.QuCari;
      with Dm.QuCari do
      begin
        Close;
        Sql.Clear;
        Sql.Add('exec Sp_CetakPOD :0, :1');
        Prepared;
        Parameters[0].Value :=QuMasterNoBukti.AsString;
        Parameters[1].Value :=0;
        Open;
      ...
      frxDataPerusahaan.DataSet.Open;
      frxReport2.LoadFromFile(currDir+'Nota\NotaPO1D.fr3');
...
procedure TFrMainPO.CetakPOTanpaHarga1Click(Sender: TObject);
begin
  if IsCetak then
  begin
      frxDBDataset1.DataSet.Close;
...
procedure TFrMainPO.tvMasterCellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TFrMainPO.ToolButton14Click(Sender: TObject);
begin
IF DataBersyarat(' select A.NoBukti,B.noPO,C.NOspb from DBPO A  '+
               ' Left outer Join DBBELIdet B on A.NOBUKTI = B.NoPO    '+
               ' left outer join DBSO C on A.Nobukti=C.NOspb '+
               ' where (B.NoPO is Null or B.NoPO='') and  (C.NoSPB is Null or C.NOSPB='') and A.nobukti=:0',[QuMasterNoBukti.ASstring],DM.Qucari)=true then
 begin
    BM := QuMaster.GetBookmark;
    begin
     pembatalan('DBPO','IsBatal',QuMasterNoBukti.AsString,IDUser,MenuPO);
      Begin
        ToolButton10.Click;
        if QuMaster.BookmarkValid(BM) then
        begin
...
procedure TFrMainPO.tvMasterCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
