...
type
  ...
  TFrMainInv = class(TForm)
    ...
    tvMasterDBUserAutho: TcxGridDBColumn;
...
procedure TFrMainInv.GetData(bulan,tahun:integer);
begin
  QuNavigator.Close;
  QuNavigator.SQL.Clear;
  QuNavigator.SQL.Add('select NoBukti from dbInvoice '+
...
procedure TFrMainInv.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainInv.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrInv')<>nil then
    begin
       MessageDlg('Modul '+FrInv.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainInv.FormDestroy(Sender: TObject);
begin
  FrMainInv:=nil;
end;

procedure TFrMainInv.FormShow(Sender: TObject);
begin
...
procedure TFrMainInv.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainInv.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainInv.ExportExcel1Click(Sender: TObject);
begin
 { with QuView do
  begin
    Close;
...
procedure TFrMainInv.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainInv.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainInv.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainInv.Show1Click(Sender: TObject);
begin
...
procedure TFrMainInv.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrInv) then FrInv.Show else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrInv,FrInv);
        CekOtoritasMenu(IDUser,tag,FrInv.Istambah,FrInv.Ishapus,
          FrInv.Iskoreksi, FrInv.isCetak, FrInv.isExcel);
...
procedure TFrMainInv.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainInv.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainInv.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainInv.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainInv.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainInv.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainInv.FormCreate(Sender: TObject);
begin
  Panel1.Color:=FrSetForm.PanelMain.Color;
  ToolBar1.Color:=FrSetForm.ToolBarMain.Color;
end;

procedure TFrMainInv.tvMasterFocusedRecordChanged(
...
procedure TFrMainInv.ToolButton4Click(Sender: TObject);
begin
 if QuMasterIsOtorisasi1.Value then
  begin
  ...
  begin
    ...
    begin
      Otorisasi('DBInvoice',Menu,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainInv.ToolButton12Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DbInvoice',Menu,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
