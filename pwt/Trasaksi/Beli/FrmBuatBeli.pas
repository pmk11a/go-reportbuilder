...
type
  ...
  private
    ...
    mValid,mUbahHd, mUbahHdDet: Boolean;
...
var
  FrBuatBeli: TFrBuatBeli;

implementation

uses FrmBrows, MyProcedure, MyModul, FrmMainBeli, FrmSetForm, FrmBeli;

{$R *.dfm}

procedure TFrBuatBeli.TampilData(No_Bukti:String);
begin
   Qubeli.Close;
   Qubeli.SQl.Clear;
...
procedure TFrBuatBeli.IsiNoBuktiBaru;
begin
  //gFlagMenu:=mFlagMenu;
  ...
  //Check_Nomor(StrToInt(PeriodBln),StrToInt(PeriodThn),TipeTrans,Nomor,NomorBukti,Tanggal.Date);
  Check_Nomor(StrToInt(PeriodBln),StrToInt(PeriodThn),TipeTrans,Nomor,
    NomorBukti,Tanggal.Date,Xcustsupp,True);
  NoUrut.Text:=NomorBukti;
  NOBUKTI.Text:=Nomor;
  XBM:=Nomor;
end;

procedure TFrBuatBeli.TampilIsiGudang;
begin
  KodeBrows:=11002;
...
procedure TFrBuatBeli.FormShow(Sender: TObject);
var i:Integer;
begin
  //JenisSuppItemIndex:=FrMainBeli.QuMasterOutJenisSupp.AsInteger;
  NOPO.Text:=FrMainBeli.QumasterPONoBukti.AsString;
  KodeGdg.Text:=FrMainBeli.QuMasterpoKodeGdg.AsString;
  Caption:='Transaksi '+FrMainBeli.Caption;
...
procedure TFrBuatBeli.NoUrutChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrBuatBeli.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrBuatBeli.NoUrutEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrBuatBeli.BitBtn2Click(Sender: TObject);
begin
  Close;
end;

procedure TFrBuatBeli.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TFrBuatBeli.KodeGdgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrBuatBeli.NoUrutExit(Sender: TObject);
begin
...
procedure TFrBuatBeli.BitBtn3Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    FrmainBeli.QuMaster.Locate('Nobukti',FrBuatBeli.XBM,[LOP,LOC]);
    frMainBeli.ModalKoreksi:=True;
...
procedure TFrBuatBeli.KodeGdgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrBuatBeli.dxDBGrid1Enter(Sender: TObject);
begin
  FrBuatBeli.KeyPreview:=False;
  dxDBGrid1.FocusedField:=dxDBGrid1IsTerima.Field;
end;

procedure TFrBuatBeli.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrBuatBeli.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrBuatBeli.dxDBGrid1Exit(Sender: TObject);
begin
  FrBuatBeli.KeyPreview:=True;
end;

procedure TFrBuatBeli.FormCreate(Sender: TObject);
begin
  TipeTrans:='PBL';
  FrBuatBeli.Color:=FrSetForm.PanelTransaksi.Color;
...
procedure TFrBuatBeli.dxDBGrid1QntTerimaValidate(Sender: TObject;
  var ErrorText: String; var Accept: Boolean);
begin
...
procedure TFrBuatBeli.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrBuatBeli.TanggalChange(Sender: TObject);
begin
  mUbahHd:=True;
end;

procedure TFrBuatBeli.TanggalEnter(Sender: TObject);
begin
  mValid:=true;
end;
