...
uses
  ...
  cxGridCustomView, cxGrid, frxClass, frxDBSet, frxDesgn;

type
  TSaveMethod = procedure (const FileName: String; ASaveAll: Boolean) of object;

  TFrMainDebetN = class(TForm)
    ToolBar1: TToolBar;
  ...
  public
    { Public declarations }
    mPosisi,xStatus,MenuDebetN : string;
...
procedure TFrMainDebetN.GetData(bulan,tahun:integer);
begin
  QuNavigator.Close;
  QuNavigator.SQL.Clear;
  QuNavigator.SQL.Add('select NoBukti from dbDebetNote '+
...
procedure TFrMainDebetN.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainDebetN.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrDebetN')<>nil then
    begin
       MessageDlg('Modul '+FrDebetN.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainDebetN.FormDestroy(Sender: TObject);
begin
  FrMainDebetN:=nil;
end;

procedure TFrMainDebetN.FormShow(Sender: TObject);
begin
...
procedure TFrMainDebetN.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainDebetN.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainDebetN.ExportExcel1Click(Sender: TObject);
begin
 { with QuView do
  begin
    Close;
...
procedure TFrMainDebetN.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainDebetN.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainDebetN.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainDebetN.Show1Click(Sender: TObject);
begin
...
procedure TFrMainDebetN.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrDebetN) then FrDebetN.Show else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrDebetN,FrDebetN);
        CekOtoritasMenu(IDUser,tag,FrDebetN.Istambah,FrDebetN.Ishapus,
          FrDebetN.Iskoreksi, FrDebetN.isCetak, FrDebetN.isExcel);
...
procedure TFrMainDebetN.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainDebetN.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainDebetN.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainDebetN.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainDebetN.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainDebetN.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainDebetN.FormCreate(Sender: TObject);
begin
  Panel1.Color:=FrSetForm.PanelMain.Color;
  ToolBar1.Color:=FrSetForm.ToolBarMain.Color;
end;

procedure TFrMainDebetN.tvMasterFocusedRecordChanged(
...
procedure TFrMainDebetN.ToolButton4Click(Sender: TObject);
begin
if Iscetak then
    MainCetak(frxDBDataset1,frxDataPerusahaan,DM.QuCari,Dm.QuCari2,'CetakdebetNote','Vwperusahaan',QuMasterNOBUKTI.AsString,'Nota\NotaDebetNote.fr3',frxReport1)
else
   MsgTidakBerhakCetakData
end;

procedure TFrMainDebetN.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainDebetN.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbDebetNote',QuMasterNOBUKTI.AsString);
end;

procedure TFrMainDebetN.ToolButton13Click(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      Otorisasi('DBDebetNote',MenuDebetN,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainDebetN.ToolButton12Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DBDebetNote',MenuDebetN,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
