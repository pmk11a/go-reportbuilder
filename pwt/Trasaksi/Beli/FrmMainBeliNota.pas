...
uses
  ...
  cxGridCustomView, cxGrid, frxClass, frxDMPExport, frxDesgn, frxDBSet,
  frxExportBIFF, frxExportCSV;

type
  TSaveMethod = procedure (const FileName: String; ASaveAll: Boolean) of object;

  TFrMainBeliNota = class(TForm)
    ToolBar1: TToolBar;
    ...
    tvMasterFakturSupp: TcxGridDBColumn;
    ...
    tvDetailDiscP2: TcxGridDBColumn;
    tvDetailDiscRp2: TcxGridDBColumn;
    tvDetailHrgNetto: TcxGridDBColumn;
...
var
  FrMainBeliNota: TFrMainBeliNota;
...
procedure TFrMainBeliNota.GetData(bulan,tahun:integer);
begin
  QuNavigator.Close;
  ...
  begin
    Close;
    SQL.Strings[2]:=' select @Tahun='+IntToStr(tahun)+', @Bulan='+IntToStr(bulan) +' ,@TorK = '+quotedstr(tunai);
    //SaveToFile('d:\nomor.txt');
    Open;
  end;
...
procedure TFrMainBeliNota.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainBeliNota.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrBeliNota')<>nil then
    begin
       MessageDlg('Modul '+FrBeliNota.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainBeliNota.FormDestroy(Sender: TObject);
begin
  FrMainBeliNota:=nil;
end;

procedure TFrMainBeliNota.FormShow(Sender: TObject);
begin
...
procedure TFrMainBeliNota.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
   //ViewOtorisasiNonBandedTableView(TvMaster,Qumaster,PBOL);
end;

procedure TFrMainBeliNota.ToolButton9Click(Sender: TObject);
begin
   Close;
   FrMainBeliNota:=nil;
end;

procedure TFrMainBeliNota.ExportExcel1Click(Sender: TObject);
begin
  with QuView do
  begin
    Close;
...
procedure TFrMainBeliNota.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainBeliNota.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainBeliNota.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainBeliNota.Show1Click(Sender: TObject);
begin
...
procedure TFrMainBeliNota.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrBeliNota) then FrBeliNota.Show else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrBeliNota,FrBeliNota);
        CekOtoritasMenu(IDUser,tag,FrBeliNota.Istambah,FrBeliNota.Ishapus,
          FrBeliNota.Iskoreksi, FrBeliNota.isCetak, FrBeliNota.isExcel);
...
procedure TFrMainBeliNota.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainBeliNota.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainBeliNota.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainBeliNota.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainBeliNota.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainBeliNota.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainBeliNota.FormCreate(Sender: TObject);
begin
  tunai:=inttostr(gFlagMenu);
  Panel1.Color:=FrSetForm.PanelMain.Color;
  ToolBar1.Color:=FrSetForm.ToolBarMain.Color;
end;

procedure TFrMainBeliNota.tvMasterFocusedRecordChanged(
...
procedure TFrMainBeliNota.ToolButton4Click(Sender: TObject);
begin

if Iscetak then
    MainCetak(frxDBDataset1,frxDataPerusahaan,DM.QuCari,Dm.QuCari2,'CetakPenerimaanACC','Vwperusahaan',QuMasterNoBukti.AsString,'Nota\NotaRBELIGudang.fr3',frxReport1)
else
   MsgTidakBerhakCetakData
end;

procedure TFrMainBeliNota.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainBeliNota.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('NilaiCetak','DBBELI',QuMasterNoBukti.AsString);
end;

procedure TFrMainBeliNota.ToolButton13Click(Sender: TObject);
begin
 {if QuMasterIsOtorisasi1.Value then
  begin
    MessageDlg('No. Pembelian '+QuMasterNoBukti.AsString+' telah diotorisasi oleh user '+QuMasterOtoUser1.AsString+#13+'pada tanggal '+
...
procedure TFrMainBeliNota.ToolButton12Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
       if DataBersyarat(' select ''true'' from dbhutpiut '+
                ' where nofaktur =:0 and tipetrans =''L''',[QuMasterNoBukti.ASstring],DM.Qucari)=true
                then
   begin
     MessageDlg('Sudah Ada Pelunasan Kas Bank'+chr(13), mtInformation,[mbOK],0);
   end
