...
uses
  ...
  frxDMPExport, frxExportBIFF, frxExportCSV;
...
begin
  ...
  begin
    if Application.FindComponent('FrBeli')<>nil then
    begin
       MessageDlg('Modul '+FrBeli.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainBeli.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainBeli.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainBeli.Show1Click(Sender: TObject);
begin
...
{  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  ...
  begin
    if IsTambah then
    begin
      Application.CreateForm(TFrBuatBeli, FrBuatBeli);
      FrBuatBeli.ShowModal;
      if FrBuatBeli.ModalResult=mrOK then
      begin
        ToolButton10.Click;
...
procedure TFrMainBeli.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainBeli.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainBeli.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainBeli.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainBeli.dxPageControl1Change(Sender: TObject);
begin
...
procedure TFrMainBeli.tvPOFocusedRecordChanged(
...
procedure TFrMainBeli.tvMasterFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TFrMainBeli.ToolButton4Click(Sender: TObject);
begin
...
procedure TFrMainBeli.ToolButton12Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
...
procedure TFrMainBeli.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('NilaiCetak','DbBeli',QuMasterNoBukti.AsString);
end;

procedure TFrMainBeli.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainBeli.tvMasterCustomDrawCell(
...
procedure TFrMainBeli.tvMasterCellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TFrMainBeli.ToolButton14Click(Sender: TObject);
begin
