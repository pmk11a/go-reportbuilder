...
var
  FrRPembelian: TFrRPembelian;

implementation

uses FrmMainRBeli, MyProcedure, MyGlobal, MyModul, FrmBrows;

{$R *.dfm}

procedure TFrRPembelian.ClearInputan;
begin
  KodeTipe.Text:='';
  LNamaTipe.Caption:='[ . . . ]';
  KodeSubTipe.Text:='';
...
procedure TFrRPembelian.BtnSimpanClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      FrMainRBeli.SimpanData('I');
      ClearInputan;
      ActiveControl:=KodeTipe;
    end else
    begin
      FrMainRBeli.SimpanData('U');
      ModalResult:=mrOk;
    end;
  end
end;

procedure TFrRPembelian.BtnBatalClick(Sender: TObject);
begin
  Close;
end;

procedure TFrRPembelian.FormShow(Sender: TObject);
begin
...
procedure TFrRPembelian.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=13 then
...
procedure TFrRPembelian.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TFrRPembelian.KodeTipeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrRPembelian.NoBuktiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrRPembelian.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrRPembelian.KodeCustSuppEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPembelian.KodeCustSuppExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrRPembelian.KodeTipeExit(Sender: TObject);
begin
  if mValid Then
  begin
    if (length(KodeTipe.Text)<>0) and
      (DataBersyarat('Select A.KODETIPE,A.Nama '+
              ' from DBTIPETRANS A where A.Tipe=:0 and A.IsJasaBeliJual=1 '+
              ' Order by A.KODETIPE',
...
procedure TFrRPembelian.KodeSubTipeExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrRPembelian.KodeVlsExit(Sender: TObject);
begin
  if mValid Then
  begin
    if (KodeVls.Text<>'') and
...
procedure TFrRPembelian.NDPPDChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrRPembelian.NDPPChange(Sender: TObject);
begin
  if mValid then
  begin
    NNET.Value:=NDPP.AsFloat+NPPN.AsFloat;
...
procedure TFrRPembelian.NPPNChange(Sender: TObject);
begin
  if mValid then
  begin
    NNET.Value:=NDPP.AsFloat+NPPN.AsFloat;
...
procedure TFrRPembelian.NPPNDChange(Sender: TObject);
begin
  if mValid then
  begin
    NNetD.Value:=NDPPD.AsFloat+NPPND.AsFloat;
...
procedure TFrRPembelian.HargaChange(Sender: TObject);
begin
