...
var
  FrMainSPKPesan: TFrMainSPKPesan;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmSPKPesan, FrmSetForm,
  FrmUtama;
{$R *.dfm}

procedure TFrMainSPKPesan.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainSPKPesan.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainSPKPesan.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrSPKPesan')<>nil then
    begin
       MessageDlg('Modul '+FrSPKPesan.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainSPKPesan.FormDestroy(Sender: TObject);
begin
  FrMainSPKPesan:=nil;
end;

procedure TFrMainSPKPesan.FormShow(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainSPKPesan.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainSPKPesan.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.Show1Click(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrSPKPesan) then FrSPKPesan.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrSPKPesan,FrSPKPesan);
        CekOtoritasMenu(IDUser,tag,FrSPKPesan.Istambah,FrSPKPesan.Ishapus,
          FrSPKPesan.Iskoreksi, FrSPKPesan.isCetak, FrSPKPesan.isExcel);
        FrSPKPesan.KdBrg.Text := QuMaster2KodeBrg.AsString;
        FrSPKPesan.NamaBrg.Caption := QuMaster2NamaBrg.AsString;
        FrSPKPesan.Show;
      end;
    end
    else
...
procedure TFrMainSPKPesan.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainSPKPesan.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainSPKPesan.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainSPKPesan.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.tvMasterFocusedRecordChanged(
...
procedure TFrMainSPKPesan.ToolButton4Click(Sender: TObject);
begin
    if Iscetak then
...
procedure TFrMainSPKPesan.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainSPKPesan.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DBSPK',QuMasterNOBUKTI.AsString);
end;

procedure TFrMainSPKPesan.ToolButton13Click(Sender: TObject);
begin
...
procedure TFrMainSPKPesan.ToolButton12Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
