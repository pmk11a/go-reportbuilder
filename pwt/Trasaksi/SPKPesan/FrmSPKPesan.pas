...
type
  TFrSPKPesan = class(TForm)
    ...
    QuPesan: TADOQuery;
    DsPesan: TDataSource;
    QuPesanNoBukti: TStringField;
    QuPesanUrut: TIntegerField;
    ...
    procedure NoPesananEnter(Sender: TObject);
  ...
  private
    ...
    procedure TampilPesananBarang;
    procedure CekSPKKosong;
...
implementation
uses FrmBrows, MyProcedure, MyModul, FrmMainSPKPesan, FrmSPKPesanProses;
{$R *.DFM}

// PROCEDURE DAN FUNCTION PRIBADI

procedure TFrSPKPesan.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrSPKPesan.CekSPKKosong;
begin
  with DM.QuCari do
    begin
      Close;
      SQL.Clear;
      SQL.Add('update DBPesanTunaiDet set NoSPK = '''' where NoSPK = :0');
      Parameters[0].Value := NoBukti.Text;
      try
        ExecSQL;
      except
        ShowMessage('Hapus SPK Pesan Gagal !');
      end;
    end;
end;

procedure TFrSPKPesan.QntPesan;
begin
  with DM.QuCari do
    begin
      Close;
      SQL.Clear;
      SQL.Add('select NoSPK, SUM(QNT) Qnt from DBPesanTunaiDet where NoSPK = :0 group by NoSPK');
      Parameters[0].Value := NoBukti.Text;
      Open;
      QntJ.Value := Fields[1].AsFloat;
    end;
...
procedure TFrSPKPesan.TampilPesanan;
begin
  KodeBrows:=500101;
  ...
  begin
    ...
    begin
      NoPesanan.Text:=Fields[0].AsString;
    end;
  end else
    ActiveControl:=NoPesanan;
end;

procedure TFrSPKPesan.TampilPesananBarang;
begin
  KodeBrows:=500102;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.ShowModal;
  ...
  begin
    with FrBrows.QuBrows do
    begin
      KdBrg.Text:=Fields[0].AsString;
      NamaBrg.Caption:='[..'+Fields[1].AsString+'..]';
    end;
  end else
    ActiveControl:=KdBrg;
end;

procedure TFrSPKPesan.TampilIsiBrgJadi;
begin
  KodeBrows:=2420111;
  Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrSPKPesan.TampilIsiBarang;
begin
  KodeBrows:=24201122;
  Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrSPKPesan.IsiNoBuktiBaru;
begin
...
procedure TFrSPKPesan.TampilData(No_Bukti:String);
begin
   QuBeli.Close;
...
Function TFrSPKPesan.CheckFormMaster:boolean;
begin
   CheckFormMaster:=true;
end;

procedure TFrSPKPesan.UpdatedbBeli;
begin
  if (not QuBeli.IsEmpty) and (mUbahHd=True)and(FrMainSPKPesan.ModalKoreksi) then
  begin
...
procedure TFrSPKPesan.ClearPanelDetail;
begin
   KodeBrg.Text:='';
   LNamaBrg.Caption:='[ . . . ]';
   Qnt.Value:=0;
...
procedure TFrSPKPesan.RefreshAll;
var i:integer;
begin
   mValid:=False;
...
procedure TFrSPKPesan.UpdateDataBeli(Choice:Char;Kodet:String);
begin
...
Procedure TFrSPKPesan.AmBilDataBeli;
begin
...
procedure TFrSPKPesan.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if mExit=true then
  begin
     ...
     if QuBeli.IsEmpty then
       CekSPKKosong;
     FrMainSPKPesan.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
     Action:=cafree;
  end
  else
  begin
...
procedure TFrSPKPesan.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=13 then SendMessage(handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrSPKPesan.FormShow(Sender: TObject);

begin
  mValid:=False;

  xModalKoreksi:=FrMainSPKPesan.ModalKoreksi;
  CSPK.Enabled:=FrMainSPKPesan.ModalKoreksi;
  If FrMainSPKPesan.ModalKoreksi = False then
  begin
    RefreshAll;
    IsiNoBuktiBaru;
    TampilData(NoBukti.Text);
  ...
  Begin
    NOBUKTI.Text := FrMainSPKPesan.QuMaster.FieldByname('Nobukti').AsString;
...
procedure TFrSPKPesan.FormCreate(Sender: TObject);
begin
  TipeTrans := 'SPK';
  Hapus_Daftar_Nomor_User('SPK',IDUser);
end;

procedure TFrSPKPesan.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrSPKPesan.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrSPKPesan.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrSPKPesan.KoreksiBtnClick(Sender: TObject);
begin
  if CheckFormMaster=true then
  begin
     mValid:=false;
...
procedure TFrSPKPesan.wwDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrSPKPesan.HapusBtnClick(Sender: TObject);
begin
...
procedure TFrSPKPesan.wwDBGrid1Enter(Sender: TObject);
begin
  FrSPKPesan.KeyPreview:=false;
...
procedure TFrSPKPesan.wwDBGrid1Exit(Sender: TObject);
begin
  FrSPKPesan.KeyPreview:=true;
end;

procedure TFrSPKPesan.wwDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrSPKPesan.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {if Key=Vk_Return then
  begin
...
procedure TFrSPKPesan.TanggalEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrSPKPesan.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrSPKPesan.KodeSuppEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrSPKPesan.CetakClick(Sender: TObject);
var N1, N2, N3 : string;
    T1, T2 : Real;
    Tot : byte;
    H1  : real;
...
procedure TFrSPKPesan.BitBtn1Click(Sender: TObject);
begin
...
procedure TFrSPKPesan.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
...
procedure TFrSPKPesan.NoDOKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrSPKPesan.FormDestroy(Sender: TObject);
begin
  FrSPKPesan:=nil;
end;

procedure TFrSPKPesan.KodeGdgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrSPKPesan.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrSPKPesan.BPrevClick(Sender: TObject);
begin
        Hapus_Daftar_Nomor('SPK',NoBukti.Text,iduser);
        RefreshAll;
        ActiveControl:=TANGGAL;
...
procedure TFrSPKPesan.BNextClick(Sender: TObject);
begin
        Hapus_Daftar_Nomor('SPK',NoBukti.Text,iduser);
        RefreshAll;
        ActiveControl:=TANGGAL;
...
procedure TFrSPKPesan.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrSPKPesan.SpeedButton8Click(Sender: TObject);
begin
  frxReport1.LoadFromFile(currDir+'Nota\NotaProduksiSPK.fr3');
  frxReport1.DesignReport;
end;

procedure TFrSPKPesan.KdBrgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrSPKPesan.KdBrgExit(Sender: TObject);
begin

  if mValid Then
  begin
    if length(KdBrg.Text)=0 then TampilPesananBarang else
    begin
      if DataBersyarat('select A.KodeBrg, B.NAMABRG from dbPesanTunaiDet A '+
                       'left outer join DBBARANG B on B.KODEBRG = A.KodeBrg '+
                       'where NoSPK = '''' and (A.KodeBrg like :0 or B.NamaBrg like :1) '+
                       'group by A.KodeBrg, B.NAMABRG',['%'+KdBrg.Text+'%','%'+KdBrg.Text+'%'],Dm.Qucari)=True then
      begin
        with DM.QuCari do
        begin
           KdBrg.Text:=fieldbyname('KodeBrg').AsString;
...

{
   if mValid then
     begin
       Application.CreateForm(TFrSPKPesanProses,FrSPKPesanProses);
       FrSPKPesanProses.ShowModal;
       if FrSPKPesanProses.ModalResult=mrOK then
       begin
         TampilData(NoBukti.Text);
       end
       else
...
procedure TFrSPKPesan.TanggalChange(Sender: TObject);
begin
  mUbahHd:=True;
end;

procedure TFrSPKPesan.NoUrutEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrSPKPesan.IsiSatuan;
begin
  LNamaSat.Caption:=IsiSatuanBrg(KodeBrg.Text, mNamaSatuan, mIsiSatuan);
end;

procedure TFrSPKPesan.IsiSatuanJ;
begin
  NmSat.Caption:=IsiSatuanBrg(KdBrg.Text, mNamaSatuan, mIsiSatuan);
  //NosatJ.Value :=1;
end;

procedure TFrSPKPesan.KodeBrgEnter(Sender: TObject);
begin
mValid:=True;
end;

procedure TFrSPKPesan.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrSPKPesan.TampilIsiGudang;
begin

end;

procedure TFrSPKPesan.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrSPKPesan.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DBSPK',nobukti.Text);
end;

procedure TFrSPKPesan.NoUrutExit(Sender: TObject);
...
procedure TFrSPKPesan.NoPesananExit(Sender: TObject);
begin
//   if mValid then
//     TampilPesanan;
   if mValid then
...
procedure TFrSPKPesan.NoPesananEnter(Sender: TObject);
begin
  mValid := True;
end;
