...
type
  TFrMemorialGiroLain = class(TForm)
    pnlGiro: TPanel;
    pnlIsianGiro: TPanel;
    Label1: TLabel;
    Label2: TLabel;
...
var
  FrMemorialGiroLain: TFrMemorialGiroLain;

implementation
Uses MyProcedure,myGlobal,FrmMemorial, ADODB, MyModul, FrmBrows;

{$R *.dfm}
procedure TFrMemorialGiroLain.TampilGiroBelumCair;
begin
...
procedure TFrMemorialGiroLain.TampildataGiro;
...
procedure TFrMemorialGiroLain.FormClose(Sender: TObject;
...
procedure TFrMemorialGiroLain.FormDestroy(Sender: TObject);
begin
  FrMemorialGiroLain := nil;
end;

procedure TFrMemorialGiroLain.btnTambahGiroClick(Sender: TObject);
begin
...
procedure TFrMemorialGiroLain.btnKoreksiGiroClick(Sender: TObject);
begin
...
procedure TFrMemorialGiroLain.btnBatalClick(Sender: TObject);
begin
...
procedure TFrMemorialGiroLain.btnHapusGiroClick(Sender: TObject);
...
procedure TFrMemorialGiroLain.btnOKClick(Sender: TObject);
...
procedure TFrMemorialGiroLain.btnTutupGiroClick(Sender: TObject);
begin
  mExit := True;
...
procedure TFrMemorialGiroLain.FormShow(Sender: TObject);
begin
  FrMemorial.TampilDataGiro(Copy(FrMemorial.THPC.Text,2,1)+'T',FrMemorial.NOBUKTI.Text,FrMemorial.StatusGiro,FrMemorial.mUrut);
  TampildataGiro;
end;

procedure TFrMemorialGiroLain.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMemorialGiroLain.DetBankKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=vk_return then
  begin
    ...
    begin
      ...
      begin
        SetLength(xValue,2);
        TampilIsiData(FrMemorialGiroLain,DetBank,LValas,100404,'Perkiraan','Keterangan');
      end;
    end
    else
    if (copy(FrMemorial.THPC.Text,2,1)='H') and (FrMemorial.StatusGiro='H-') then
...
procedure TFrMemorialGiroLain.KodeVlsEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrMemorialGiroLain.KodeVlsExit(Sender: TObject);
var i : Integer;
begin
  if mValid Then
  begin
    ...
    begin
      ...
      TampilIsiData(FrMemorialGiroLain,Kodevls,LValas,1006,'kodevls','Namavls');
    if ModelGiro='write' then
      for I := LOW(xValue) to High(xValue) do
      begin
