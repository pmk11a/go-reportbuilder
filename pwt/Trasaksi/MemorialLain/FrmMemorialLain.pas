...
type
  TFrMemorialLain = class(TForm)
    ...
    QuTransaksiKodeBrg: TStringField;
    QuTransaksiKodeGdg: TStringField;
    KodeGdg: TEdit;
    Label15: TLabel;
    LNamaBrg: TRxLabel;
    KodeBrg: TEdit;
    Label16: TLabel;
    QuTransaksinamabrg: TStringField;
...
var
  FrMemorialLain: TFrMemorialLain;
...
implementation
uses FrmBrows,MyProcedure,MyGlobal,MyModul, FrmMainMemorialLain,ATTerbilang, FrmMemorialGiroLain,
  FrmMemorialAktivaLain, FrmMemorialHutPiutLain;
{$R *.DFM}


//Cari Koma

procedure TFrMemorialLain.HapusTempHutPiut(pDK: String);
begin
   with DM.QuCari do
   begin
     Close;
...
procedure TFrMemorialLain.TampilIsiNoSPK;
begin
...
procedure TFrMemorialLain.IsiTempHutPiut(pDK, pKode, pNoBukti, pUrut, pValas, pKodeCustSupp, pPerkiraan: String);
begin
...
function TFrMemorialLain.CariJumlahPembayaranHutPiut(pStatusHutPiut, pValas: String): Real;
begin
  with DM.QuCari do
  begin
    Close;
...
procedure TFrMemorialLain.CloseQuerry;
begin
  QuAktiva.Close;
  QuHutPiut.Close;
  QuHutPiutP.Close;
...
procedure TFrMemorialLain.SimpanDataHutPiut;
begin
...
procedure TFrMemorialLain.TampilHutPiut(xkode,xStatus,xPerkiraan,DK:String;Var xKodeCustSupp: TEdit);
...
begin
  ...
  begin
    ...
    begin
      ...
      if xResult then
      begin
        Application.CreateForm(TFrMemorialHutPiutLain, FrMemorialHutPiutLain);
        FrMemorialHutPiutLain.TambahBtn.Enabled := ((xStatus='PT-') or (xStatus='HT-') or (xStatus='UPT-') or (xStatus='UHT-'));
        FrMemorialHutPiutLain.SpeedButton2.Enabled := ((xStatus='PT+') or (xStatus='HT+') or (xStatus='UPT+') or (xStatus='UHT+'));
        FrMemorialHutPiutLain.Caption :='';
        if xkode='HT' then
        begin
          if xStatus='HT-' then
             FrMemorialHutPiutLain.Caption := 'Pelunasan Hutang '+xPerkiraan
          else
          if xStatus='HT+' then
             FrMemorialHutPiutLain.Caption := 'Penambahan Hutang '+xPerkiraan;

        end
        else if xkode='PT' then
        begin
          if xStatus='PT-' then
             FrMemorialHutPiutLain.Caption := 'Pelunasan Piutang '+xPerkiraan
          else
          if xStatus='PT+' then
             FrMemorialHutPiutLain.Caption := 'Penambahan Piutang '+xPerkiraan;
        end
        else if xkode='UHT' then
        begin
          if xStatus='UHT-' then
             FrMemorialHutPiutLain.Caption := 'Pelunasan Uang Muka Hutang '+xPerkiraan
          else
          if xStatus='UHT+' then
             FrMemorialHutPiutLain.Caption := 'Penambahan Uang Muka Hutang '+xPerkiraan;
        end
        else if xkode='UPT' then
        begin
          if xStatus='UPT-' then
             FrMemorialHutPiutLain.Caption := 'Pelunasan Uang Muka Piutang '+xPerkiraan
          else
          if xStatus='UPT+' then
             FrMemorialHutPiutLain.Caption := 'Penambahan Uang Muka Piutang '+xPerkiraan;
        end;
        FrMemorialHutPiutLain.StatusHutPiut := xStatus;
        FrMemorialHutPiutLain.Kurs.Value := Kurs.Value;
        FrMemorialHutPiutLain.Valas.Text := Valas.Text;
        FrMemorialHutPiutLain.Total.Value := Jumlah.Value;
        FrMemorialHutPiutLain.StatusHutPiut := xStatus;
        FrMemorialHutPiutLain.PerkiraanhutPiut := xPerkiraan;
        FrMemorialHutPiutLain.Label1.Caption := xKodeCustSupp.Text;
        FrMemorialHutPiutLain.Tipe := xkode;
        FrMemorialHutPiutLain.DK := DK;
         if MyFindField('dbCustSupp','KodeCustSupp',xKodeCustSupp.Text)=true then
            FrMemorialHutPiutLain.Label2.Caption:='[ '+dm.QuCari.fieldbyname('NamaCustSupp').AsString+' ]';
        FrMemorialHutPiutLain.Label3.Caption:=FrMemorialLain.NOBUKTI.Text;
        IsiTempHutPiut(DK,xkode,NOBUKTI.Text,IntToStr(mUrut),Valas.Text,xKodeCustSupp.Text,xPerkiraan);
        FrMemorialHutPiutLain.ShowModal;
      end;
    end;
  end;
end;

procedure TFrMemorialLain.TampilIsiPerkiraan(xkodebrows:Integer;xPerkiraan:String;Var xEdit : TEdit; Var xLabel:TLabel);
begin
  ...
  begin
    ...
    begin
      ...
      FrMemorialLain.ActiveControl := xedit;
  end
  else
    FrMemorialLain.ActiveControl := xedit;
end;

procedure TFrMemorialLain.TampilDataAktiva(mNoAktivaP,mNoAktivaL,mGroupaktivaP,mGroupAktivaL,mDevisi,mBagian:String);
begin
  ...
  begin
    ...
    with FrMemorialLain.dxAktiva do
    begin
      Close;
      CreateFieldsFromDataSet(QuAktiva);
      LoadFromDataSet(FrMemorialLain.QuAktiva);
      Open;
...
procedure TFrMemorialLain.SimpanDataAktiva(pChoice: String);
begin
...
procedure TFrMemorialLain.TampilAktiva(Var xNoAktiva,xStatus:string;xGroupaktiva:String);
begin
  ...
  begin
    Application.CreateForm(TfrMemorialAktivaLain, FrMemorialAktivaLain);
    with frMemorialAktivaLain,DM.QuCari do
    begin
      IsSimpan := False;
      ...
      StsKasBank :='U';
      frMemorialAktivaLain.ShowModal;
      if frMemorialAktivaLain.ModalResult=mrok then
         xNoAktiva := frMemorialAktivaLain.KodeAktiva.Text;
    end;
  end
  else
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Devisi.Text := FrMemorialLain.Devisi.Text;
            Label32.Caption := FrMemorialLain.LDevisi.Caption;
    ...
    begin
      ...
      begin
        ...
        frMemorialAktivaLain.Devisi.Text := FrMemorialLain.Devisi.Text;
...
procedure TFrMemorialLain.Aktiva(xStatus,mPerkiraan:String; Var xNoAktiva:String);
begin
  if (xStatus='AKV+') or (xStatus='AKM+') or (xStatus='AKV-') or (xStatus='AKM-')then
     TampilAktiva(xNoAktiva,xStatus,mPerkiraan);
end;

function TFrMemorialLain.TransaksiSudahCair(mHP, mNoBukti: String; mUrut: Integer): Boolean;
begin
  with DM.QuCari do
  begin
    Close;
...
procedure TFrMemorialLain.SimpanDataGiro(pChoice: String);
begin
...
procedure TFrMemorialLain.TampilDataGiro(pTHPC, pNoBukti,pStatusGiro: String; pUrut: Integer);
var nRecNoGiro: Integer;
begin
  if not dxGiro.Active then
  begin
    with FrMemorialLain.QuGiro do
    begin
      close;
      sql.Clear;
    ...
    JumlahNilaiGiro := 0;
    with FrMemorialLain.dxGiro do
    begin
      Close;
      CreateFieldsFromDataSet(FrMemorialLain.QuGiro);
      LoadFromDataSet(FrMemorialLain.QuGiro);
      Open;
      DisableControls;
...
procedure TFrMemorialLain.IsiNoBuktiBaru;
begin
  ...
  //Check_Nomor(StrToInt(PeriodBln),StrToInt(PeriodThn),Mode.Text,Nomor,NomorBukti,Tanggal.Date,'1',False);
  Check_Nomor(StrToInt(PeriodBln),StrToInt(PeriodThn),Mode.Text,Nomor,
    NomorBukti,Tanggal.Date,'1',True);
  NoUrut.Text := NomorBukti;
  NOBUKTI.Text := Nomor;
end;

procedure TFrMemorialLain.UpdateTransaksi;
begin
...
Function TFrMemorialLain.KursValas(Tgl1,tgl2:Tdatetime;Valas:String):Real;
begin
    Result :=0;
    with DM.Qucari do
    begin
...
procedure TFrMemorialLain.AmbilDataAktiva(NoPerkiraan,Devisi:String);
begin
   with DM.QuCari do
   begin
      close;
   ...
   with FrMemorialAktivaLain do
   begin
      FrMemorialAktivaLain.Perkiraan.Text:=DM.QuCari.fieldbyname('Perkiraan').AsString;
      FrMemorialAktivaLain.Keterangan.Text:=DM.QuCari.fieldbyname('Keterangan').AsString;
      FrMemorialAktivaLain.Kuantum.Value:=DM.QuCari.fieldbyname('Quantity').AsFloat;
      FrMemorialAktivaLain.Susut.Value:=DM.QuCari.fieldbyname('Persen').AsFloat;
      FrMemorialAktivaLain.TglPengakuan.Date:=DM.QuCari.fieldbyname('Tanggal').AsDateTime;
      FrMemorialAktivaLain.TipeAktiva.ItemIndex:=DM.QuCari.fieldbyname('TipeAktiva').Value;
      FrMemorialAktivaLain.AkSusut.Text:=DM.QuCari.fieldbyname('Akumulasi').AsString;
      FrMemorialAktivaLain.BiayaSusut.Text:=DM.QuCari.fieldbyname('biaya').AsString;
      FrMemorialAktivaLain.BiayaSusut2.Text:=DM.QuCari.fieldbyname('biaya2').AsString;
      FrMemorialAktivaLain.PersenSusut1.Value:=DM.QuCari.fieldbyname('Persenbiaya1').AsFloat;
      FrMemorialAktivaLain.PersenSusut2.Value:=DM.QuCari.fieldbyname('Persenbiaya2').AsFloat;
      FrMemorialAktivaLain.Perkiraan.Enabled:=false;
      FrMemorialAktivaLain.Keterangan.Enabled:=false;
      FrMemorialAktivaLain.Kuantum.Enabled:=false;
      FrMemorialAktivaLain.Susut.Enabled:=false;
      FrMemorialAktivaLain.TglPengakuan.Enabled:=false;
      FrMemorialAktivaLain.TipeAktiva.Enabled:=false;
      FrMemorialAktivaLain.AkSusut.Enabled:=false;
      FrMemorialAktivaLain.BiayaSusut.Enabled:=false;
      FrMemorialAktivaLain.ActiveControl:=BitBtn1;
   end;

end;

procedure TFrMemorialLain.SimpanData(Choice: String);
begin
  ...
  begin
     ...
     begin
        ...
        Parameters[35].value:= KodeBrg.Text;
        Parameters[36].value:= KodeGdg.Text;
...
function TFrMemorialLain.CekLawanDiPosting(mLawan,DK : String;var Kode:String):Boolean;
begin
   Result := False;
   with DM.QuCari do
   begin
...
procedure TFrMemorialLain.RefreshAll;
var I : integer;
begin
...
procedure TFrMemorialLain.ClearDetailPanel;
begin
  Lawan.Text:='';
  Perkiraan.Text:='';
  Jumlah.Value:=0;
...
procedure TFrMemorialLain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if FrMainMemorialLain.Acetak then  Action:=cafree
  else
  if mExit=true then
  begin
    FrMainMemorialLain.mPosisi:=mBukti;
    FrMainMemorialLain.xStatus := 'M';
    Hapus_Daftar_Nomor_User(Copy(Mode.Text,1,2),IDUser);
    BM := FrMainMemorialLain.QuMaster.GetBookmark;
    FrMainMemorialLain.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
    if FrMainMemorialLain.QuMaster.BookmarkValid(BM) then
    begin
      try
       FrMainMemorialLain.QuMaster.GotoBookmark(BM);
      finally
       FrMainMemorialLain.QuMaster.FreeBookmark(BM);
      end
    end else
    begin
      FrMainMemorialLain.QuMaster.FreeBookmark(BM);
      //FrMainMemorial.QuMaster.First;
      FrMainMemorialLain.QuMaster.Last
    end;
    Action:=cafree;
  end
  else
...
procedure TFrMemorialLain.TampilData(No_bukti:string);
begin
   with QuTransaksi do
   begin
      Close;
...
procedure TFrMemorialLain.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key=13 then SendMessage(Handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrMemorialLain.FormShow(Sender: TObject);
begin
  xModalKoreksi := FrMainMemorialLain.ModalKoreksi;
  mValid:=False;
  IsTambah:=FrMainMemorialLain.IsTambah;
  IsKoreksi:=FrMainMemorialLain.IsKoreksi;
  IsHapus:=FrMainMemorialLain.IsHapus;
  IsCetak:=FrMainMemorialLain.IsCetak;
  ...
  Begin
    NOBUKTI.Text:=FrMainMemorialLain.QuMasterNoBukti.AsString;
...
procedure TFrMemorialLain.PerkiraanEnter(Sender: TObject);
begin
  mValid:=true;
...
procedure TFrMemorialLain.TANGGALKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMemorialLain.ModeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=vk_escape then
  begin
...
procedure TFrMemorialLain.wwDBGrid1Exit(Sender: TObject);
begin
FrMemorialLain.KeyPreview:=true;
...
procedure TFrMemorialLain.TambahBtnClick(Sender: TObject);
begin
  if IsTambah then
  begin
    Model:='write';
...
procedure TFrMemorialLain.KoreksiBtnClick(Sender: TObject);
begin
 if IsKoreksi then
 begin
   mValid:=false;
   ...
   begin
     ...
     xStatusAktivaP := StatusAktivaP;
     Kodebrg.Text:=QuTransaksiKodeBrg.AsString;
     Kodegdg.Text:=QuTransaksiKodeGdg.AsString;
     LNamaBrg.Caption :=QuTransaksinamabrg.AsString;
     ...
     begin
        if StatusGiro<>'' then
        begin
          Application.CreateForm(TFrMemorialGiroLain,FrMemorialGiroLain);
          FrMemorialGiroLain.Showmodal;
        end;
     end
     else
     begin
...
procedure TFrMemorialLain.HapusBtnClick(Sender: TObject);
...
procedure TFrMemorialLain.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
  Panel1.Enabled:=true;
  Panel2.Enabled:=true;
...
procedure TFrMemorialLain.LawanKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrMemorialLain.KeteranganKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=VK_Escape then
   begin
...
procedure TFrMemorialLain.TANGGALExit(Sender: TObject);
begin
if mValid then
begin
...
procedure TFrMemorialLain.TANGGALEnter(Sender: TObject);
begin
mValid:=true;
end;

procedure TFrMemorialLain.KeteranganEnter(Sender: TObject);
begin
KeyPreview:=false;
end;

procedure TFrMemorialLain.KeteranganExit(Sender: TObject);
begin
KeyPreview:=true;
end;

procedure TFrMemorialLain.ValasExit(Sender: TObject);
var i : Integer;
begin
  if (mValid) Then
  begin
    ...
    begin
      TampilIsiData(FrMemorialLain,Valas,LValas,1006,'kodevls','Namavls');
...
procedure TFrMemorialLain.ValasEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrMemorialLain.BitBtn1Click(Sender: TObject);
...
procedure TFrMemorialLain.ModeEnter(Sender: TObject);
begin
 mvalid:=true;
 mExit:=true;
end;

procedure TFrMemorialLain.TerimaChange(Sender: TObject);
begin
   KETERANGAN.Text:=Terima.Text;
end;

procedure TFrMemorialLain.CetakClick(Sender: TObject);
var N1, N2, N3 : string;
    T2 : integer;
    Tot : byte;
    TempCode : String;
...
procedure TFrMemorialLain.BankKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
   begin
...
procedure TFrMemorialLain.DevisiExit(Sender: TObject);
begin
  if mValid then
  begin
    SetLength(xValue,2);
    ...
    begin
      ...
      TampilIsiData(FrMemorialLain,Devisi ,LDevisi,1004,'Devisi','NamaDevisi');
  end;
end;

procedure TFrMemorialLain.FormDestroy(Sender: TObject);
begin
 FrMemorialLain:=nil;
end;

procedure TFrMemorialLain.FormCreate(Sender: TObject);
begin
...
procedure TFrMemorialLain.DevisiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrMemorialLain.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMemorialLain.NoUrutEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrMemorialLain.NoUrutChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrMemorialLain.KodeBagEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrMemorialLain.KodeBagExit(Sender: TObject);
begin
  {SetLength(xValue,2);
  if mValid then
  begin
...
procedure TFrMemorialLain.tvMasterKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrMemorialLain.ModeChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrMemorialLain.SpeedButton2Click(Sender: TObject);
var N1, N2, N3 : string;
    T1, T2 : integer;
    Tot : byte;
    TempCode : String;
...
procedure TFrMemorialLain.THPCChange(Sender: TObject);
begin
  ...
  begin
    ...
    if StatusGiro='P+' then
       FrMemorialGiroLain.Caption := 'Terima Giro'
    else
    if StatusGiro='P-' then
       FrMemorialGiroLain.Caption := 'Pencairan Terima Giro'
    else
    if StatusGiro='H+' then
       FrMemorialGiroLain.Caption := 'Buka Giro'
    else
    if StatusGiro='H-' then
       FrMemorialGiroLain.Caption := 'Pencairan Buka Giro';
    FrMemorialGiroLain.Showmodal;
  end;
end;

procedure TFrMemorialLain.PerkiraanKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrMemorialLain.LawanExit(Sender: TObject);
begin
  if mValid then
  begin
    if DataBersyarat('select * from dbPerkiraan where Perkiraan=:0 and Tipe=1 and Perkiraan<>:1 ',[Lawan.Text,Perkiraan.Text],DM.Qucari) then
...
procedure TFrMemorialLain.PerkiraanExit(Sender: TObject);
begin
    if mValid then
    begin
...
procedure TFrMemorialLain.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {if Key=Vk_Return then
  begin
...
procedure TFrMemorialLain.NoUrutExit(Sender: TObject);
begin
  if mValid then
  begin
    If Length(NoUrut.Text)<5 then
...
procedure TFrMemorialLain.KodeBrgEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrMemorialLain.KodeGdgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrMemorialLain.TampilIsiBarang;
begin
  KodeBrows:=3001101;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.IsiData:=KodeBrg.Text;
  //FrBrows.IsiData1:=kodebrg.Text;
  FrBrows.ShowModal;
  if FrBrows.ModalResult=mrOk then
  begin
    with FrBrows.QuBrows do
...
procedure TFrMemorialLain.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Return then
  begin
      ...
      begin
        if DataBersyarat('Select KodeBrg, NamaBrg, Sat1 from dbBarang where KodeBrg=:0 ', [KodeBrg.Text], DM.QuCari)=True then
...
procedure TFrMemorialLain.TampilIsiGudang;
begin
  KodeBrows:=11002;
  Application.CreateForm(TFrBrows, FrBrows);
