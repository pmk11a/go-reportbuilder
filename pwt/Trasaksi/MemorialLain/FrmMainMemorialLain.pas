...
type
  ...
  TFrMainMemorialLain = class(TForm)
    ToolBar1: TToolBar;
...
var
  FrMainMemorialLain: TFrMainMemorialLain;
  S: array[0..255] of char;
implementation
uses myModul,Myprocedure,MyGlobal, FrmBrows, FrmMemorialLain,  ATTerbilang,
  frmLogin;
{$R *.dfm}

procedure TFrMainMemorialLain.GetData(bulan,tahun:integer);
begin
  QuMaster.Close;
...
procedure TFrMainMemorialLain.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainMemorialLain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrMainMemorialLain.FormDestroy(Sender: TObject);
begin
  FrMainMemorialLain:=nil;
end;

procedure TFrMainMemorialLain.FormShow(Sender: TObject);
begin
  xStatus := 'S';
  ToolButton10.Click;
  begin
...
procedure TFrMainMemorialLain.ToolButton10Click(Sender: TObject);
begin
   GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainMemorialLain.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainMemorialLain.ExportExcel1Click(Sender: TObject);
begin
  if (IsExcel) then
  begin
      ...
      ExportGrid4ToExcel(FrMainMemorialLain.Caption,cxGrid1,True,True,True);
  end else
  begin
...
procedure TFrMainMemorialLain.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainMemorialLain.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainMemorialLain.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainMemorialLain.Show1Click(Sender: TObject);
begin
...
procedure TFrMainMemorialLain.ToolButton1Click(Sender: TObject);
begin
   ...
   begin
     ...
     begin
      if IsTambah then
      begin
         if Assigned(FrMemorialLain) then
            frMemorialLain.Show
         else
         begin
           ModalKoreksi:=FAlse;
           xStatus := 'H';
           Application.CreateForm(TFrMemorialLain,FrMemorialLain);
...
procedure TFrMainMemorialLain.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainMemorialLain.ToolButton2Click(Sender: TObject);
begin
           ...
           begin
             ...
             begin
              ...
              begin
                ...
                //CekOtoritasMenu(IDUser,tag,FrMemorial.Istambah,FrMemorial.Ishapus,
                //         FrMemorial.Iskoreksi, FrMemorial.isCetak, FrMemorial.isExcel);
                FrMemorialLain.xmodalkoreksi := ModalKoreksi;
                FrMemorialLain.Show;
              end;
              {With QuView do
              begin
...
procedure TFrMainMemorialLain.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainMemorialLain.ToolButton4Click(Sender: TObject);
...
begin
  ...
  begin
    Application.CreateForm(TFrMemorialLain, FrMemorialLain);
    FrMemorialLain.mBukti:=FrMainMemorialLain.QuMasterNoBukti.AsString;
    FrMemorialLain.Mode.Text:=Copy(FrMemorialLain.mBukti,10,2);
    FrMemorialLain.TampilData(FrMainMemorialLain.QuMasterNoBukti.AsString);
  ...
  myTerbilang := UpperCase(myTerbilang);
    ...
    FrMemorialLain.frxReport1.LoadFromFile(ExtractFilePath(Application.ExeName)+'Nota\BuktiMemorial.fr3');
    FrMemorialLain.frxReport1.ShowReport;
    FrMemorialLain.Destroy;
  end
  else
  begin
...
procedure TFrMainMemorialLain.FormActivate(Sender: TObject);
begin
   QuMaster.Locate('Nobukti',mPosisi,[LOC, LOP]);
end;

procedure TFrMainMemorialLain.tvMasterFocusedRecordChanged(
...
procedure TFrMainMemorialLain.ToolButton3Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
...
procedure TFrMainMemorialLain.tvMaster2FocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TFrMainMemorialLain.ToolButton11Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
