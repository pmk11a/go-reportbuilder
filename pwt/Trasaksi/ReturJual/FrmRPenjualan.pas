...
var
  FrRPenjualan: TFrRPenjualan;

implementation

uses FrmMainRJual, MyProcedure, MyGlobal, MyModul, FrmBrows;

{$R *.dfm}

procedure TFrRPenjualan.ClearInputan;
begin
  KodeTipe.Text:='';
  LNamaTipe.Caption:='[ . . . ]';
  KodeSubTipe.Text:='';
...
procedure TFrRPenjualan.BtnSimpanClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      FrMainRJual.SimpanData('I');
      ClearInputan;
      ActiveControl:=KodeTipe;
    end else
    begin
      FrMainRJual.SimpanData('U');
      ModalResult:=mrOk;
    end;
  end
end;

procedure TFrRPenjualan.BtnBatalClick(Sender: TObject);
begin
  Close;
end;

procedure TFrRPenjualan.FormShow(Sender: TObject);
begin
...
procedure TFrRPenjualan.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=13 then
...
procedure TFrRPenjualan.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TFrRPenjualan.KodeTipeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrRPenjualan.NoBuktiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrRPenjualan.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrRPenjualan.KodeCustSuppEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrRPenjualan.KodeCustSuppExit(Sender: TObject);
begin
  if mValid Then
  begin
      if (length(KodeCustSupp.Text)<>0) and
         (DataBersyarat(' select X.KodeCustSupp, Y.NamaCustSupp, '+
                        ...
                        '   where B.Perkiraan=A.Perkiraan and B.Kode='+QuotedStr('PT')+
...
procedure TFrRPenjualan.KodeTipeExit(Sender: TObject);
begin
  if mValid Then
  begin
    if (length(KodeTipe.Text)<>0) and
      (DataBersyarat('Select A.KODETIPE,A.Nama '+
              ' from DBTIPETRANS A where A.Tipe=:0 and A.IsJasaBeliJual=2 '+
              ' Order by A.KODETIPE',
...
procedure TFrRPenjualan.KodeSubTipeExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrRPenjualan.KodeVlsExit(Sender: TObject);
begin
  if mValid Then
  begin
    if (KodeVls.Text<>'') and
...
procedure TFrRPenjualan.NDPPDChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrRPenjualan.NDPPChange(Sender: TObject);
begin
  if mValid then
  begin
    NNET.Value:=NDPP.AsFloat+NPPN.AsFloat;
...
procedure TFrRPenjualan.NPPNChange(Sender: TObject);
begin
  if mValid then
  begin
    NNET.Value:=NDPP.AsFloat+NPPN.AsFloat;
...
procedure TFrRPenjualan.NPPNDChange(Sender: TObject);
begin
  if mValid then
  begin
    NNetD.Value:=NDPPD.AsFloat+NPPND.AsFloat;
...
procedure TFrRPenjualan.HargaChange(Sender: TObject);
begin
