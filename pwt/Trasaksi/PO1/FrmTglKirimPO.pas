...
var
  FrTglKirimPO: TFrTglKirimPO;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmPO, DB, ADODB;

{$R *.dfm}

procedure TFrTglKirimPO.BtnBatalClick(Sender: TObject);
begin
  Close;
end;

procedure TFrTglKirimPO.BtnSimpanClick(Sender: TObject);
var iForDo: Integer;
    TotalKirim : Real;
begin
  with DM.QuCari do
...
procedure TFrTglKirimPO.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=13 then
...
procedure TFrTglKirimPO.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TFrTglKirimPO.FormShow(Sender: TObject);
begin
  LabelKodeBrg.Caption:='Kode Barang : '+FrPO.QukirimKodeBrg.AsString;
  LabelNamaBrg.Caption:='Nama Barang : '+FrPO.QukirimNamaBrg.AsString;
  LabelSatuan.Caption:='Satuan : '+FrPO.QukirimSatuan.AsString;
  Tanggal1.Date:=FrPO.QukirimTanggal1.AsDateTime;
  Qnt1.Value:=FrPO.QukirimQnt1.AsCurrency;
  Tanggal2.Date:=FrPO.QukirimTanggal2.AsDateTime;
  Qnt2.Value:=FrPO.QukirimQnt2.AsCurrency;
  Tanggal3.Date:=FrPO.QukirimTanggal3.AsDateTime;
  Qnt3.Value:=FrPO.QukirimQnt3.AsCurrency;
  Tanggal4.Date:=FrPO.QukirimTanggal4.AsDateTime;
  Qnt4.Value:=FrPO.QukirimQnt4.AsCurrency;
  Tanggal5.Date:=FrPO.QukirimTanggal5.AsDateTime;
  Qnt5.Value:=FrPO.QukirimQnt5.AsCurrency;
  if Qnt1.Value=0 then
  begin
    Tanggal1.Date:=FrPO.Tanggal.Date;
    Qnt1.Value:=FrPO.QukirimQnt.AsFloat;
  end;
...
procedure TFrTglKirimPO.Qnt1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
