...
type
  TFrMemorialHutPiut = class(TForm)
    Panel1: TPanel;
    Panel3: TPanel;
    Shape4: TShape;
    Label10: TLabel;
...
var
  FrMemorialHutPiut: TFrMemorialHutPiut;
...
procedure TFrMemorialHutPiut.SaldoHutPiut;
...
procedure TFrMemorialHutPiut.TampilDataHutPiut(xNobukti,xPerkiraan,xkodeCustSupp,DK:String);
begin
{
...
procedure TFrMemorialHutPiut.SimpanData(Choice:String);
...
Function TFrMemorialHutPiut.JumlahYgDibayar(Nota:String):Real;
begin
{   Result :=0;
   with DM.QuCari do
   begin
...
Function TFrMemorialHutPiut.CekPelunasanMax(Nota:String;JmlPelunasan:Real;Var JmlHut,SisaPiut:Real):Boolean;
var JmlHutang : Real;
begin
   with DM.QuCari do
   begin
      Close;
      SQL.Clear;
      if FrMemorialHutPiut.Valas.Text='IDR' then
      begin
...
procedure TFrMemorialHutPiut.ClearDetailPanel;
var i:integer;
begin
  NoBukti.Text := '';
...
procedure TFrMemorialHutPiut.TambahBtnClick(Sender: TObject);
...
procedure TFrMemorialHutPiut.TampilData(KodeCust:String);
begin
  if mDetail=0 then
  begin
     mSaldo:=0;
...
procedure TFrMemorialHutPiut.FormShow(Sender: TObject);
begin
...
procedure TFrMemorialHutPiut.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
  Panel1.Enabled:=true;
  Panel2.Enabled:=true;
...
procedure TFrMemorialHutPiut.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=13 then SendMessage(handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrMemorialHutPiut.FormClose(Sender: TObject;
...
procedure TFrMemorialHutPiut.HapusBtnClick(Sender: TObject);
begin
...
procedure TFrMemorialHutPiut.SpeedButton2Click(Sender: TObject);
begin
  if Sisa.Value<>0 then
  begin
...
procedure TFrMemorialHutPiut.NoBuktiKeyDown(Sender: TObject;
...
procedure TFrMemorialHutPiut.NoBuktiEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrMemorialHutPiut.dxDBhutPiutCustomDrawCell(Sender: TObject;
...
procedure TFrMemorialHutPiut.btnTutupGiroClick(Sender: TObject);
begin
  mExit := True;
  {if DK='D' then
  begin
...
procedure TFrMemorialHutPiut.TotalChange(Sender: TObject);
begin
  Sisa.Value:=Total.Value-Dibayar.Value;
end;

procedure TFrMemorialHutPiut.BitBtn1Click(Sender: TObject);
var JmlHutPiut :Real;
begin
...
procedure TFrMemorialHutPiut.JumlahKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=vk_Escape then
  begin
...
procedure TFrMemorialHutPiut.dxDBhutPiutChangeNode(Sender: TObject; OldNode,
  Node: TdxTreeListNode);
begin
