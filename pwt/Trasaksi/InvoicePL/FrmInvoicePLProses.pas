...
type
  TFrInvoicePLProses = class(TForm)
    Label1: TLabel;
    BtnSimpan: TBitBtn;
    BtnBatal: TBitBtn;
    dxDBGrid1: TdxDBGrid;
  ...
  public
    { Public declarations }
    mNOSO_SJProses,mNOSOKodecust_SJProses: String;
...
var
  FrInvoicePLProses: TFrInvoicePLProses;
...
procedure TFrInvoicePLProses.Simpandata(Choice:Char);
var Kodebrg : string;
begin
  with DM.QuCari do
  begin
    ...
    SQL.Add('       A.Catatan, G.NoBukti Noso, G.Tanggal TglSO, F.NoBukti NoSPP, F.Tanggal TglSPP,G.Discp2,G.Discp3,Discp4,Discp5,ppnp');
    ...
    SQL.Add('                             case when iscetakkitir =1 then y.HARGA else 0 end HARGA, y.DISCP1, y.DISCRP1, y.DISCTOT, x.KODEVLS, x.KURS, x.TIPEBAYAR, x.PPN, x.HARI, x.Disc,Y.Discp2,Discp3,Discp4,Discp5,ppnp');
    SQL.Add('                      From dbSO x');
 ...
 { with dm.QuCari2 do
  ...
  begin
    kodebrg := DM.QuCari.FieldByname('KodeBrg').Value;
    With Sp_Beli do
    begin
      close;
      Prepared;
      ...
      Parameters[46].Value := '';
      Parameters[47].Value := '';
      Parameters[48].Value := 0;//DM.QuCari.FieldByname('ppnp').Value;//0;ppnp
      ...
      Parameters[64].Value := DM.QuCari.FieldByname('ppnp').Value;//0;ppnp

      try
        ExecProc;
        LoggingData(IDUser,'I','PNJ', nobukti,
          'Kode Brg = '+kodebrg);
...
procedure TFrInvoicePLProses.IsiNoBuktiBaru;
var YY,MM,DD : Word;
begin
...
procedure TFrInvoicePLProses.FormShow(Sender: TObject);
var xKeyUrut: Integer;
begin
  QuSO.Close;
  //QuSO.SQL.Strings[2]:='select @NoBukti='+QuotedStr(mNOSO_SJProses)+', @Bulan='+PeriodBln+', @Tahun='+PeriodThn;
  QuSO.SQL.Strings[2]:='select @NoBukti='+QuotedStr(mNOSOKodecust_SJProses)+', @Bulan='+PeriodBln+', @Tahun='+PeriodThn;
  QuSO.Open;
  xKeyUrut:=0;
...
procedure TFrInvoicePLProses.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TFrInvoicePLProses.BtnSimpanClick(Sender: TObject);
...
procedure TFrInvoicePLProses.BtnBatalClick(Sender: TObject);
begin
  Close;
end;

procedure TFrInvoicePLProses.FormCreate(Sender: TObject);
begin
  TipeTrans := 'INVC';
end;
