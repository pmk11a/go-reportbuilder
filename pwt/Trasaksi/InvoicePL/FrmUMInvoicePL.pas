...
uses
  ...
  cxGridCustomView, cxGridLevel, cxGrid, frxRich;

type
  TFrUMInvoicePL = class(TForm)
    Panel1: TPanel;
    Panel3: TPanel;
    QuBeli: TADOQuery;
...
implementation
uses FrmBrows, MyProcedure, MyGlobal, MyModul, FrmMainUMInvoicePL, FrmUMInvoicePLProses;
{$R *.DFM}

Procedure TFrUMInvoicePL.HitungRincian;
begin
  DiskonP.Value:=QuBeliDisc.AsFloat;
  DiskonRp.Value:=QuBeliDiskon.AsFloat;
  DPP.Value:=QuBeliTotalDPP.AsFloat;
  //DPP.Value:=dp.Value;
//  pajak.Value := (dp.Value*0.1);
  Pajak.Value:=QuBeliTotalPPn.AsFloat;
  GrandTotal.Value:=QuBeliTotalNetto.AsFloat;
end;

function TFrUMInvoicePL.CekSudahAdaProsesSelanjutnya: Boolean;
...
procedure TFrUMInvoicePL.TampilanQntSatuan(pNilai: Integer);
begin
  if pNilai=0 then
  begin
    Qnt.Visible:=True;
...
procedure TFrUMInvoicePL.TampilIsiBarang;
begin
  KodeBrows:=4006;
...
procedure TFrUMInvoicePL.TampilIsiSPP;
begin
     KodeBrows:=40421;
     Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrUMInvoicePL.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrUMInvoicePL.UpdatedbBeli;
begin
  ...
  begin
    ...
    begin
       ...
       Sql.add('     FootNote=:8, IssuingBank=:9, PPn=:10, Valas=:11, Kurs=:12, Disc=:13,DP=:14 ');
       ...
       Parameters[14].Value := DP.AsFloat;
       try
         ExecSQL;
       except
       end;
...
procedure TFrUMInvoicePL.TampilData(No_Bukti:String);
begin
  QuBeli.Close;
...
procedure TFrUMInvoicePL.IsiNoBuktiBaru;
begin
  NoUrut.Enabled:=True;
...
Function TFrUMInvoicePL.CheckFormMaster:boolean;
begin
   CheckFormMaster:=true;
end;

procedure TFrUMInvoicePL.ClearPanelDetail;
begin
   KodeBrg.Text:='';
   LNamaBrg.Caption:='[ . . . ]';
   LSatuan2.Caption:='[ . . . ]';
...
procedure TFrUMInvoicePL.RefreshAll;
var i:integer;
begin
   mValid:=False;
   mUbahHd:=False; mUbahHdDet:=False;
...
procedure TFrUMInvoicePL.UpdateDataBeli(Choice:Char;Kodet:String);
begin
  ...
  begin
    ...
    begin
      ...
      Parameters[34].Value:= QuBeliHARGA.Value;
      ...
      Parameters[40].Value:= (isLokal.ItemIndex=1);
...
procedure TFrUMInvoicePL.UpdateDataBeli1(Choice:Char;Kodet:String);
begin
...
Procedure TFrUMInvoicePL.AmBilDataBeli;
begin
...
procedure TFrUMInvoicePL.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if mExit=true then
  begin
     FrMainUMInvoicePL.ToolButton10.Click;
...
procedure TFrUMInvoicePL.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=13 then SendMessage(Handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrUMInvoicePL.FormShow(Sender: TObject);
begin
  RefreshAll;
  xModalKoreksi:=FrMainUMInvoicePL.ModalKoreksi;
...
procedure TFrUMInvoicePL.FormCreate(Sender: TObject);
begin
  TipeTrans:='INVC';
  Hapus_Daftar_Nomor_User(TipeTrans,IDUser);
  Qnt2.Top:=Qnt.Top;
...
procedure TFrUMInvoicePL.KoreksiBtnClick(Sender: TObject);
begin
  mValid:=false;
  Model:='koreksi';
...
procedure TFrUMInvoicePL.wwDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  ...
  begin
    ...
    FrMainUMInvoicePL.GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
  end
...
procedure TFrUMInvoicePL.HapusBtnClick(Sender: TObject);
begin
...
procedure TFrUMInvoicePL.wwDBGrid1Enter(Sender: TObject);
begin
  FrUMInvoicePL.KeyPreview:=False;
end;

procedure TFrUMInvoicePL.wwDBGrid1Exit(Sender: TObject);
begin
  FrUMInvoicePL.KeyPreview:=True;
end;

procedure TFrUMInvoicePL.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
...
procedure TFrUMInvoicePL.BitBtn1Click(Sender: TObject);
begin
...
procedure TFrUMInvoicePL.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
...
procedure TFrUMInvoicePL.FormDestroy(Sender: TObject);
begin
  FrUMInvoicePL:=nil;
end;

procedure TFrUMInvoicePL.dxDBGrid1CustomDrawCell(Sender: TObject; ACanvas: TCanvas;
  ARect: TRect; ANode: TdxTreeListNode; AColumn: TdxTreeListColumn;
  ASelected, AFocused, ANewItemRow: Boolean; var AText: String;
  var AColor: TColor; AFont: TFont; var AAlignment: TAlignment;
  var ADone: Boolean);
...
procedure TFrUMInvoicePL.NoUrutChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrUMInvoicePL.KodeBrgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrUMInvoicePL.NoUrutEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrUMInvoicePL.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrUMInvoicePL.TambahBtnClick(Sender: TObject);
begin
  model:='write';
  Application.CreateForm(TFrUMInvoicePLProses,FrUMInvoicePLProses);
  FrUMInvoicePLProses.mNOSO_SJProses:=gTempNoBuktiSO;
  FrUMInvoicePLProses.NoBukti := NoBukti.Text;
  FrUMInvoicePLProses.ShowModal;
  ActiveControl := cxGrid1;
  TampilData(NoBukti.Text);
end;

procedure TFrUMInvoicePL.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
    SpeedButton1.Click;
...
procedure TFrUMInvoicePL.Qnt2Change(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrUMInvoicePL.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Return then
  begin
...
procedure TFrUMInvoicePL.TanggalChange(Sender: TObject);
begin
  if mValid then
  begin
    mUbahHd:=True;
...
procedure TFrUMInvoicePL.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
     if CekPeriode(IdUser,Tanggal.Date)=false then
...
procedure TFrUMInvoicePL.NosatEnter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrUMInvoicePL.NosatChange(Sender: TObject);
var xSatuan : string;
    xHarga : Real;
begin
  if mValid then
...
procedure TFrUMInvoicePL.ConsigneeEnter(Sender: TObject);
begin
  mValid := True;
  FrUMInvoicePL.KeyPreview := False;
end;

procedure TFrUMInvoicePL.ConsigneeExit(Sender: TObject);
begin
  if mValid then
  begin
    mUbahHd := True;
...
procedure TFrUMInvoicePL.CetakClick(Sender: TObject);
var xNet : Real;
begin
...
procedure TFrUMInvoicePL.SpeedButton8Click(Sender: TObject);
var xNet : Real;
begin
{
  frxDBCetak.DataSet.Close;
...
procedure TFrUMInvoicePL.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrUMInvoicePL.KetBiayaEnter(Sender: TObject);
begin
  KeyPreview := false;
end;

procedure TFrUMInvoicePL.KetBiayaExit(Sender: TObject);
begin
  KeyPreview := True;
end;

procedure TFrUMInvoicePL.KetBiayaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
    SpeedButton1.Click;
...
procedure TFrUMInvoicePL.KodeVls1Enter(Sender: TObject);
begin
  mValid := True;
end;

procedure TFrUMInvoicePL.KodeVls1Exit(Sender: TObject);
var i : Integer;
begin
  if mValid Then
  begin
    ...
    begin
      ...
      begin
        TampilIsiData(FrUMInvoicePL,kodevls1,lsatuan2,1006,'kodevls','Namavls');
...
procedure TFrUMInvoicePL.FootNoteExit(Sender: TObject);
begin
  if mValid then
  begin
    mUbahHd := True;
