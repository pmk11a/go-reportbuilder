...
begin
  ...
  begin
    if Application.FindComponent('Frfinishing')<>nil then
    begin
       MessageDlg('Modul '+FrFinishing.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
begin
  ...
  begin
    if IsTambah then
    begin
      if Assigned(FrFinishing) then FrFinishing.Show else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrFinishing,FrFinishing);
        CekOtoritasMenu(IDUser,tag,FrFinishing.Istambah,FrFinishing.Ishapus,
          FrFinishing.Iskoreksi, FrFinishing.isCetak, FrFinishing.isExcel);
...
begin
if Iscetak then
    MainCetak1(frxDBDataset1,frxDataPerusahaan,DM.QuCari,Dm.QuCari2,'Cetakfinishing','VwCetakFinishing',QuMasterNoBukti.AsString,'Nota\Notafinishing.fr3',frxReport1)
else
...
procedure TFrMainFinishing.ToolButton14Click(Sender: TObject);
begin
