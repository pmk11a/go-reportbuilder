...
var
  FrMainRPBSampel: TFrMainRPBSampel;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmRPBSampel;
{$R *.dfm}

procedure TFrMainRPBSampel.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainRPBSampel.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainRPBSampel.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrRPBSampel')<>nil then
    begin
       MessageDlg('Modul '+FrRPBSampel.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainRPBSampel.FormDestroy(Sender: TObject);
begin
  FrMainRPBSampel:=nil;
end;

procedure TFrMainRPBSampel.FormShow(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainRPBSampel.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainRPBSampel.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.Show1Click(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrRPBSampel) then FrRPBSampel.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrRPBSampel,FrRPBSampel);
        CekOtoritasMenu(IDUser,tag,FrRPBSampel.Istambah,FrRPBSampel.Ishapus,
          FrRPBSampel.Iskoreksi, FrRPBSampel.isCetak, FrRPBSampel.isExcel);
        FrRPBSampel.Show;
      end;
    end
    else
...
procedure TFrMainRPBSampel.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainRPBSampel.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainRPBSampel.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainRPBSampel.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainRPBSampel.tvMasterFocusedRecordChanged(
...
procedure TFrMainRPBSampel.tvPOCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrMainRPBSampel.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainRPBSampel.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbRpenyerahan',QuMasterNoBukti.AsString);
end;

procedure TFrMainRPBSampel.ToolButton4Click(Sender: TObject);
begin
if Iscetak then
    MainCetak(frxDBDataset1,frxDataPerusahaan,DM.QuCari,Dm.QuCari2,'CetakRPemakaianBahan','Vwperusahaan',QuMasterNoBukti.AsString,'Nota\NotareturPemakaianBahan.fr3',frxReport1)
else
   MsgTidakBerhakCetakData
end;

procedure TFrMainRPBSampel.ToolButton13Click(Sender: TObject);
begin
 {if QuMasterIsOtorisasi1.Value then
  ...
  begin
    ...
    begin
      Otorisasi('DbRpenyerahanBhn',MenuRPB,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainRPBSampel.ToolButton12Click(Sender: TObject);
begin
...
BM := QuMaster.GetBookmark;
  ...
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DbRpenyerahanBhn',MenuRPB,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
