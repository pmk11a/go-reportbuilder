...
var
  FrMainPBSampel: TFrMainPBSampel;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmPBSampel;
{$R *.dfm}

procedure TFrMainPBSampel.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainPBSampel.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainPBSampel.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrPBSampel')<>nil then
    begin
       MessageDlg('Modul '+FrPBSampel.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainPBSampel.FormDestroy(Sender: TObject);
begin
  FrMainPBSampel:=nil;
end;

procedure TFrMainPBSampel.FormShow(Sender: TObject);
begin
...
procedure TFrMainPBSampel.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainPBSampel.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainPBSampel.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainPBSampel.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainPBSampel.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainPBSampel.Show1Click(Sender: TObject);
begin
...
procedure TFrMainPBSampel.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrPBSampel) then FrPBSampel.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrPBSampel,FrPBSampel);
        CekOtoritasMenu(IDUser,tag,FrPBSampel.Istambah,FrPBSampel.Ishapus,
          FrPBSampel.Iskoreksi, FrPBSampel.isCetak, FrPBSampel.isExcel);
        FrPBSampel.Show;
      end;
    end
    else
...
procedure TFrMainPBSampel.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainPBSampel.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainPBSampel.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainPBSampel.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainPBSampel.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainPBSampel.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainPBSampel.tvMasterFocusedRecordChanged(
...
procedure TFrMainPBSampel.dxPageControl1Change(Sender: TObject);
begin
 //ToolButton1.Enabled:=(dxPageControl1.ActivePageIndex=0);// and (QuBPPBStok.AsFloat>=QuBPPBSisa.AsFloat);
 //ToolButton2.Enabled:=dxPageControl1.ActivePageIndex=1;
end;

procedure TFrMainPBSampel.tvPOCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrMainPBSampel.tvPOKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  //ToolButton1.Enabled:= (QuBPPBStok.AsFloat>=QuBPPBSisa.AsFloat);
end;

procedure TFrMainPBSampel.tvPOMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
 //ToolButton1.Enabled:= (QuBPPBStok.AsFloat>=QuBPPBSisa.AsFloat);
end;

procedure TFrMainPBSampel.ToolButton4Click(Sender: TObject);
begin
if Iscetak then
...
procedure TFrMainPBSampel.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainPBSampel.frxReport1AfterPrintReport(Sender: TObject);
begin
  NilaiCetak('CetakKe','DbpenyerahanBhn',QuMasterNoBukti.AsString);
end;

procedure TFrMainPBSampel.ToolButton13Click(Sender: TObject);
begin
...
procedure TFrMainPBSampel.ToolButton12Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
