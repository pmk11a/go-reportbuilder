...
implementation
uses FrmBrows, MyProcedure, MyModul, FrmMainPBSampel;
{$R *.DFM}

// PROCEDURE DAN FUNCTION PRIBADI

procedure TFrPBSampel.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrPBSampel.TampilIsiDept;
begin
...
procedure TFrPBSampel.TampilIsiBarang;
begin
  KodeBrows:=300161;
...
procedure TFrPBSampel.IsiNoBuktiBaru;
begin
...
procedure TFrPBSampel.TampilData(No_Bukti,ID:String);
begin
   QuBeli.Close;
...
Function TFrPBSampel.CheckFormMaster:boolean;
begin
   CheckFormMaster:=true;
end;

procedure TFrPBSampel.UpdatedbBeli;
begin
  if (not QuBeli.IsEmpty)and(FrMainPBSampel.ModalKoreksi) then
  begin
...
procedure TFrPBSampel.ClearPanelDetail;
begin
   mValid:=False;
   KodeBrg.Text:='';
   LNamaBrg.Caption:='[ . . . ]';
...
procedure TFrPBSampel.RefreshAll;
begin
  mValid:=False;
  KodeGdg.Text:='G002';
  NoBppB.Text:='';
...
procedure TFrPBSampel.UpdateDataBeli(Choice:Char;Kodet:String);
begin
   if Choice='I' then
   begin
   ...
   FrMainPBSampel.mMainPB_NoBukti:=NoBukti.Text;
   With Sp_Beli do
   begin
       Parameters.Refresh;
...
Procedure TFrPBSampel.AmBilDataBeli;
begin
...
procedure TFrPBSampel.TampilanQnt1Qnt2;
begin
  Qnt1.Enabled:=False;
  Qnt2.Enabled:=False;
  if mNFixBarang then
...
procedure TFrPBSampel.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if mExit=true then
  begin
     Hapus_Daftar_Nomor(TipeTrans ,NoBukti.Text,IDUser);
     FrMainPBSampel.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
     Action:=cafree;
  end
  else
  begin
...
procedure TFrPBSampel.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=13 then SendMessage(handle,WM_NEXTDLGCTL,0,0);
end;

procedure TFrPBSampel.FormShow(Sender: TObject);
begin
  mValid:=False;
  xModalKoreksi:=FrMainPBSampel.ModalKoreksi;
  IsTampil:=False;
  IsTampil1:=False;
  If FrMainPBSampel.ModalKoreksi = False then
  begin
    RefreshAll;
    IsiNoBuktiBaru;
    NoBppB.Text:=FrMainPBSampel.QuBPPBNOBUKTI.AsString;
    TampilData(NoBukti.Text,'0');
    NoUrut.Enabled:=True;
  end
  else
  Begin
    NOBUKTI.Text := FrMainPBSampel.QuMaster.FieldByname('Nobukti').AsString;
...
procedure TFrPBSampel.FormCreate(Sender: TObject);
begin
  TipeTrans := 'BP';
  Hapus_Daftar_Nomor_User(TipeTrans,IDUser);
end;

procedure TFrPBSampel.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrPBSampel.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPBSampel.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPBSampel.KoreksiBtnClick(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    mValid:=False;
...
procedure TFrPBSampel.wwDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
  ...
  begin
  if FrMainPBSampel.ModalKoreksi Then
    KoreksiBtn.Click
  else
    TambahBtn.Click;
  end
...
procedure TFrPBSampel.HapusBtnClick(Sender: TObject);
begin
      if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
      begin
...
procedure TFrPBSampel.wwDBGrid1Enter(Sender: TObject);
begin
  FrPBSampel.KeyPreview:=false;
...
procedure TFrPBSampel.wwDBGrid1Exit(Sender: TObject);
begin
  FrPBSampel.KeyPreview:=true;
end;

procedure TFrPBSampel.wwDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrPBSampel.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {if Key=Vk_Return then
  begin
...
procedure TFrPBSampel.TanggalEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrPBSampel.TanggalExit(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrPBSampel.CetakClick(Sender: TObject);
var N1, N2, N3 : string;
    T1, T2 : Real;
    Tot : byte;
    H1  : real;
...
procedure TFrPBSampel.BitBtn1Click(Sender: TObject);
begin
  if Model='write' then
  begin
...
procedure TFrPBSampel.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
...
procedure TFrPBSampel.FormDestroy(Sender: TObject);
begin
  FrPBSampel:=nil;
end;

procedure TFrPBSampel.KodeGdgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrPBSampel.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrPBSampel.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrPBSampel.SpeedButton8Click(Sender: TObject);
begin
  frxReport1.LoadFromFile(currDir+'Nota\NotaPemakaianBahan.fr3');
  frxReport1.DesignReport;
end;

procedure TFrPBSampel.TanggalChange(Sender: TObject);
begin
  mUbahHd:=True;
end;

procedure TFrPBSampel.NoUrutEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrPBSampel.IsiSatuan;
begin
...
procedure TFrPBSampel.KodeBrgEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrPBSampel.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
  {
...
procedure TFrPBSampel.TampilIsiGudang;
begin
  KodeBrows:=11003;
  Application.CreateForm(TFrBrows, FrBrows);
...
procedure TFrPBSampel.KodeGdgExit(Sender: TObject);
begin
 if mValid Then
  begin
...
procedure TFrPBSampel.QntChange(Sender: TObject);
begin
  if mValid then
  begin
    if NoSat.AsInteger=1 then
...
procedure TFrPBSampel.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrPBSampel.NoSatChange(Sender: TObject);
begin
  if mValid then
  begin
    TampilanQnt1Qnt2;
...
procedure TFrPBSampel.NoBppBExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrPBSampel.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DbpenyerahanBhn',NoBukti.Text);
end;

procedure TFrPBSampel.NoUrutExit(Sender: TObject);
