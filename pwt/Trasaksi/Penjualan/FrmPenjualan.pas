...
uses FrmMainJual, MyProcedure, MyGlobal, MyModul, FrmBrows;

{$R *.dfm}

procedure TFrPenjualan.ClearInputan;
begin
  KodeTipe.Text:='';
  LNamaTipe.Caption:='[ . . . ]';
  KodeSubTipe.Text:='';
...
procedure TFrPenjualan.BtnSimpanClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      FrMainJual.SimpanData('I');
      ClearInputan;
      ActiveControl:=KodeTipe;
    end else
    begin
      FrMainJual.SimpanData('U');
      ModalResult:=mrOk;
    end;
  end
end;

procedure TFrPenjualan.BtnBatalClick(Sender: TObject);
begin
  Close;
end;

procedure TFrPenjualan.FormShow(Sender: TObject);
begin
...
procedure TFrPenjualan.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=13 then
...
procedure TFrPenjualan.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TFrPenjualan.KodeTipeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrPenjualan.NoBuktiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrPenjualan.TanggalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_Escape then
  begin
...
procedure TFrPenjualan.KodeCustSuppEnter(Sender: TObject);
begin
  mValid:=True;
end;

procedure TFrPenjualan.KodeCustSuppExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrPenjualan.KodeTipeExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrPenjualan.KodeSubTipeExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrPenjualan.KodeVlsExit(Sender: TObject);
begin
  if mValid Then
  begin
    if (KodeVls.Text<>'') and
...
procedure TFrPenjualan.NDPPDChange(Sender: TObject);
begin
  if mValid then
  begin
...
procedure TFrPenjualan.NDPPChange(Sender: TObject);
begin
  if mValid then
  begin
    NNET.Value:=NDPP.AsFloat+NPPN.AsFloat;
...
procedure TFrPenjualan.NPPNDChange(Sender: TObject);
begin
  if mValid then
  begin
    NNetD.Value:=NDPPD.AsFloat+NPPND.AsFloat;
...
procedure TFrPenjualan.NPPNChange(Sender: TObject);
begin
  if mValid then
  begin
    NNET.Value:=NDPP.AsFloat+NPPN.AsFloat;
...
procedure TFrPenjualan.HargaChange(Sender: TObject);
begin
