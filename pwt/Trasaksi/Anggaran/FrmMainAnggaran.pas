...
uses
  ...
  cxGridCustomView, cxGrid, dxPageControl, frxClass, frxDBSet, frxDMPExport;

type
  TSaveMethod = procedure (const FileName: String; ASaveAll: Boolean) of object;

  TFrMainAnggaran = class(TForm)
    ToolBar1: TToolBar;
    ...
    tvPOStok: TcxGridDBColumn;
    tvPOQntBPPB: TcxGridDBColumn;
    tvPOQntBP: TcxGridDBColumn;
    tvPOSisa: TcxGridDBColumn;
    tvPOOSPPL: TcxGridDBColumn;
    tvPOOSPO: TcxGridDBColumn;
    tvPODBColumn1: TcxGridDBColumn;
    tvPODBColumn2: TcxGridDBColumn;
    ...
    QuBppBBulan: TIntegerField;
    QuBppBTahun: TIntegerField;
    QuBppBKODEBRG: TStringField;
    QuBppBNamaBrg: TStringField;
    QuBppBSALDOQNT: TBCDField;
    QuBppBSALDO2QNT: TBCDField;
    QuBppBQnt1outPO: TBCDField;
    QuBppBQnt2outPO: TBCDField;
    QuBppBQnt1OutPR: TBCDField;
    QuBppBQnt2OutPR: TBCDField;
    QuBppBQnt1Fiktif: TBCDField;
    QuBppBQnt2Fiktif: TBCDField;
    QuBppBqntmin: TBCDField;
    ...
    QuDetail1perkpers: TStringField;
    tvDetailperkpers: TcxGridDBColumn;
...
var
  FrMainAnggaran: TFrMainAnggaran;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmAnggaran;
{$R *.dfm}

procedure TFrMainAnggaran.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainAnggaran.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainAnggaran.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrAnggaran')<>nil then
    begin
       MessageDlg('Modul '+FrAnggaran.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainAnggaran.FormDestroy(Sender: TObject);
begin
  FrMainAnggaran:=nil;
end;

procedure TFrMainAnggaran.FormShow(Sender: TObject);
begin
...
procedure TFrMainAnggaran.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
   ViewOtorisasiNonBandedTableView(TvMaster,Qumaster,PPLOL);
end;

procedure TFrMainAnggaran.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainAnggaran.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainAnggaran.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainAnggaran.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainAnggaran.Show1Click(Sender: TObject);
begin
...
procedure TFrMainAnggaran.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrAnggaran) then FrAnggaran.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrAnggaran,FrAnggaran);
        CekOtoritasMenu(IDUser,tag,FrAnggaran.Istambah,FrAnggaran.Ishapus,
          FrAnggaran.Iskoreksi, FrAnggaran.isCetak, FrAnggaran.isExcel);
        FrAnggaran.Show;
      end;
    end
    else
...
procedure TFrMainAnggaran.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainAnggaran.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainAnggaran.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainAnggaran.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainAnggaran.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainAnggaran.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainAnggaran.tvMasterFocusedRecordChanged(
...
procedure TFrMainAnggaran.dxPageControl1Change(Sender: TObject);
begin
 //ToolButton1.Enabled:=dxPageControl1.ActivePageIndex=0;
 //ToolButton2.Enabled:=dxPageControl1.ActivePageIndex=1;
end;

procedure TFrMainAnggaran.ToolButton4Click(Sender: TObject);
begin
    if IsCetak then
      begin
        ...
        begin
          Close;
          Sql.Clear;
          Sql.Add('exec CetakPR '+QuotedStr(QuMasterNoBukti.AsString));
          Open;
        end;
        frxDBDataset1.Close;
        frxDBDataset1.Open;
...
procedure TFrMainAnggaran.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainAnggaran.frxReport1AfterPrintReport(Sender: TObject);
var cetakke:Integer;
begin
 DataBersyarat('Select Isnull(Cetakke,0)+1 cetakke from dbAnggaran where nobukti=:0',[QuMasterNoBukti.AsString], Dm.Qucari);
 cetakke:=Dm.Qucari.Fieldbyname('Cetakke').AsInteger;
 with dm.qucari do
  begin
    close;
    sql.Clear;
    Sql.add('Update DBAnggaran set cetakke=:0 where Nobukti='+Quotedstr(QuMasterNoBukti.asstring));
...
procedure TFrMainAnggaran.ToolButton13Click(Sender: TObject);
begin
  {if QuMasterIsOtorisasi1.Value then
  begin
    MessageDlg('No. Purchace Request '+QuMasterNoBukti.AsString+' telah diotorisasi oleh user '+QuMasterOtoUser1.AsString+#13+'pada tanggal '+
  ...
  begin
    ...
    begin
      Otorisasi('DBAnggaran',MenuPPL,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainAnggaran.ToolButton12Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DBAnggaran',MenuPPL,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
...
procedure TFrMainAnggaran.tvMasterCellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TFrMainAnggaran.ToolButton14Click(Sender: TObject);
begin
IF DataBersyarat(' select A.Nobukti,B.NOPPL from DBAnggaran A   '+
                 ' Left Outer Join DbPODET B on A.nobukti = B.noPPL   '+
                 ' where B.NOPPL is null and A.nobukti=:0',[QuMasterNoBukti.ASstring],DM.Qucari)=true then
 begin
    BM := QuMaster.GetBookmark;
    begin
      pembatalan('DBPPL','IsBatal',QuMasterNoBukti.AsString,IDUser,MenuPPL);
      Begin
        ToolButton10.Click;
        if QuMaster.BookmarkValid(BM) then
        begin
 ...
 ShowMessage('Nomor PR '+QuMasterNoBukti.AsString+' Sudah Terdaftar Dalam PO') ;
...
procedure TFrMainAnggaran.tvMasterCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
if AViewInfo.GridRecord.DisplayTexts[17]='True' then
