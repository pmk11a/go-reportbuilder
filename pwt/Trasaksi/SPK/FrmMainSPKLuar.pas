...
var
  FrMainSPKLuar: TFrMainSPKLuar;
  S: array[0..255] of char;

implementation

uses MyModul, MyProcedure, MyGlobal, FrmBrows, FrmSPKLuar;
{$R *.dfm}

procedure TFrMainSPKLuar.GetData(bulan,tahun:integer);
begin
  with QuMaster do
  begin
    Close;
...
procedure TFrMainSPKLuar.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainSPKLuar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if (Application.MessageBox('Anda yakin keluar modul ?','Keluar - Modul',
           MB_YESNO or MB_ICONSTOP + MB_DEFBUTTON2)=IDYES)then
  begin
    if Application.FindComponent('FrSPKLuar')<>nil then
    begin
       MessageDlg('Modul '+FrSPKLuar.Caption+' belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else Action:=caFree;
  end
...
procedure TFrMainSPKLuar.FormDestroy(Sender: TObject);
begin
  FrMainSPKLuar:=nil;
end;

procedure TFrMainSPKLuar.FormShow(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.ToolButton10Click(Sender: TObject);
begin
   GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainSPKLuar.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainSPKLuar.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.Show1Click(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrSPKLuar) then FrSPKLuar.Show else
      begin
        ModalKoreksi:=False;
        Application.CreateForm(TFrSPKLuar,FrSPKLuar);
        CekOtoritasMenu(IDUser,tag,FrSPKLuar.Istambah,FrSPKLuar.Ishapus,
          FrSPKLuar.Iskoreksi, FrSPKLuar.isCetak, FrSPKLuar.isExcel);
        FrSPKLuar.Show;
      end;
    end
    else
...
procedure TFrMainSPKLuar.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainSPKLuar.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsKoreksi then
...
procedure TFrMainSPKLuar.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainSPKLuar.TampilValidClick(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.TampilBatalClick(Sender: TObject);
begin
...
procedure TFrMainSPKLuar.tvMasterFocusedRecordChanged(
...
procedure TFrMainSPKLuar.ToolButton4Click(Sender: TObject);
begin
    if Iscetak then
...
procedure TFrMainSPKLuar.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainSPKLuar.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('CetakKe','DBSPK',QuMasterNOBUKTI.AsString);
end;

procedure TFrMainSPKLuar.ToolButton13Click(Sender: TObject);
begin
{ if QuMasterIsOtorisasi1.Value then
  ...
  begin
    ...
    begin
      Otorisasi('DBSPKLuar',MenuSPK,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainSPKLuar.ToolButton12Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DBSPKLuar',MenuSPK,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
