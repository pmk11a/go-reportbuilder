...
uses
  ...
  Dialogs, ComCtrls, ToolWin, dxCntner, dxTL, dxDBCtrl, dxDBGrid, DB, ADODB,
   StdCtrls, ExtCtrls, Menus, ppBands, ppVar, ppCtrls, ppPrnabl,
  ...
  cxGridCustomView, cxGrid, frxClass, frxDMPExport, frxDBSet, ImgList;

type
  TSaveMethod = procedure (const FileName: String; ASaveAll: Boolean) of object;

  TFrMainKoreksiBJ = class(TForm)
    ToolBar1: TToolBar;
    ...
    QuDetailKodeGrp: TStringField;
    QuDetailKodeSubGrp: TStringField;
    QuDetailSaldoComp: TBCDField;
    QuDetailQntOpname: TBCDField;
    QuDetailSelisih: TBCDField;
  ...
  public
    { Public declarations }
    mPosisi,xStatus,MenuOpname : string;
...
var
  FrMainKoreksiBJ: TFrMainKoreksiBJ;
  S: array[0..255] of char;
implementation
uses myModul,Myprocedure,MyGlobal, 
  FrmBrows, FrmKoreksiBJ ;
{$R *.dfm}

procedure TFrMainKoreksiBJ.GetData(bulan,tahun:integer);
begin
        with QuMaster do
        begin
                Close;
                SQL.Clear;
                SQL.Add('Select * from vwMasterKoreksi where month(tanggal)=:0 and year(tanggal)=:1 and nobukti like ''%OBJ%'' Order by nobukti');
                Prepared;
...
procedure TFrMainKoreksiBJ.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainKoreksiBJ.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ...
  begin
    ...
    begin
       MessageDlg('Modul Transaksi Koreksi Stock belum ditutup !',mtWarning,[mbok],0);
       Action := caNone
    end
    else
    begin
...
procedure TFrMainKoreksiBJ.FormDestroy(Sender: TObject);
begin
  FrmainKoreksiBJ:=nil;
end;

procedure TFrMainKoreksiBJ.FormShow(Sender: TObject);
begin
  xStatus := 'S';
  ToolButton10.Click;
  {if FileExists('inifile\'+FrmainKoreksiBJ.Caption+'_'+IDUser+'.ini') then
  begin
        dxDBGrid1.LoadFromIniFile('inifile\'+FrmainKoreksiBJ.Caption+'_'+IDUser+'.ini');
  end; }
end;

procedure TFrMainKoreksiBJ.ToolButton10Click(Sender: TObject);
begin
   GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainKoreksiBJ.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainKoreksiBJ.ExportExcel1Click(Sender: TObject);
begin
  with QuView do
  begin
    Close;
  ...
  if (IsExcel) then
  begin
      Save('xls', 'Microsoft Excel (*.xls)|*.xls', FrmainKoreksiBJ.Caption, dxDBGrid1.SaveToXLS);
      ShellExecute(Handle, 'Open', PChar(SaveDialog.FileName), nil, nil, sw_shownormal);
      LoggingData(IDUser,'C','Export','',
                  IdUser+' Export Data '+ SaveDialog.FileName)
  end else
  begin
...
procedure TFrMainKoreksiBJ.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiBJ.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiBJ.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiBJ.Show1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiBJ.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrKoreksiBJ) then
         FrKoreksiBJ.Show
      else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrKoreksiBJ,FrKoreksiBJ);
        CekOtoritasMenu(IDUser,Tag,FrKoreksiBJ.Istambah,FrKoreksiBJ.Ishapus,
                        FrKoreksiBJ.Iskoreksi, FrKoreksiBJ.isCetak, FrKoreksiBJ.isExcel);
        FrKoreksiBJ.PanelShow2.Visible:=false;
        FrKoreksiBJ.Show;
        if  ((FrKoreksiBJ.IsKoreksi=False) or (FrKoreksiBJ.IsHapus=False)) then
          FrKoreksiBJ.NoUrut.Enabled:=False
        else  FrKoreksiBJ.NoUrut.Enabled:=True;
      end;
    end
    else
    begin
...
procedure TFrMainKoreksiBJ.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainKoreksiBJ.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
     if IsKoreksi then
        ...
        begin
         ...
         begin
           ...
           begin
             ...
             CekOtoritasMenu(IDUser,Tag,FrKoreksiBJ.Istambah,FrKoreksiBJ.Ishapus,
                          FrKoreksiBJ.Iskoreksi, FrKoreksiBJ.isCetak, FrKoreksiBJ.isExcel);
             FrKoreksiBJ.PanelShow2.Visible:=true;
...
procedure TFrMainKoreksiBJ.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksiBJ.ToolButton4Click(Sender: TObject);
begin
  {if (IsCetak) then
  begin
  ...
  if Iscetak then
    MainCetak(frxDBDataset1,frxDataPerusahaan,DM.QuCari,Dm.QuCari2,'CetakOpnamebarang','Vwperusahaan',QuMasterNobukti.AsString,'Nota\NotaKoreksi.fr3',frxReport1)
else
   MsgTidakBerhakCetakData
end;

procedure TFrMainKoreksiBJ.ppReport1PreviewFormCreate(Sender: TObject);
begin
//  TppReport(Sender).PreviewForm.WindowState:=wsMaximized;
//  TppViewer(TppReport(Sender).PreviewForm.Viewer).ZoomSetting:=zs100Percent;
end;

procedure TFrMainKoreksiBJ.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainKoreksiBJ.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrMainKoreksiBJ.dxDBGrid1CustomDrawFooter(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
procedure TFrMainKoreksiBJ.tvMasterFocusedRecordChanged(
...
procedure TFrMainKoreksiBJ.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('NilaiCetak','DBKoreksi',QuMasterNobukti.AsString);
end;

procedure TFrMainKoreksiBJ.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainKoreksiBJ.ToolButton3Click(Sender: TObject);
begin
 {if QuMasterIsOtorisasi1.Value then
  ...
  begin
    ...
    begin
      Otorisasi('DBKoreksi',MenuOpname,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainKoreksiBJ.ToolButton11Click(Sender: TObject);
begin
 BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DbKoreksi',MenuOpname,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
