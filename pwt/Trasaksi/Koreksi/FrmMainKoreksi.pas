...
procedure TFrMainKoreksi.GetData(bulan,tahun:integer);
begin
        with QuMaster do
        begin
                Close;
                SQL.Clear;
                SQL.Add('Select * from vwMasterKoreksi where month(tanggal)=:0 and year(tanggal)=:1 and nobukti like ''%OPN%'' Order by nobukti');
                Prepared;
...
procedure TFrMainKoreksi.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainKoreksi.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrMainKoreksi.FormDestroy(Sender: TObject);
begin
  FrMainKoreksi:=nil;
end;

procedure TFrMainKoreksi.FormShow(Sender: TObject);
begin
  xStatus := 'S';
  ToolButton10.Click;
  {if FileExists('inifile\'+FrMainKoreksi.Caption+'_'+IDUser+'.ini') then
  begin
        dxDBGrid1.LoadFromIniFile('inifile\'+FrMainKoreksi.Caption+'_'+IDUser+'.ini');
  end; }
end;

procedure TFrMainKoreksi.ToolButton10Click(Sender: TObject);
begin
   GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainKoreksi.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainKoreksi.ExportExcel1Click(Sender: TObject);
begin
  with QuView do
  begin
    Close;
  ...
  if (IsExcel) then
  begin
      Save('xls', 'Microsoft Excel (*.xls)|*.xls', FrMainKoreksi.Caption, dxDBGrid1.SaveToXLS);
...
procedure TFrMainKoreksi.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksi.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksi.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksi.Show1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksi.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(Frkoreksi) then
         FrKoreksi.Show
      else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrKoreksi,FrKoreksi);
        CekOtoritasMenu(IDUser,Tag,FrKoreksi.Istambah,FrKoreksi.Ishapus,
                        FrKoreksi.Iskoreksi, FrKoreksi.isCetak, FrKoreksi.isExcel);
        FrKoreksi.PanelShow2.Visible:=false;
        FrKoreksi.Show;
        if  ((FrKoreksi.IsKoreksi=False) or (FrKoreksi.IsHapus=False)) then
          FrKoreksi.NoUrut.Enabled:=False
        else  FrKoreksi.NoUrut.Enabled:=True;
      end;
    end
    else
    begin
...
procedure TFrMainKoreksi.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainKoreksi.ToolButton2Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
     if IsKoreksi then
        ...
        begin
         ...
         begin
           ...
           begin
             ...
             CekOtoritasMenu(IDUser,Tag,FrKoreksi.Istambah,FrKoreksi.Ishapus,
                          FrKoreksi.Iskoreksi, FrKoreksi.isCetak, FrKoreksi.isExcel);
             FrKoreksi.PanelShow2.Visible:=true;
...
procedure TFrMainKoreksi.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainKoreksi.ppReport1PreviewFormCreate(Sender: TObject);
begin
//  TppReport(Sender).PreviewForm.WindowState:=wsMaximized;
//  TppViewer(TppReport(Sender).PreviewForm.Viewer).ZoomSetting:=zs100Percent;
end;

procedure TFrMainKoreksi.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainKoreksi.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrMainKoreksi.dxDBGrid1CustomDrawFooter(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
procedure TFrMainKoreksi.tvMasterFocusedRecordChanged(
...
procedure TFrMainKoreksi.frxReport1AfterPrintReport(Sender: TObject);
begin
 NilaiCetak('NilaiCetak','DBKoreksi',QuMasterNoBukti.AsString);
end;

procedure TFrMainKoreksi.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrMainKoreksi.ToolButton3Click(Sender: TObject);
begin
 { if (IsCetak) then
  begin
...
procedure TFrMainKoreksi.ToolButton4Click(Sender: TObject);
begin
...
procedure TFrMainKoreksi.ToolButton11Click(Sender: TObject);
begin
BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
