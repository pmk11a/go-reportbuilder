...
implementation
uses FrmBrows,MyProcedure,MyGlobal,MyModul, FrmMainKoreksiBJ;
{$R *.DFM}

// PROCEDURE DAN FUNCTION PRIBADI

procedure TFrKoreksiBJ.IsiHargaBrg;
begin
  with DM.QuCari do
  begin
    Close;
...
procedure TFrKoreksiBJ.IsiNoBuktiBaru;
begin
...
procedure TFrKoreksiBJ.TampilIsiBarang;
begin
  KodeBrows:=2420115;
  //KodeBrows:=9132;
...
procedure TFrKoreksiBJ.TampilIsiGudang;
begin
  KodeBrows:=11004;
  Application.CreateForm(TFrBrows, FrBrows);
  FrBrows.IsiData:=KodeGdg.Text;
...
procedure TFrKoreksiBJ.TampilTombolDetail(pNilai: Boolean);
begin
...
procedure TFrKoreksiBJ.UpdatedbBeli;
begin
...
Function TFrKoreksiBJ.CheckFormMaster:boolean;
begin
...
procedure TFrKoreksiBJ.ClearPanelDetail;
begin
  KodeBrg.Text:='';
   Qnt.Value:=0;
   Qnt2.Value:=0;
...
procedure TFrKoreksiBJ.RefreshAll;
var i:integer;
begin
...
procedure TFrKoreksiBJ.UpdateDataBeli(Choice:Char;Kodet:String);
begin
...
Procedure TFrKoreksiBJ.AmBilDataBeli;
begin
...
procedure TFrKoreksiBJ.TampilData(No_Bukti:String);
begin
  QuBeli.close;
...
procedure TFrKoreksiBJ.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if mExit=true then
  begin
     FrmainKoreksiBJ.xStatus := 'M';
...
procedure TFrKoreksiBJ.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrKoreksiBJ.NoUrutEnter(Sender: TObject);
begin
...
procedure TFrKoreksiBJ.FormCreate(Sender: TObject);
begin
  TipeTrans :='OPBJ';
  Hapus_Daftar_Nomor_User(TipeTrans ,IDUser);
end;

procedure TFrKoreksiBJ.TambahBtnClick(Sender: TObject);
begin
...
procedure TFrKoreksiBJ.KodeBrgKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrKoreksiBJ.QntKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=VK_Escape then
   begin
...
procedure TFrKoreksiBJ.TANGGALKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  ...
  begin
    ...
    begin
       if  (FrmainKoreksiBJ.ModalKoreksi ) then
       UpdatedbBeli;
    end
    else
    begin
   ...
   if (not FrmainKoreksiBJ.ModalKoreksi) then
     begin
      with QuUpdatedbbeli do
       begin
        Close;
  ...
  begin
    If FrmainKoreksiBJ.ModalKoreksi = False then
    begin
      Hapus_Daftar_Nomor_User(TipeTrans ,IDUser);
      IsiNoBuktiBaru;
      Keterangan.Text :='';
...
procedure TFrKoreksiBJ.KodeBrgEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrKoreksiBJ.KoreksiBtnClick(Sender: TObject);
begin
  if CheckFormMaster=true then
  begin
     mValid:=false;
...
procedure TFrKoreksiBJ.HapusBtnClick(Sender: TObject);
begin
  if CheckFormMaster=true then
  begin
...
procedure TFrKoreksiBJ.NoUrutKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {if Key=Vk_Return then
  begin
...
procedure TFrKoreksiBJ.TANGGALEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrKoreksiBJ.TANGGALExit(Sender: TObject);
begin
  if mValid then
  begin
     if CekPeriode(IdUser,TANGGAL.Date)=false then
...
procedure TFrKoreksiBJ.KodeBrgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrKoreksiBJ.BitBtn1Click(Sender: TObject);
begin
  if (not FrmainKoreksiBJ.ModalKoreksi) then
     begin
      with QuUpdatedbbeli do
   ...
   begin
     ...
     ShowMessage('Tanggal Periode Tidak Sesuai !');
   FrmainKoreksiBJ.GetData(StrToInt(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrKoreksiBJ.SpeedButton1Click(Sender: TObject);
begin
  mValid:=false;
  TampilTombolDetail(False);
  ActiveControl:=dxDBGrid1;
...
procedure TFrKoreksiBJ.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
   QuBeli.Sort:=AFieldName;
end;

procedure TFrKoreksiBJ.KodegdgExit(Sender: TObject);
begin
  if mValid Then
  begin
...
procedure TFrKoreksiBJ.KodegdgEnter(Sender: TObject);
begin
  mValid:=true;
end;

procedure TFrKoreksiBJ.QntChange(Sender: TObject);
begin
...
procedure TFrKoreksiBJ.FormShow(Sender: TObject);
begin
 If FrmainKoreksiBJ.ModalKoreksi = False then
  begin
    FTambah:=1;
  ...
  Begin
    NOBUKTI.Text := FrmainKoreksiBJ.QuMaster.FieldByname('Nobukti').AsString;
    NoUrut.Text :=FrmainKoreksiBJ.QuMasterNOURUT.AsString;
    TampilData(NOBUKTI.Text);
    AmBilDataBeli;
    //NoUrut.Text := RightStr(FrMainKoreksiBJ.QuMaster.FieldByname('Nobukti').AsString,4);
  end;
end;

procedure TFrKoreksiBJ.FormDestroy(Sender: TObject);
begin
  FrKoreksiBJ:=nil;
end;

procedure TFrKoreksiBJ.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
  begin
...
procedure TFrKoreksiBJ.dxDBGrid1Enter(Sender: TObject);
begin
  FrKoreksiBJ.KeyPreview:=false;
...
procedure TFrKoreksiBJ.dxDBGrid1Exit(Sender: TObject);
begin
  FrKoreksiBJ.KeyPreview:=true;
end;

procedure TFrKoreksiBJ.dxDBGrid1DblClick(Sender: TObject);
begin
  KoreksiBtn.Click;
end;

procedure TFrKoreksiBJ.BPrevClick(Sender: TObject);
begin
   Hapus_Daftar_Nomor(TipeTrans,mBukti,iduser);
   RefreshAll;
   ActiveControl:=TANGGAL;
   ...
   FrmainKoreksiBJ.QuNavigator.Locate('nobukti',FrmainKoreksiBJ.QuMaster.fieldbyname('nobukti').AsString,[LOC,LOP]);
   FrmainKoreksiBJ.QuNavigator.Prior;
   FrmainKoreksiBJ.QuMaster.Locate('nobukti',FrmainKoreksiBJ.QuNavigator.fieldbyname('nobukti').AsString,[LOC,LOP]);

   FormShow(self);
end;

procedure TFrKoreksiBJ.BNextClick(Sender: TObject);
begin
   Hapus_Daftar_Nomor(TipeTrans,mBukti,iduser);
   RefreshAll;
   ActiveControl:=TANGGAL;
   ...
   FrmainKoreksiBJ.QuNavigator.Locate('nobukti',FrmainKoreksiBJ.QuMaster.fieldbyname('nobukti').AsString,[LOC,LOP]);
   FrmainKoreksiBJ.QuNavigator.Next;
   FrmainKoreksiBJ.QuMaster.Locate('nobukti',FrmainKoreksiBJ.QuNavigator.fieldbyname('nobukti').AsString,[LOC,LOP]);

   FormShow(self);
end;

procedure TFrKoreksiBJ.HargaExit(Sender: TObject);
begin
  {if Qnt.Value = 0 then
     Harga.Value :=0; }
end;

procedure TFrKoreksiBJ.NoUrutChange(Sender: TObject);
begin
  if Length(NoUrut.Text)=4 then
  begin
...
procedure TFrKoreksiBJ.QntOpnameChange(Sender: TObject);
begin
...
procedure TFrKoreksiBJ.QntOpnameKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_Escape then
   begin
...
procedure TFrKoreksiBJ.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrKoreksiBJ.CetakClick(Sender: TObject);
begin
  With QuUpdatedbbeli do
  begin
    close;
  ...
  if FrMainKoreksiBJ.Iscetak then
    MainCetak(frxDBDataset1,frxDataPerusahaan,DM.QuCari,Dm.QuCari2,'CetakOpnamebarang','Vwperusahaan',NoBukti.Text,'Nota\NotaKoreksi.fr3',frxReport1)
else
...
procedure TFrKoreksiBJ.SpeedButton8Click(Sender: TObject);
begin
  frxReport1.LoadFromFile(currDir+'Nota\NotaKoreksi.fr3');
  frxReport1.DesignReport;
end;

procedure TFrKoreksiBJ.frxReport1GetValue(const VarName: String;
  var Value: Variant);
begin
...
procedure TFrKoreksiBJ.Qnt2OpnameChange(Sender: TObject);
begin
Selisih2.AsFloat := Qnt2Opname.AsFloat-Stock2Comp.AsFloat;
  if Selisih2.AsFloat >= 0 then
  begin
...
procedure TFrKoreksiBJ.Qnt2OpnameKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=VK_Escape then
   begin
...
procedure TFrKoreksiBJ.frxReport1AfterPrintReport(Sender: TObject);
begin
NilaiCetak('NilaiCetak','DBKoreksi',NoBukti.Text);
end;

procedure TFrKoreksiBJ.NoUrutExit(Sender: TObject);
var myTempNoUrut: String;
begin
 if FrKoreksiBJ.IsKoreksi= false then
 begin
  if mValid then
  begin
