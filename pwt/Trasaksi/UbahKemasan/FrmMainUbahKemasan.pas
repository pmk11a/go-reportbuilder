...
uses
  ...
  cxGridCustomView, cxGrid, frxClass, frxDBSet;

type
  TSaveMethod = procedure (const FileName: String; ASaveAll: Boolean) of object;

  TFrMainUbahKemasan = class(TForm)
    ToolBar1: TToolBar;
  ...
  public
    { Public declarations }
    mPosisi,xStatus,MenuUkemas : string;
...
var
  FrMainUbahKemasan: TFrMainUbahKemasan;
  S: array[0..255] of char;
implementation
uses myModul,Myprocedure,MyGlobal, FrmBrows,FrmSetForm,FrmUbahKemasan ;
{$R *.dfm}

procedure TFrMainUbahKemasan.GetData(bulan,tahun:integer);
begin
        with QuMaster do
        begin
                Close;
                SQL.Clear;
                SQL.Add('Select * from vwMasterUbahKemasan where month(tanggal)=:0 and year(tanggal)=:1 and NoBukti Like ''%KMS%'' Order by nobukti');
                Prepared;
...
procedure TFrMainUbahKemasan.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TFrMainUbahKemasan.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrMainUbahKemasan.FormDestroy(Sender: TObject);
begin
  FrMainUbahKemasan:=nil;
end;

procedure TFrMainUbahKemasan.FormShow(Sender: TObject);
begin
  xStatus := 'S';
  ToolButton10.Click;
end;

procedure TFrMainUbahKemasan.ToolButton10Click(Sender: TObject);
begin
   GetData(strtoint(PeriodBln),StrToInt(PeriodThn));
end;

procedure TFrMainUbahKemasan.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TFrMainUbahKemasan.ExportExcel1Click(Sender: TObject);
begin
  with QuView do
  begin
    Close;
  ...
  if (IsExcel) then
  begin
      Save('xls', 'Microsoft Excel (*.xls)|*.xls', FrMainUbahKemasan.Caption, dxDBGrid1.SaveToXLS);
...
procedure TFrMainUbahKemasan.AutoFilter1Click(Sender: TObject);
begin
...
procedure TFrMainUbahKemasan.ShowGrid1Click(Sender: TObject);
begin
...
procedure TFrMainUbahKemasan.ShowHeader1Click(Sender: TObject);
begin
...
procedure TFrMainUbahKemasan.Show1Click(Sender: TObject);
begin
...
procedure TFrMainUbahKemasan.ToolButton1Click(Sender: TObject);
begin
  if IsLockPeriode(StrToInt(PeriodBln),StrToInt(PeriodThn)) then
  begin
    if IsTambah then
    begin
      if Assigned(FrUbahKemasan) then
         FrUbahKemasan.Show
      else
      begin
        ModalKoreksi:=False;
        xStatus := 'H';
        Application.CreateForm(TFrUbahKemasan,FrUbahKemasan);
        CekOtoritasMenu(IDUser,tag,FrUbahKemasan.Istambah,FrUbahKemasan.Ishapus,
                        FrUbahKemasan.Iskoreksi, FrUbahKemasan.isCetak, FrUbahKemasan.isExcel);
        FrUbahKemasan.PanelShow2.Visible:=false;
        FrUbahKemasan.Show;
      end;
    end
    else
...
procedure TFrMainUbahKemasan.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrMainUbahKemasan.ToolButton2Click(Sender: TObject);
begin
                ...
                begin
                 ...
                 begin}
                   ...
                   begin
                     ...
                     CekOtoritasMenu(IDUser,tag,FrUbahKemasan.Istambah,FrUbahKemasan.Ishapus,
                                  FrUbahKemasan.Iskoreksi, FrUbahKemasan.isCetak, FrUbahKemasan.isExcel);
                     FrUbahKemasan.PanelShow2.Visible:=true;
...
procedure TFrMainUbahKemasan.MultiSelect1Click(Sender: TObject);
begin
...
procedure TFrMainUbahKemasan.ToolButton4Click(Sender: TObject);
begin
{  if (IsCetak) then
  begin
  ...
  begin
    Close;
    Sql.Clear;
    Sql.Add('Exec CetakUbahKemasan '+QuotedStr(QuMasterNobukti.AsString ));
    open
  end;
  frxDBDataset1.close;
  frxDBDataset1.Open;
...
procedure TFrMainUbahKemasan.ppReport1PreviewFormCreate(Sender: TObject);
begin
//  TppReport(Sender).PreviewForm.WindowState:=wsMaximized;
//  TppViewer(TppReport(Sender).PreviewForm.Viewer).ZoomSetting:=zs100Percent;
end;

procedure TFrMainUbahKemasan.dxDBGrid1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrMainUbahKemasan.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TFrMainUbahKemasan.dxDBGrid1CustomDrawFooter(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
procedure TFrMainUbahKemasan.tvMasterFocusedRecordChanged(
...
procedure TFrMainUbahKemasan.ToolButton3Click(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      Otorisasi('DBUbahKemasan',MenuUkemas,IDUser,'',QuMasterNoBukti.Value);
      Begin
        ToolButton10.Click;
...
procedure TFrMainUbahKemasan.ToolButton11Click(Sender: TObject);
begin
  BM := QuMaster.GetBookmark;
  {frLogin := TFrLogin.Create(Self);
  frLogin.Label2.Caption :='Otorisasi';
  if frLogin.ShowModal=mrok then  }
  begin
    BatalOtorisasi('DbUbahKemasan',MenuUkemas,IDUser,'',QuMasterNoBukti.Value);
    Begin
      ToolButton10.Click;
      if QuMaster.BookmarkValid(BM) then
      begin
