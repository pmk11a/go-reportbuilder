...
- `frontend/CLAUDE.md` — frontend checklist, folder layout, BFF pattern, i18n, Each/Show rendering
...
- A TanStack Router API changed and the workflow step is wrong
...
3. Implement in order: `types/` → `services/` → `hooks/` → `components/` → `routes/` → `locales/`.
