...
interface IUserSessionsTabProps {
  userId: string
}

/**
 * UserSessionsTab displays active sessions for a user.
 * Reuses logic from SessionsTab component.
 */
export function UserSessionsTab({ userId }: IUserSessionsTabProps) {
  ...
  function getStatusBadgeVariant(session: ISessionInfo): 'success' | 'warning' | 'secondary' {
    if (isExpired(session.expires_at)) return 'secondary'
    if (isExpiringSoon(session.expires_at)) return 'warning'
    return 'success'
  }

  function getStatusLabel(session: ISessionInfo): string {
    if (isExpired(session.expires_at)) return t('sessions.expired', 'Expired')
    if (isExpiringSoon(session.expires_at)) return t('sessions.expiringSoon', 'Expiring Soon')
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              {t('sessions.description', 'View and manage active sessions for this user.')}
            </p>
          </div>
        </div>
