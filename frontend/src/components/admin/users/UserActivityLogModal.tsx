...
interface IUserActivityLogModalProps {
  open: boolean
...
/**
 * UserActivityLogModal displays a modal dialog showing activity logs
 * for a specific user with search, date range filter, and pagination.
 */
export function UserActivityLogModal({
  open,
  onOpenChange,
  user,
}: IUserActivityLogModalProps) {
  ...
  const handleMainDialogOpenChange = (isOpen: boolean) => {
    if (!isOpen) {
      setSelectedLog(null)
    }
    onOpenChange(isOpen)
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleMainDialogOpenChange}>
        ...
        <DialogHeader>
          <DialogTitle>{t('userActivityLog.title', 'Activity Log')}</DialogTitle>
          <DialogDescription>
            {t('userActivityLog.description', 'Activity log for {{name}}', { name: user?.full_name || user?.user_id || '' })}
          </DialogDescription>
        </DialogHeader>

        {/* Filter Section */}
        <div className="flex flex-wrap gap-3 p-4 border-b border-slate-200 dark:border-slate-700">
          <Input
            type="date"
            value={startDate}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <p className="font-medium text-slate-700 dark:text-slate-300">
                {selectedLog?.pemakai || user?.user_id || '-'}
              </p>
            </div>
            <div className="sm:col-span-2">
